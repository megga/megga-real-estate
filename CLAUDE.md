...
- Modals : `createPortal(document.body)` — ⚠ **sauf les sept modales de la Messagerie, portées
  ...
  et 4100, sont le voile et le menu de libellé du Calendrier, posés au-dessus de sa
  bulle à 4000 ; les deux derniers sites, les fantômes du glissé d'un jour à l'autre,
  reprennent 30 et 4000 au lieu d'en inventer une. ⚠ Le « 186 et 51 » écrit le matin
  même ne se reproduisait pas : 183 et 50 à ce motif), dont seulement **10** valent 100.
...
- UPPERCASE dans les titres → capitalize — ✅ **la SEULE des sept règles visuelles
  qui soit encore vraie ET gardée** (16 août 2026). Mesuré le 13.09.2026 :
  7 `textTransform: 'uppercase'` vivants — 3 dans `kyc-report` (papier A4, exempté
...
⛔ **LE JOURNAL D'UN COURRIER PORTE LE FAIT, JAMAIS LE CONTENU** (13.09.2026, D11 amendé). `ingest` et `mail-send` écrivaient l'objet du mail en `object_label` et ses adresses en métadonnées, sans lire la visibilité de la boîte — or `activity_events` est lisible de toute l'agence et du super-admin, et relue sans RLS par les outils IA, les exports et les RPC de la console. Seul contrat : `mailAuditEvent` (`_shared/mail/ingest.ts`), `object_label` NULL, trois identifiants et la date du courrier (`metadata.sent_at`, jamais un `created_at` antidaté), **quelle que soit la visibilité**. Ne pas y remettre l'objet « pour la lisibilité » : la table est append-only, une fuite écrite ne se reprend plus. Rien n'avait fui — remesuré le 13.09.2026, 0 ligne `email_*` en prod. ⚠ `mail_contact_aliases` est **service-role seul** depuis la même date (20260913150000), et `email_sent` ne se compte plus comme de l'e-mail Resend dans la console : il n'est écrit que par la Messagerie (20260913150100). Détail : system-map §6ter.

**Logos des expéditeurs (14.09.2026, « comme Spark ») : résolus CHEZ NOUS, jamais chez un tiers.** La liste, le lecteur et l'écran mobile montrent le logo de la société expéditrice, sinon ses initiales (`MailSenderAvatar`, `useMailSenderLogos`). Spark les tirait de Clearbit, fermé le 08.12.2025 ; ses successeurs exigent qu'on charge chaque logo chez eux depuis le navigateur — ils verraient passer, domaine par domaine, les correspondants de chaque agence. L'edge `mail-logos` les résout donc elle-même (`_shared/mail/logos.ts` : BIMI, puis les icônes du site du domaine, via `safeFetchResponse`), pour les seuls domaines que la boîte a REÇUS, et les range dans `mail_sender_logos` **par boîte**, sous `mail_account_visible` : une boîte personnelle ne prête pas ses domaines au bureau. ⛔ Aucune recopie d'une boîte à l'autre — une réponse instantanée trahirait au chronomètre qu'une autre boîte reçoit ce domaine. Messageries de particuliers exclues, sans requête. Validé sur dix domaines réels (BIMI : UBS, PayPal, Homegate, Comparis ; icônes : BCGE, Raiffeisen, Swisscom, Naef, SPG). ⛔ **Toute résolution DNS se fait au NOM COMPLET, point final compris** — ici et dans `_shared/safe-fetch.ts` : sans lui, un nom sans l'enregistrement demandé repart vers le domaine de recherche du résolveur, 5 s perdues par requête (mesuré le 14.09.2026). ✅ En production depuis le 15.09.2026 (PR #1328) : migration `20260915080000` appliquée par `deploy.yml` le jour de la fusion, `mail-logos` déployée (404 → 401 sans jeton). Cerveau : `megga/messagerie-logos`.

**Le dossier Spam (14.09.2026, en production depuis le 15.09.2026) : le spam n'apparaît QUE dans « Spam ».** ⛔ Il fuyait dans « Archivé » : le lot 1 l'écartait à l'import, pas ce qui y entrait ensuite, et les trois synchros lisaient « sorti de la Réception » comme archivé. `is_spam` sur `mail_messages` et `mail_threads` (le fil suit son entrant le plus récent), exclu de tous les autres dossiers, des non-lus et des compteurs ; Gmail `SPAM`, Graph `junkemail`, IMAP `\Junk` ; import sur 30 jours. Le spam n'est **rattaché à personne et n'écrit rien au journal** ; « Ce n'est pas un spam » le rattache et le journalise une fois. Un courrier rattaché PUIS signalé garde sa ligne (append-only) : c'est la fiche du contact qui l'écarte (`mail_spam_message_ids`). Migration `20260915080200`, appliquée le jour de la fusion. Détail : system-map §6ter ; cerveau `megga/messagerie-spam`.

**L'e-mail et le Calendrier (15.09.2026, en production le jour même).** Le lecteur montre l'adresse de l'expéditeur à côté de son nom (copiable). « Planifier » ouvre le Calendrier dans un onglet neuf sur un événement pré-rempli, relié à l'e-mail que sa bulle rouvre. ⛔ Le Calendrier n'enregistrait que les visites : tout autre événement partait en relance et revenait « Tâche », titre et fin perdus — il vit désormais dans `calendar_events` (migration `20260915080300`, appliquée à la fusion ; **0 ligne** relevée en prod le 15.09.2026 à 19:28 UTC), la tâche restant une relance au titre relu. Détail : system-map §6ter ; cerveau `megga/calendrier-evenements`.

**Plusieurs messages d'un coup (15.09.2026, en production le jour même).** La pastille d'expéditeur devient une case (au survol, et partout dès qu'une ligne est cochée), Maj+clic coche une plage, une case « tout » coche la page ; la barre de la liste devient celle des gestes — archiver, spam, lu, supprimer (une confirmation qui nomme les objets). `mail-actions` accepte `thread_ids` (50 au plus) et rend **un verdict par fil, jamais un refus en bloc** (`_shared/mail/lot.ts`) : l'écran dit « 11 sur 12 traités ». La sélection se vide dès que la liste change de sens. Cerveau `megga/messagerie-selection`.

⛔ **Les jetons ne sont JAMAIS en colonne.** Ils vivent dans Supabase Vault, atteints par quatre ponts `SECURITY DEFINER` à `search_path` vide (`mail_secret_store` / `_read` / `_update` / `_delete`), révoqués de `public`/`anon`/`authenticated` et accordés au seul `service_role` ; `mail_accounts.vault_secret_id` n'est qu'un pointeur. ⛔ **Et ils ne survivent plus au compte** (13.09.2026) : `delete-account` déconnecte chaque boîte de l'agent AVANT toute destruction (étape 5c), par le même chemin que « Déconnecter » (`_shared/mail/disconnect.ts`, partagé avec `mail-oauth`) — révocation chez Google, secret effacé. La cascade de `mail_accounts` n'atteint pas Vault : jusque-là le jeton survivait au compte, non révoqué, sans plus rien pour le désigner. Un jeton que Google tient déjà pour révoqué (`400 invalid_token`) n'est plus un échec : il rendait la boîte indéconnectable. L'OAuth est un **code + PKCE en pop-up, hors GoTrue** — Supabase Auth ne sait pas détenir un jeton fournisseur pour un usage serveur. ⚠ **L'URI de redirection ne dérive PAS de `MEGGA_APP_URL`** : `redirectUriFor` la bâtit sur l'origine de l'APPELANT, une fois celle-ci trouvée dans la liste blanche `MAIL_OAUTH_ORIGINS` (`_shared/mail/guard.ts`). La rendre pilotable par un réglage casserait la connexion de toutes les boîtes (`redirect_uri_mismatch`), l'URI devant correspondre **caractère pour caractère** à celle enregistrée chez Google et chez Microsoft — exemption inscrite dans `tests/unit/app-url-unique.spec.ts`.

⛔ **CE QUI N'EST PAS ÉPROUVÉ, et qu'aucune CI verte ne dira jamais.** **Aucun appel réel à Google ni à Microsoft n'a jamais eu lieu, dans aucun des trois lots** : tous les tests d'adaptateur injectent un faux `fetch` — ils éprouvent la construction des requêtes et le décodage des réponses, pas le fournisseur. Le code de bout en bout existe désormais des deux côtés ; ce qui manque est **entièrement hors du dépôt**, et Julien seul peut le poser :

1. **Google** — ✅ l'URI `https://app.getmegga.com/oauth/mail/callback` est **ACCEPTÉE** par le client OAuth depuis le 15.09.2026 : l'oracle `authorize` ouvre la page de connexion (mesuré à 19:50 UTC), quand une URI témoin non déclarée rend `redirect_uri_mismatch` — ce que cette URI rendait encore le 14.09.2026. ⚠ L'**API Gmail** (non activée) et le scope **`gmail.modify`** (non déclaré en Data Access) : état écrit ici jusqu'au 14.09.2026, qui ne se lit que dans la console — **non remesuré depuis**. `gmail.modify` est **RESTRICTED**, donc plus strict que les deux scopes Calendar déjà en attente (voir « Se connecter avec Google » plus bas) : écran « application non validée » et plafond de 100 utilisateurs tant qu'il ne l'est pas.
...
**Lot 3 (IMAP / SMTP) — EN PRODUCTION depuis le 15.09.2026, JAMAIS ÉPROUVÉ CONTRE UNE VRAIE BOÎTE.** Écrit et éprouvé en test le 14.09.2026, fusionné avec la [PR #1328](https://github.com/megga/megga-real-estate/pull/1328) le 15.09.2026 à 19:20 UTC (`61221068`) — logos, Spam, gestes en lot et événements du Calendrier compris. `deploy.yml` a appliqué ses quatre migrations (`20260915080000` à `…080300`, HTTP 201 ×4 : la fusion tombant le 15.09, le date-guard les a prises sans renommage) et déployé 89 edges sans échec ; la porte de dérive voyait encore 12 objets absents à 19:23:31 UTC, zéro à 19:24:32 ; relus en prod à 19:28 UTC (`mail_sender_logos` et `calendar_events` sous RLS et vides, `mail_drafts.bcc`, `is_spam` sur `mail_messages` et `mail_threads`). ⚠ **L'écran est parti AVANT la base** : `deploy-app.yml`, lancé par le même push sans attendre `deploy.yml`, servait le CRM dès 19:22:27 UTC — 76 s avant la fin des migrations, 5 min 20 avant les edges. Aucun workflow ne tient l'ordre « migration avant écran ». Toute boîte hors Google et Microsoft se connecte par mot de passe : clients IMAP et SMTP écrits sur `Deno.connectTls` (STARTTLS sur 143 et 587 — la sonde T3.1 a mesuré **587 OUVERT**, 25 seul est filtré), mot de passe dans Vault, hôtes saisis passés à `assertPublicHost`. ⚠ **Plus de tuile Infomaniak ni Bluewin** (décision Julien, « juste mettre IMAP ») : `mail-oauth imap_detect` reconnaît le fournisseur à l'adresse (domaine, puis MX) et pré-remplit les serveurs. C'est la seule voie qui ne dépend d'aucun geste chez Google ou Microsoft — une boîte Gmail y passe aussi, en mot de passe d'application. ✅ **Revue de sécurité du 15.09.2026** : sept failles graves fermées avant la production (injection IMAP et d'en-têtes par le Message-ID, reprise de la boîte d'un collègue, écrasement par un Message-ID repris, courrier piégé, lecteur sans plafond, hôte revérifié à chaque connexion), puis ses huit défauts moyens (un spam tenu hors des conversations, l'UIDVALIDITY avant chaque geste, le débit de `connect_imap`, les logos du spam, les occurrences et les liens des événements du Calendrier, les liens `?fil=` et `?nouveau=`), puis le reste (le sens d'un message décidé par « Envoyés » et plus par `From`, un message IMAP rangé ailleurs retrouvé, le refus temporaire au LOGIN, le secret Vault d'une reconnexion, les noms à virgule, les gestes groupés, les séries du Calendrier sur « Aujourd'hui ») — system-map §6ter, cerveau `megga/messagerie-revue-securite`. ⛔ **Jamais contre une vraie boîte** : l'épreuve T3.8 ([plan du lot 3](docs/superpowers/plans/2026-09-03-messagerie-crm-lot3-imap.md)) attend une boîte de test — et c'est désormais la production qui propose cette voie. Restent l'épreuve T3.8, puis T3.9 : retrait et purge de la sonde `mail-imap-probe`, toujours déployée (401 sans jeton le 15.09.2026 à 19:42 UTC, là où une fonction absente rend 404). Détail : system-map §6ter.
...
**Intégrations :** Resend, Stripe, Google/Outlook Calendar (OAuth), virtual staging (Gemini), Flatfox sync. ⚠ **Resend est le sortant TRANSACTIONNEL de la plateforme** — confirmations, alertes, invitations — il n'a jamais lu une boîte. La lecture et l'envoi depuis la boîte de l'agent passent par **l'API Gmail et Microsoft Graph** (et, depuis le 15.09.2026, par IMAP/SMTP), inscrites ici le 05.09.2026 — la PR #1274 ayant été mergée la veille, ce paragraphe demandait lui-même de les ajouter « le jour du merge ». ⚠ **Inscrites ne veut pas dire éprouvées** : aucun appel réel n'a jamais été fait à Google ni à Microsoft (tous les tests d'adaptateur injectent un faux `fetch`), ni à un vrai serveur IMAP (épreuve T3.8), `MICROSOFT_CLIENT_ID` / `_SECRET` restent absents du projet, et — c'était le troisième motif jusqu'au 05.09.2026 — la route `/oauth/mail/callback` a été **déployée avec le lot 2**. Il ne reste donc plus AUCUN blocage dans le dépôt : ce qui manque à Google et à Microsoft est **entièrement HORS dépôt**, listé au point Messagerie (inscription Entra ID et les deux secrets Microsoft ; côté Google, voir le point 1). ⛔ **« Aucune boîte ne peut être connectée » est FAUX depuis le 15.09.2026** : une boîte IMAP n'attend aucun de ces gestes, et sa voie est en production.
