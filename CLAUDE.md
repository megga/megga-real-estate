...
> Synthèse courte. Pour l'historique détaillé des implémentations datées, voir [docs/CHANGELOG.md](docs/CHANGELOG.md).

### Vue d'ensemble (mise à jour : 15 avril 2026)

Le MVP Compliance-First Transaction OS est en production sur `main`. Fonctionnalités live principales :

- **Marketplace publique** : page `/acheter` (carte Mapbox 3D, comparateur, favoris, alertes email via pg_cron). La table `market_listings` a été vidée temporairement — 12 biens démo statiques en attendant le prochain `scrape-paginated.mjs`.
- **CRM agent** : 11/14 pages connectées Supabase — Contacts, Pipeline 14 colonnes, Matching, Listings, KYC (screening dilisense), ContactDetail, ListingForm (import manuel/duplication/PDF/URL), ActionBoard, Chat, Dashboard.
- **MEGGA AI copilote** : Edge Function `ai-copilot` (Claude Sonnet 4), streaming, context builder, score engine comportemental (buyer/seller score 0-100).
- **Portail vendeur** : accès tokénisé `/portail/:token`, 6 pages (dossier, visites, offres, documents, messages, analyse positionnement).
- **Super-Admin** : 13 pages (rôle `super_admin`, accent violet) — impersonate, Stripe billing, monitoring Pro, feature flags par plan, NPS, audit sécurité.
- **Directory agents** : 2'517 agences + 1'981 agents en DB, pages `/agents` + `/agents/:slug` + `/agences/:slug`, sync hebdomadaire via GitHub Action.
- **Intégrations** : Resend (megga.ch DKIM/SPF), Stripe, Google Calendar + Outlook Calendar (OAuth), C2PA/MEGGA Shield (hash SHA-256).
- **Galerie photo** : swipe tactile, pinch-zoom, vidéo, before/after staging, AI auto-labeling, quality scoring.
- **Mapbox** : 3D buildings, heatmap prix, geocoding, dessin zone, orbit cinématique, light presets, mode immersif plein écran.
- **Virtual staging** : Edge Function Gemini 2.0 Flash (CHF 0.034/image), 5 styles, quotas par plan.
- **Dark mode** : dashboard agent + portail vendeur (CSS variables + `ThemeProvider`).
- **i18n** : FR/DE/EN/IT, ~330 clés (3 pages marketing restantes à migrer).
- **Audit technique + sécurité** : RLS audit appliqué en prod (migration `20260410_001_rls_security_audit.sql`), dark mode complet, boutons ghost, typographie standardisée, TypeScript 0 `any`.

**Score qualité global (audit 10 avril 2026) : 92/100** (vs 85 avant fixes).

**En cours (avril 2026)** : rental listings — ajout de la location en plus de la vente. Plans écrits (`docs/superpowers/plans/2026-04-15-rental-listings-plan-{1..3}.md`), fondations DB + types + utils implémentées (branche `claude/check-system-setup-bz6Xd`). Tâches 4-11 restantes.
...
> Historique détaillé des corrections RLS, refontes, features datées : voir [docs/CHANGELOG.md](docs/CHANGELOG.md).
