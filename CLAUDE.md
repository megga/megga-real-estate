...
compte bouge), 50 (17 août) puis 51 (4 septembre). Le passage de 50 à 51 était net de trois gestes
du 3 septembre : `visit-reminder-hourly` RETIRÉ (il lisait deux GUC inexistants et n'a jamais envoyé
un rappel ; son doublon `visit-reminders-j1` couvrait déjà une fenêtre plus large), et deux jobs
d'hygiène ajoutés — `pg-net-response-vacuum-hourly` (`50 * * * *`, empêche `net._http_response` de
reprendre le gigaoctet par mois qu'un VACUUM FULL vient de rendre) et
`cron-job-run-details-retention` (`55 3 * * *`, 30 jours ; sans elle `get_cron_health` expirait
22 fois sur 24 et l'alerting des crons était aveugle). Le 52ᵉ est **`mail-sync-2min`**
(`*/2 * * * *`), arrivé avec le merge du lot 1 de la messagerie.

⚠ **Ce chiffre-là s'est périmé exactement comme le §8 l'avait annoncé, et la porte est restée
verte** : l'écart d'un job vaut 2 % pour une tolérance de 20 %, donc `lint:claude-md` ne pouvait pas
le voir. Corrigé à la main le 05.09.2026, registre compris. Une tolérance large protège d'un faux
rouge à chaque sonde ajoutée ; elle ne remplace pas la main de celui qui merge.
...
**Messagerie (e-mail) : ⚠ LE SOCLE EST EN PRODUCTION, L'ÉCRAN EST SUR UNE PR, ET AUCUNE BOÎTE N'A JAMAIS ÉTÉ CONNECTÉE.** Les trois propositions sont vraies en même temps, et les confondre est le seul vrai piège de ce module.
...
**Lot 2 (l'écran) — sur `claude/crm-messagerie-lot2-front`, [PR #1276](https://github.com/megga/megga-real-estate/pull/1276)**, 15 tâches sur 15. Contre `main` : **137 fichiers, 79 créations, ~26 000 lignes**. Trois routes neuves — `/dashboard/messagerie` (bureau **et** mobile via `ResponsiveRoute`), `/oauth/mail/callback`, `/dev/messagerie`. **22 composants** dans `src/components/crm/messagerie/` + 4 modules purs dans `src/lib/mail/`, **12 hooks**, **252 clés** dans le namespace i18n `messages` (⚠ déclaré depuis longtemps et consommé par PERSONNE jusqu'ici — c'est ce lot qui lui donne enfin un lecteur), **5 specs unitaires** neuves.
...
**Intégrations :** Resend, Stripe, Google/Outlook Calendar (OAuth), virtual staging (Gemini), Flatfox sync, **API Gmail + Microsoft Graph** (messagerie agent — inscrites ici le 05.09.2026, au merge du lot 1, comme cette ligne le prévoyait). ⚠ **Resend est le sortant TRANSACTIONNEL de la plateforme** — confirmations, alertes, invitations — il n'a jamais lu une boîte, et la messagerie ne passe pas par lui. ⛔ **Inscrites ≠ éprouvées** : le code qui les appelle est déployé, mais aucun appel réel n'a jamais eu lieu et aucune boîte n'est connectable tant que les trois gestes hors dépôt ci-dessus ne sont pas faits.
