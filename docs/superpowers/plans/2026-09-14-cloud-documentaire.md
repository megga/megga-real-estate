...
```
 ...
 │ (MIME octets,       (URL signée 60 s    (lien magique,       (sélection, OTP,  (Gemini › proposition,│
 │  sha256, dédup,      ou octets          plafonds, dépôt      filigrane,        DeepSeek › résumé,    │
 │  quota, journal)     filigranés)        public)              journal)          après pii-redaction)  │
 └───────────────┬───────────────────────────────┬──────────────────────────────────┬────────────────┘
                 ▼                               ▼                                  ▼
...
|---|---|---|
| `documents` | **élargie** | + `storage_backend`, `deleted_at`, `replaced_by`, `ai_suggestion jsonb`, `hash_source`, `mime_type`, `page_count` ; `document_category` CHECK élargi (D4) ; index `(agency_id, sha256_hash)`, `(agency_id, property_id) where deleted_at is null`, idem `transaction_id`, `contact_id` |
...
- Une URL signée expirée rend 4xx ; une URL signée d'une autre agence rend 4xx ; le bucket lui-même rend 4xx sans jeton.
...
|---|---|---|---|---|
| **0 — Socle** | migration (§5), `documents-ingest`, `documents-access`, quota dur, catégories + rétention, corbeille, 2 crons, `category='documents'`, specs RLS, `delete-account` étendu | — | 2 · 5 | oui : les 4 fichiers KYB orphelins entrent au registre ; `mail-attachment` et `esign-finalize` passent par le registre élargi |
...
| **6 — R2 UE (volume)** | `_shared/r2-eu.ts`, `storage_backend='r2'`, PUT présigné + multipart, vidéos et visites virtuelles, règle de cycle de vie → accès peu fréquent pour les dossiers clos (≥ 30 jours, relecture facturée), outil de migration Supabase → R2, purge dans les deux coffres | — | 5 | oui, **et seulement quand une agence dépasse 20 Mio par fichier ou 100 Go en tout** — avant, c'est un coût de complexité sans usage |
...
- Pas de **versions** (D15).
- Pas de **chiffrement côté client** : Supabase et R2 chiffrent au repos ; la protection est l'accès signé et le journal. Un chiffrement dont l'agence tient la clé se décide à part, avec son coût de récupération.
...
1. Cerveau : ajouter dans `.claude-flow/knowledge/megga-memory.seed.json` les clés `megga/documents-architecture` (D1-D15 condensées, tables, edges, crons), `megga/documents-face-publique` (les deux pages à jeton, plafonds miroir, OTP), `megga/documents-retention` (catégories → durées, corbeille, purge) ; puis `npm run ruflo:seed`.
