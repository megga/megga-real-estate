...
5. Répondre ⇒ le message est dans le fil Gmail, en-tête `In-Reply-To` = `Message-ID` d'origine ; carte « moi → … » dans le CRM ; `activity_events` `email_sent` sur le contact rattaché, **sans objet ni adresse** (`object_label` NULL, `metadata` = identifiants seuls — D11 amendé le 13.09.2026).
