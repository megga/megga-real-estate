...
| `megga/auth-google-oauth-client` | Le client OAuth vivant (`833483825712-…`), l'URI `api.getmegga.com/auth/v1/callback` (`api.megga.ch` jusqu'au 09.09.2026), et le fait que Google ne réaffiche plus un secret. |
...
**D11 — Rattachement au contact, automatique puis manuel.** À l'ingestion, chaque adresse externe du fil est cherchée dans `contacts` et dans `mail_contact_aliases` (adresses apprises) ; un match unique pose `mail_threads.contact_id`. ⚠ **La recherche passe par une RPC `mail_match_contact_by_emails`, pas par un filtre PostgREST** : l'index est `btree (agency_id, lower(email)) WHERE email IS NOT NULL` — une EXPRESSION. Un `.in('email', …)` ne peut pas s'en servir (mesuré le 03.09.2026 : EXPLAIN retombe sur `idx_contacts_agency_created` + filtre) **et compare en respectant la casse**, donc `Jean.Dupont@ex.ch` ne rattacherait jamais un `jean.dupont@ex.ch` entrant — en silence, sans erreur, le fil restant « Adresse non rattachée ». Rien ne normalise `contacts.email` à l'écriture (`src/hooks/useContacts.ts:126`). Le bandeau « Adresse non rattachée » et la modale « Rapprocher l'adresse » (maquette §7) écrivent `contact_id` **et** apprennent l'alias. ⛔ **`contacts.email` est NULLABLE** — corrigé le 03.09.2026 : la base dit `is_nullable = 'YES'`, et **3 des 15 contacts vivants ont `email IS NULL`**. La conséquence n'est pas théorique : dans un `bool_and` de jetons, `lower(c.email) like '%tok%'` rend NULL (pas FALSE) sur ces lignes, et `bool_and` **ignore les NULL** — le ET de jetons dégénère en OU dès qu'un seul jeton correspond, précisément sur les contacts SANS adresse, c'est-à-dire ceux qu'on cherche pour leur en attacher une. Toute comparaison sur `contacts.email` prend donc un `coalesce(…, '')`. Ce qui reste vrai de la règle : on n'écrase jamais l'adresse de la fiche, on apprend un alias. Chaque message rattaché écrit un `activity_events` (`category='messaging'`, `entity_type='contact'`, `entity_id=contact_id`, `action='email_received' | 'email_sent'`, `object_label` NULL, `actor_kind='system'` + `actor_id NULL` pour tout ce qu'écrit la synchronisation — réception ET copies des Envoyés, donc aussi des `email_sent` —, `'user'` + l'id de l'agent authentifié pour un envoi depuis le CRM, `mail-send`) : c'est ce qui le fait apparaître sur la timeline. ⛔ **AMENDÉ LE 13.09.2026 — LE FAIT, JAMAIS LE CONTENU.** Cette décision écrivait `object_label=objet`, et le code y ajoutait l'expéditeur et les destinataires en métadonnées, sans lire la visibilité de la boîte. Or `activity_events` n'est pas gardée par `mail_account_visible` : lisible de toute l'agence (`events_select`) et du super-admin (contre D14), relue SANS RLS par `get_contact_brief` / `prepare_meeting` (l'objet partait à DeepSeek), les exports PDF et DSAR et les RPC de la console, append-only enfin — l'objet survivait à la suppression du mail comme à D15. La ligne ne porte plus que trois identifiants et une date (`thread_id`, `message_id`, `account_id`, `sent_at` — voir ci-dessous —, plus `kind` à l'envoi), **quelle que soit la visibilité** : une boîte partagée peut redevenir personnelle, et le super-admin ne lit le courrier d'aucune. Contrat unique `mailAuditEvent` (`_shared/mail/ingest.ts`), partagé par `ingest` et `mail-send` ; la cloche de l'agent écarte ces deux actions. ⚠ **Et la DATE du fait** (même jour) : la ligne porte `metadata.sent_at`, l'instant du courrier (ou de l'acceptation par le fournisseur à l'envoi), borné à l'enregistrement, forme `Z` — jamais un `created_at` antidaté, qui reste l'horloge de l'audit (garde des 10 ans, purge). La passe initiale journalisait 90 jours datés du jour de la connexion, dans l'ordre inverse ; la timeline et les outils IA trient désormais par la date du fait. **Décision D14 écrite ici** : que la DATE d'un échange d'une boîte personnelle soit lisible de l'agence est admis, comme le fait lui-même — c'est un horodatage, pas un contenu. ⚠ **Et un envoi, UNE ligne** : un nouveau courrier Outlook envoyé du CRM n'était jamais journalisé (fil provisoire sans contact) — la copie provisoire naît rattachée (`recordPendingSend`), `mail-send` ne journalise que si la copie locale existe (sans elle, c'est la synchro qui ingère la copie du fournisseur comme neuve et la journalise), et une seconde copie sortante au même Message-ID (l'agent en Cc de lui-même) ne rejournalise pas.
...
**D13 — WhatsApp n'est pas une boîte de cet écran.** Le README l'annonce « vue sœur… module distinct si le périmètre ne la demande pas » ; il n'existe aucun fil WhatsApp dans le CRM (§2). ~~La ligne « WhatsApp Business » de l'assistant « Ajouter une boîte » **navigue** vers `/dashboard/settings?tab=integrations` (carte d'appairage existante).~~ ⛔ **Amendé le 14.09.2026 (Julien) : la ligne est RETIRÉE** — « on ne va pas ajouter une boîte avec WhatsApp, ça n'a pas de sens » ; WhatsApp s'appaire dans Réglages › Intégrations, et l'assistant ne propose plus que Google, Microsoft et « Autre boîte (IMAP / SMTP) ». Pas d'entrée « WhatsApp » dans la nav, pas de boîte `wa` dans le sélecteur, pas de modales §7-§9 en version numéro de téléphone.

**D14 — Visibilité d'une boîte : `owner` ou `agency`.** La maquette distingue « boîte générale du cabinet » et « Dr Müller · personnelle ». `mail_accounts.visibility` ∈ `('owner','agency')`, choisie à la connexion (case « Partager avec l'agence », décochée par défaut). RLS : `owner` ⇒ `owner_id = auth.uid()` ; `agency` ⇒ `agency_id = get_my_agency_id()`. Le super-admin ne lit **pas** les messages (aucune policy `is_super_admin()` sur `mail_messages`) : les mails d'une agence ne sont pas de la télémétrie plateforme. ⚠ Jusqu'au 13.09.2026 il en lisait pourtant l'objet et les adresses — recopiés dans `activity_events`, que `super_admin_read_all_events` lui ouvre (voir D11 amendé).

**D15 — Rétention.** Déconnecter une boîte **supprime** ses fils, messages et pièces (`ON DELETE CASCADE`) et le secret Vault — et la suppression du COMPTE de l'agent passe par le même chemin (`disconnectMailAccount`, étape 5c de `delete-account`, 13.09.2026 : la cascade du compte n'atteignait pas Vault, le jeton lui survivait) ; les documents classés au dossier survivent dans `documents` avec leur propre `retention_until`. Les `activity_events` `messaging` de sévérité `info` rejoignent le seau « 3 ans » de `purge_activity_events_retention()` (aujourd'hui non listés ⇒ conservés sans borne, ce qui n'est pas une décision).
...
```
  ...
  app.getmegga.com  │  rail · liste · lecture · 7 modales · pop-up OAuth        │
                    └───────┬──────────────┬──────────────┬─────────────┬──────┘
...
1. L'assistant (étape « OAuth ») appelle `mail-oauth` `{action:'start', provider, origin: location.origin, login_hint, visibility}` → `{url, state}`. ⚠ **`origin` est OBLIGATOIRE** (corrigé le 03.09.2026 : sans lui l'edge rend `400 invalid_origin`) et doit être l'une des `MAIL_OAUTH_ORIGINS` — `https://app.getmegga.com`, `http://localhost:5173`, `http://localhost:5174`. ⚠ Le §6 ne fait déclarer que les deux premières : `localhost:5174` (le repli de Vite quand 5173 est pris) passe l'edge mais rend `redirect_uri_mismatch` chez le fournisseur tant qu'il n'y est pas déclaré aussi. (⚠ Corrigé le 13.09.2026 : ce point, le §6 et les consignes des plans des lots 1-2 donnaient encore `app.megga.ch`, que la liste n'accepte plus depuis le 09.09.2026, #1287 — déclarer l'ancienne URI chez Google ou dans Entra mène à `redirect_uri_mismatch`. Les extraits de code DATÉS du lot 1, eux, gardent l'hôte de leur époque.)
...
| `mail_contact_aliases` | `agency_id`, `email` (unique par `(agency_id, lower(email))`), `contact_id`, `learned_by`, `created_at` | ⛔ service-role SEUL depuis le 13.09.2026 (le socle l'avait ouverte aux membres, UPDATE compris : lecture d'adresses apprises dans une boîte personnelle, réaffectation anonyme) — écrite par `mail-actions link_contact` (adresse d'un correspondant externe du fil, jamais la boîte, un alias d'envoi ni une adresse interne), lue par l'ingestion |
...
1. **Google Cloud Console** (compte **hello@megga.ai**, projet « My First Project » `tribal-dispatch-504619-c1`, client OAuth `833483825712-vh715spjupqcl86qffv3hvffsaqk0g8e`) :
   - Credentials → ce client → **Authorized redirect URIs** : ajouter `https://app.getmegga.com/oauth/mail/callback` et `http://localhost:5173/oauth/mail/callback` — au caractère près ce que rend `redirectUriFor()` (`_shared/mail/guard.ts`). **Ne pas** ajouter de domaine `supabase.co` (cf. `megga/auth-google-oauth-client`).
...
2. **Azure (Entra ID)** — `portal.azure.com` → App registrations → **New registration** « MEGGA » : *Accounts in any organizational directory and personal Microsoft accounts* ; Redirect URIs (Web) : `https://api.getmegga.com/auth/v1/callback` (pour le fournisseur Azure de Supabase, si on veut que le calendrier partage l'inscription — c'est l'URI que GoTrue émet depuis le 09.09.2026), `https://app.getmegga.com/oauth/mail/callback`, `http://localhost:5173/oauth/mail/callback` ; Certificates & secrets → **New client secret** (24 mois) — copier la VALEUR immédiatement ; API permissions → Microsoft Graph, *Delegated* : `Mail.ReadWrite`, `Mail.Send`, `User.Read`, `offline_access` (+ `Calendars.ReadWrite` si le calendrier partage). Puis Supabase → Authentication → Providers → Azure : mettre le même Client ID / Secret (sinon le calendrier Outlook ne rafraîchira jamais ses jetons). ⚠ **AVANT de créer une inscription, regarder celle qui existe** (relevé le 13.09.2026) : le fournisseur Azure de Supabase Auth porte DÉJÀ un client — l'oracle `curl -s -o /dev/null -w '%{redirect_url}' 'https://api.getmegga.com/auth/v1/authorize?provider=azure&redirect_to=https%3A%2F%2Fapp.getmegga.com%2Fauth%2Fcallback'` renvoie vers `login.microsoftonline.com` avec `client_id=365bcffe-131b-46f2-942e-1c10bd2f34c7` et `redirect_uri=https://api.getmegga.com/auth/v1/callback`. Aucun document du dépôt ne nomme cette inscription ni son propriétaire. En créer une seconde et la coller dans ce panneau écraserait ce client en silence ; D3 dit de RÉUTILISER — l'identifier dans Entra, puis l'étendre (URI de la messagerie, permissions `Mail.*`, secret), ou acter par écrit son remplacement.
...
5. Répondre ⇒ le message est dans le fil Gmail, en-tête `In-Reply-To` = `Message-ID` d'origine ; carte « moi → … » dans le CRM ; `activity_events` `email_sent` sur le contact rattaché, **sans objet ni adresse** (`object_label` NULL, `metadata` = identifiants et `sent_at` — D11 amendé le 13.09.2026).
...
9. Rejouer 1-5 avec une boîte Outlook. ⚠ En plus (13.09.2026) : un NOUVEAU message Outlook à un contact ⇒ `email_sent` (acteur l'agent) dès l'envoi, et TOUJOURS une seule ligne après la synchro des Envoyés — y compris en se mettant soi-même en Cc. C'est aussi là que se vérifie la prémisse jamais éprouvée : Exchange garde-t-il l'`internetMessageId` posé au brouillon ? Sinon la ligne `pending:` n'est jamais rapprochée.
