...
4. **`agency_profiles`** : `read_agency_profiles` = `SELECT` / `authenticated` / `USING(true)` sur 5 906 lignes portant 5 906 `claim_token`. `anon` est coupé depuis le 19.07 ; `authenticated` ne l'a jamais été.
