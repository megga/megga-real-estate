...
- **Quotas** (`_shared/labs.ts`, miroir client `src/lib/labs.ts`, garde `tests/unit/labs-studio.spec.ts`) : images = le quota `virtual_staging` du catalogue (Starter 0 · Pro 50 · Entreprise 200) ; vidéos = nouveau poste `labs_video` (0 · 10 · 40). Comptés sur `labs_assets` du mois civil, hors échecs. Coût affiché = ESTIMATION (barème au jeton de fal.ai : 720p ≈ 0,46 $/s, 1080p ≈ 1,04 $/s ; image 2K ≈ 0,10 $).
...
- Gardes : `tests/backend/labs-helpers.spec.ts` (WAV, durées, coûts, prompts de garde), `tests/unit/labs-studio.spec.ts` (miroir écran/edge, section et i18n, codes d'erreur libellés), cliquets de couleur et de grammaire (zone `src/components/crm/labs`, page `LabsPage.tsx`). Cerveau `megga/labs-studio`.
