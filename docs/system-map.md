...
- **IMAP / SMTP (lot 3, sur branche au 14.09.2026, PAS en production)** — toute boîte hors Google et Microsoft, par mot de passe. Client IMAP et client SMTP écrits à la main sur `Deno.connectTls` / `Deno.connect` (TLS d'emblée sur 993/465, STARTTLS sur 143/587 — le PORT décide), `postal-mime` pour le RFC 822. Mot de passe dans Vault (`ImapSecret`), comme un jeton. Deux dossiers synchronisés (Réception, Envoyés), `provider_message_id` = `dossier:uidValidity:uid`, fil recousu par `References` ; la synchro n'ouvre qu'en LECTURE SEULE (`EXAMINE`) ; un message déplacé dans le webmail est rebaptisé par son Message-ID, jamais dupliqué ; l'envoi part en SMTP SANS l'en-tête Cci puis se dépose dans « Envoyés » AVEC lui (`APPEND`). Un mot de passe refusé met la boîte en `reauth_required`, comme un jeton révoqué.
  ...
  - **Plus de tuile Infomaniak ni Bluewin** (Julien, 14.09.2026 : « juste mettre IMAP ») : `imap_detect` reconnaît le fournisseur à l'ADRESSE — domaine (bluewin.ch, gmx.ch, icloud.com…), puis MX pour un domaine d'agence (Infomaniak, Hostpoint, Spacemail, UPC) — et l'assistant pré-remplit des serveurs modifiables. Chaque serveur de `imap-presets.ts` a été mesuré (bannière + certificat, sans identifiants). Google et Microsoft y sont renvoyés vers leur OAuth ; Microsoft n'y a AUCUN serveur IMAP (mot de passe refusé chez lui).
