...
- **Limite connue** : section « More properties » de la fiche **masquée** (pas encore peuplée d'annonces similaires). Formulaire de soumission : `canton`/`postalCode` non collectés (l'agent complète au claim). Pages nav lieu/type/catégorie (`los-angeles`/`houses`/`for-sale`) encore en grille démo US + liens nav morts (404) — à brancher/nettoyer. Pages Agents/About/FAQ/Blog encore démo.
