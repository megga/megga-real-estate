...
conservé pour le matching, qui reste chez l'agent : rien de ce qu'il produit ne part vers l'acheteur
(21.09.2026). Stack React/Vite (Cloudflare Pages) + Supabase (Postgres, 85 edge functions,
...
app.getmegga.com (SPA React CRM, /dashboard/* — console  ─┘         ▲
              ...
              /rendez-vous/:token, /accept-invite/:token)
```
...
- **Cycle de vie des capability tokens (3 août 2026, #1114 + #1119)** — le durcissement de juillet avait scopé les RPC au token mais leur avait laissé **zéro cycle de vie**. Les quatre `*_visit_by_token` ont désormais : péremption **dérivée** (`now() < scheduled_at + 30 j` — pas de colonne, donc pas de dérive `database.ts`), refus des statuts terminaux, annulation et avis à usage unique, note hors 1..5 rendue en booléen au lieu d'un 23514 brut, `search_path` `public, pg_temp`, `VOLATILE` (une fonction `STABLE` est servie en **GET** par PostgREST — donc jeton en query string, donc dans les journaux d'accès), `manage_token` retiré de la réponse, et une ligne `activity_events` par geste.
  ...
  - ~~**Retrait d'un lien de réception**~~ — **sans objet depuis le 21.09.2026** : la table `buyer_reception_links` et les RPC `revoke_reception_link` / `record_buyer_reaction` sont supprimées par `20260921130000_matching_sans_sortie` (0 lien jamais créé en production). La leçon survit à l'objet : un retrait tenu par une SECDEF exige la garde sur l'`UPDATE` final, sans quoi un geste concurrent l'efface.
  - **Droits de table** : `anon` révoqué totalement sur `visits`, `kyc_magic_links`, `kyc_magic_link_uploads` ; `authenticated` perd `TRUNCATE`/`REFERENCES`/`TRIGGER` (**`TRUNCATE` échappe au RLS** — aucune policy ne peut le rattraper). Anti-récidive : ces trois tables sont dans `scripts/check-privilege-drift.mjs` (quatre jusqu'au 21.09.2026, `buyer_reception_links` supprimée).
...
---
## 5. Edge functions (85) — catalogue par domaine
...
| **Email (Resend)** | **sortant transactionnel seul**, depuis `noreply@getmegga.com`, sans `reply_to` : `send-email` · `send-relance-email` (« Aujourd'hui » et copilote, plus jamais le matching) · `send-reminder-email` (refuse `follow_up_sent_property` en 400 depuis le 21.09.2026) · `send-team-invite` · `send-visit-email` · `detect-new-device` — `send-property-email` (fiche bien envoyée à l'acheteur) retirée du dépôt le 21.09.2026, avec `buyer-reception-create/get/react` : le matching reste chez l'agent. ⚠ Un déploiement ne retire rien : les quatre restent en ligne jusqu'à leur dépublication manuelle (`purge-orphan-functions.yml`, sur accord de Julien) |
...
**E · Matching & alertes** : `client_searches` (criteria JSONB) → `matching-engine` **v2** (durci PR #634 : pré-filtre **DUR** `transaction_type`+budget±15%+canton via RPC `match_candidate_listings`, puis scoring **soft** 0-100 — barème dans `app_config.matching_scoring_v2`, déterministe ; + axe **bonus** `pricePosition` en location = position du loyer vs marché du secteur via la MV `market_rent_stats`, PR #674, raison dans `budget.detail`, activation = redéploiement edge) sur `market_listings`+`properties` → `matches` (score+raisons, `score_version`, dédup dure par couple contact×bien, insert via RPC `ON CONFLICT`) → **Atelier Matching** (triptyque plein écran, gestes `E/X/P/R/I/N/V`). ⛔ **Le matching reste chez l'agent (décision de Julien, 21.09.2026)** : aucun geste n'écrit à l'acheteur — ni lien, ni e-mail, ni WhatsApp ; l'agent présente les biens par ses propres moyens et le CRM consigne (garde `tests/unit/matching-sans-sortie.spec.ts`, cerveau `megga/matching-sans-sortie`). « Je l'ai proposé » (E) = `status='sent'`, `sent_via='agent'` + deal `new_lead` (créé/rattaché, `transactions.market_listing_id` si bien de veille) + journal `match_propose` + UNE relance interne à +3 j (canal `task`, par geste : un bien ou une sélection) ; « J'ai relancé » (R) = `sent_at` reset + relance repoussée de 3 j, rien n'est envoyé ; Intéressé (I) / « Pas intéressé » (N) = `interested` / `rejected` ; la Recherche « Ajoute à la sélection » (matchs `suggested` par `insert_market_matches`) au lieu d'envoyer ; la relance J+3 automatique d'`automation-engine` (§1) est retirée ; Plus tard = `snoozed_until`+7 j + reminder custom à échéance ; Écarter = `ignored` (jamais re-proposé) ; Visite = bascule `/dashboard/visits/new` (bien interne). Écritures différées 4,5 s (undo toast avant toute écriture). Alertes email publiques (`market_alerts`/`search-alert` cron via Resend) inchangées.
...
- **Roadmap** : suite « outbound fiable » — ✅ les 6 templates sont approuvés chez Meta depuis le 14-17.08.2026 ; le repli de relance client s'active par `WA_TEMPLATE_FOLLOWUP` (posé le 15.09.2026) et le teaser du morning brief est codé (15.09.2026) ; puis triage numéros inconnus → leads ; DE/IT (les médias sortants de `send_listings` ne sont plus au programme : l'outil est retiré le 21.09.2026). Ph.3 sync temps réel ; purge `raw` (cron quotidien actif). Secrets : `WHATSAPP_WEBHOOK_SECRET`, `META_APP_SECRET`, `WHATSAPP_VERIFY_TOKEN`, `WHATSAPP_PROVIDER`.
