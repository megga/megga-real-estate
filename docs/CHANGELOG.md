...
- **Les données en base basculent en DERNIER**, via `scripts/migrate-domaine-getmegga.mjs` (constat par défaut, `--apply` pour écrire) et non une migration : mesuré, **9 434 lignes** de `market_listings.photos_cf` portent des URLs `img.megga.ch` et alimentent le matching. Les réécrire avant que `img.getmegga.com` ne serve R2, c'est 9 434 annonces sans photos — et une `<img>` cassée ne lève rien.
