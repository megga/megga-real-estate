...
- **L'accent ne portait presque rien.** `sp.accent` n'est lu que **16 fois** dans tout `crm-sugar/`, sur 5 fichiers du pipeline : repeindre l'accent ne se voit quasiment pas. Ce qui porte l'identité du CRM, ce sont les teintes d'étape `SG_STAGE_HUE`, les surfaces et la typo. Le comparateur `/design-system/da-compare` ne montrait donc rien tant qu'il ne rendait pas un atome consommant l'accent (le sélecteur de vue).
