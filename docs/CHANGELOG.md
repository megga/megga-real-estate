...
- **⚠ Date des migrations.** Les douze migrations de l'audit sont datées du 13.09.2026 : `deploy.yml` n'applique que celles datées du jour du merge ou après. Mergées plus tard, elles seraient sautées SANS BRUIT, alors que les edges qui en dépendent partiraient (sans `email_recipient_scope`, `send-email` refuse tout envoi d'agent). Les appliquer à la main avant le merge, ou les re-dater au jour du merge. Toutes ont été éprouvées en production dans des transactions annulées.
