...
- **⛔ Quatre actions IA n'avaient JAMAIS été journalisées — et leurs quotas n'ont donc jamais rien compté.** `kyc-report-import`, `extract-property-pdf`, `extract-property-url` et `virtual-staging` écrivaient `actor_kind: 'ai'` avec un `actor_id` : le CHECK `activity_events_actor_kind_coherence` refuse ce couple, et aucune ne lisait l'erreur (0 ligne en production sur 60 jours). Leurs plafonds mensuels comptent précisément ces lignes — celui du home staging, le poste d'IA le plus cher, compris — et aucun import de rapport KYC externe n'avait laissé de trace LBA. Convention du dépôt désormais appliquée (`actor_id` NULL, agent dans `metadata.profile_id`), erreur lue ; l'import KYC ne livre rien que le journal n'a pas retenu. Garde `tests/unit/audit-acteur-ia.spec.ts` sur TOUTES les edge functions — c'est elle qui a trouvé les deux fonctions que le signalement ne nommait pas.
- **La relance e-mail du « oui » WhatsApp part enfin.** L'exécuteur appelait `send-relance-email` sous la clé de service, que la garde d'agent refuse en 401 depuis le 02.08.2026 : l'agent lisait « L'email n'est pas parti ». L'envoi vit dans `_shared/relance-email-send.ts`, partagé avec l'edge, sous la même garde de sortie (`guardOutboundEmail` : périmètre → STOP → quota), avec l'agence du lien WhatsApp vérifié ; une adresse modifiée depuis le brouillon refuse l'envoi. Garde de toute la famille : `tests/unit/appels-internes-cle-service.spec.ts` — un appel de fonction à fonction sous la clé de service doit viser une fonction qui accepte `isServiceSecret`.
- **Les invitations d'équipe ont un quota, et la limite de sièges compte enfin.** `send-team-invite` appelait `get_agency_member_count`, absente de la production : la limite ne bloquait jamais, et l'envoi n'avait aucun quota alors que tout inscrit est admin de son agence solo. `team_seat_status` compte les sièges (membres + invitations en cours) depuis l'abonnement ; la limite suit `plan_limits_enforced` — **inactive**, comme le quota de biens. Le quota anti-abus, lui, est **actif** : 20 invitations par agence et par 24 h (`app_config.team_invite_daily_cap`), comptées aussi dans le plafond e-mail commun de l'agence. Noms d'agence et d'invitant bornés à 80 caractères dans le gabarit. Migration `20260914090100`.
...
- **S14 — les erreurs ne sortent plus en clair, et l'alerte « nouvel appareil » ne croit plus l'appelant.** Douze fonctions renvoyaient le texte d'une erreur Postgres ou d'un fournisseur (noms de colonnes et de contraintes, corps Resend, texte Deepgram, description OAuth) — dont cinq atteignables SANS compte (`kyc-report-data`, `c2pa-verify`, `log-auth-event`, `stripe-webhook` avant signature, `accept-team-invite`). L'appelant reçoit un code stable (celui que le front lit déjà quand il existe), le détail part caviardé dans `console.error` (`redactedErrorMessage`, extrait d'`audit-edge-error.ts`). `detect-new-device` prenait la TÊTE de `x-forwarded-for` — écrite par l'appelant — et l'imprimait dans l'alerte envoyée à la victime : il lit désormais `trustedClientIp` (et PAS `cf-connecting-ip`, que le rapport suggérait mais qu'un client peut poser lui-même sur les edges Supabase), valide la syntaxe avant ipapi.co. Garde : `tests/unit/edge-corps-erreur.spec.ts` (28 fonctions ; 41 autres, derrière une authentification, restent inventoriées dans la PR). ⚠ Relevé en passant, non corrigé : `kyc-report-import` écrit `actor_kind: 'ai'` avec un `actor_id`, refusé par la contrainte de cohérence sans que l'erreur soit lue.
...
- **S17 — la porte `lint:edge-auth` mesure l'ORDRE, plus seulement la présence.** Les 88 fonctions sont déployées sans vérification de JWT par la passerelle (choix assumé) : la porte vérifiait qu'un symbole de garde était présent, pas qu'il s'exécutait avant ce qu'il protège — c'est ainsi que S1 et S8 étaient passés au vert. `scripts/_shared/edge-guard-order.mjs` lit le gestionnaire de chaque fonction : la première garde doit précéder le premier effet (écriture de table ou de stockage, `auth.admin`, tout `.rpc(` sauf les lectures STABLE vérifiées dans les migrations, envoi d'e-mail ou WhatsApp, appel d'une autre fonction), aucun effet au chargement du module, helpers du fichier et de `_shared` suivis. Et tout expéditeur d'e-mail (21, calculés depuis le code) passe par `guardOutboundEmail` ou nomme son périmètre de destinataires dans une table fermée, qui rougit si une entrée se périme. Deux « violations » trouvées, toutes deux des gardes par jeton que la porte ne connaissait pas (`accept-team-invite`, `onboarding-slots`) : nommées, aucune exemption temporaire. Limites écrites dans l'en-tête : lecture textuelle, pas de flot de contrôle. ⚠ Relevé en passant : la limite de sièges de `send-team-invite` appelle `get_agency_member_count`, **absente de la production** — elle ne bloque jamais.
- **✅ Migrations appliquées en production AVANT le merge** : `20260913170000` et `20260913170100` le 13.09.2026 à 19:13 et 19:14 UTC, après une CI verte où leurs deux specs backend ont réellement tourné ; objets vérifiés, sonde en lecture seule annulée (oracle muet pour une autre agence, `kyc_cases_insert` intacte pour l'agence validée). Jusqu'au déploiement, `IntercomMessenger` et `useSubscription` de `main` reçoivent 42501 sur les colonnes fermées — tolérés, sans effet visible.
...
- **⛔ Le relais e-mail est fermé.** `send-email`, `send-property-email` et `send-relance-email` acceptaient un `to` arbitraire derrière un jeton d'agent — gratuit, l'inscription provisionnant une agence solo — sans limite de débit : un relais ouvert signé DKIM par `noreply@getmegga.com`. Chaque envoi passe désormais par `_shared/email-recipient.ts` : **périmètre** (le destinataire est un contact, un lead attribué, un membre ou l'adresse de l'agence de l'appelant — RPC `email_recipient_scope`, sinon 403), **suppression** (registre STOP, déjà là sur deux des trois), **quota** (60/h, 300/j par agence, `app_config.email_send_caps`, journal `email_send_log` — RPC `email_send_quota_take`, sinon 429). Le repli qui acceptait un document HTML fourni par l'appelant est retiré deux jours avant son échéance ; `tests/unit/email-senders-scope.spec.ts` tient l'ordre périmètre → suppression → quota → Resend. ⚠ Conséquence assumée : un e-mail libre depuis le copilote vers une adresse qui n'est dans aucune fiche est refusé avec un message clair — créer le contact d'abord. Au passage, `useSendEmail.ts` envoyait la clé anon en Bearer et répondait 401 à tous les coups : il passe par `functions.invoke`.
- **⛔ La CI E2E ne frappe plus la production.** Les trois suites Playwright sous bypass (agent, admin, visuelle) démarraient `npm run dev` sans `VITE_SUPABASE_URL`, donc sur le projet cloud codé en repli : mesuré, ~10 000 requêtes par jour depuis les runners GitHub (6 212 × 401, ~4 000 × 400, 749 connexions Realtime, référent `localhost:5173`), zéro écriture aboutie — la RLS a tenu — mais les journaux Postgres de prod noyés de « permission denied ». `playwright.local-supabase.ts` donne aux trois configs l'URL locale (injoignable en CI : les requêtes échouent en `connection refused`, ce que les écrans sous bypass encaissent exactement comme un 401) et REFUSE tout hôte non local ; `tests/unit/e2e-local-supabase.spec.ts` le garde. Même défaut côté base : `realadvisor_health_check` écrivait l'URL de production EN DUR, donc la base locale de la CI postait vers la prod (117 × 401/jour) — elle lit `app_config.supabase_url` (migration `20260913120300`).
- **⛔ Deux supervisions muettes par construction, rétablies.** (1) L'alerte RealAdvisor était refusée en 401 par `send-email` depuis le 02.08.2026 (la garde d'agent rejette toute clé d'API, la fonction SQL en envoie une) : `send-email` reconnaît le secret de service pour le seul gabarit `realadvisor_health_alert`, destinataire lu dans `app_config` et jamais dans le corps. (2) `get_cron_health` expirait encore 22 fois par 24 h malgré la rétention du 03.09 (régime de croisière ~111 000 lignes, 52 parcours par appel) et le `count: 'exact'` Flatfox 23 fois : deux instantanés calculés sous pg_cron (rôle postgres, sans timeout) — `cron_health_snapshot` toutes les 5 min, `app_config.flatfox_active_count` toutes les heures — et les lecteurs (`get_cron_health`, admin-monitoring, page de monitoring via `admin_flatfox_active_count`) ne font plus qu'une lecture. Migrations `20260913120100` et `20260913120200` ; §7 de CLAUDE.md corrigé (54 jobs).
- **S2 — un agent parti ne lit plus ses anciens contacts.** `contacts_select` perd la branche `user_id = auth.uid()` (héritée de la marketplace) : lecture par l'agence seule. Les conversations du copilote gagnent la condition d'agence ; la pile d'onglets (noms de clients) est purgée par `team_remove_member` et par `accept-team-invite`. Migration `20260913130000`.
- **S3 — le journal d'audit n'accepte plus d'acteur forgé.** Trigger BEFORE INSERT sur `activity_events` (invoker) : sous `authenticated`, `actor_id` = l'appelant (posé si absent, refusé sinon) et `actor_kind = 'user'` ; les rôles de service écrivent librement les événements `ai`/`system`. Et une suppression de contact depuis le CRM laisse désormais `contact_deleted` (nom, source, type — jamais e-mail ni téléphone). ⚠ Constaté en relisant : les changements d'étape sont DÉJÀ journalisés par la base (`capture_transaction_lifecycle`), le `logAudit` « Étape changée » du pipeline bureau est un doublon — laissé tel quel, à trancher. Migration `20260913130100`.
- **S4 — en-têtes HTTP.** `public/_headers` sert `X-Frame-Options: DENY`, `Content-Security-Policy: frame-ancestors 'none'` (enforcée, et réduite à cela), HSTS un an, et une CSP complète en **report-only** dont les rapports partent vers Sentry — la promotion en enforcée se décidera sur ces rapports. Garde : `tests/unit/http-headers.spec.ts`.
- **S5 — `anon` n'écrit plus nulle part.** Mesuré en prod : `anon` détenait INSERT/DELETE sur 69 tables par les droits par défaut de Supabase (`anon=arwdDxtm`), la RLS étant le seul verrou. Révoqués — INSERT, UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER, MAINTAIN — sur toutes les tables de `public` que `postgres` possède, sauf l'INSERT de `seller_leads` (seule policy d'écriture anonyme, l'entonnoir public). `authenticated` perd TRUNCATE (la seule écriture qu'aucune policy ne gouverne — `email_delivery_events` y était ouverte), REFERENCES, TRIGGER, MAINTAIN. Les droits par défaut du rôle `postgres` sont resserrés : une table créée demain naît fermée. ⚠ Les tables PostGIS de `supabase_admin` restent hors d'atteinte. Gardes : `check:privileges` (production, propriété étendue et contrôle positif sur `seller_leads`) et `tests/backend/anon-write-privileges.spec.ts` (base fraîche, table neuve comprise). Migration `20260913140000`.
- **S6 — SSRF après contrôle.** `c2pa-sign`, `virtual-staging` et `photo-vision` validaient l'URL puis la lisaient par un `fetch` direct, qui suit les redirections : une URL publique redirigeant vers une adresse interne passait, et le staging publiait ensuite les octets dans le bucket public. `safeFetchResponse` (dans `_shared/safe-fetch.ts`) suit les redirections saut par saut en re-validant chacun (3 au plus — de quoi couvrir `img.megga.ch` → `img.getmegga.com`), plafonne le corps PENDANT la lecture, et rend le type MIME ; `safeFetch` garde son contrat (aucune redirection). La liste d'IP bloquées couvre désormais la plage CGNAT (100.64/10) et les IPv4 internes encapsulées dans une IPv6 (`::ffff:169.254.169.254`). Le staging ne fetch plus la photo qu'une fois (au lieu de deux), et son encodage base64 est chunké (`String.fromCharCode(...octets)` levait RangeError dès quelques centaines de Ko). Gardes : `safe-fetch.test.ts` (32 cas) et `tests/unit/ssrf-photo-fetch.spec.ts` (câblage). ⚠ Résiduel assumé : le rebinding DNS entre la validation et le fetch.
- **S7 — `returnTo` ne mène plus hors du CRM.** `navigate()` n'est pas confiné à l'origine : quand `pushState` jette (`//hote`, `/\hote`, `javascript:`), `@remix-run/router` retombe sur `window.location.assign` — redirection hors du CRM, voire exécution de script, depuis un lien `/dashboard/import-lead?returnTo=…`. `safeInternalPath` n'accepte qu'un chemin qui reste sur l'origine (ni `//`, ni `\`, ni caractère de contrôle, préfixe `/dashboard` pour ImportLead) et rend la valeur BRUTE. `calendarOauth` et `sectionPath` lisent leur table par clé PROPRE : `?from=constructor` rendait une fonction à `navigate()` et figeait l'écran d'arrivée. Garde par les PUITS, pas par les noms de paramètre : `redirection-ouverte.spec.ts` tient la table fermée des 47 points de navigation dynamiques, chacun avec sa raison.
- **S8 — plus de secret comparé à la main, plus de super_admin sans allowlist.** `magic-link-send-email`, `automation-engine` et `matching-engine` passent par `isServiceSecret` (temps constant, clé d'`app_config` comprise) ; `weekly-report` n'a plus que `requireSuperAdmin` (le chemin `x-cron-secret` n'avait aucun appelant) et n'écrit qu'aux super-admins ALLOWLISTÉS, lus par leur e-mail d'authentification. Trois policies et deux triggers de rétention KYC testaient le rôle seul : ils appellent `is_super_admin()` (`20260913160000`). Un compte sans profil pouvait INSÉRER le sien en `super_admin` : le trigger de garde couvre désormais l'INSERT (`20260913160100`). Détecteur `scripts/_shared/edge-secret-compare.mjs` branché dans `check-edge-auth`.
- **S9 — accepter une invitation ne détruit plus l'agence quittée.** L'edge supprimait l'agence solo de l'invité sans regarder ce qu'elle contenait (49 clés étrangères en CASCADE) — rejoué en transaction annulée, trois agences réelles partaient en entier, dossier KYB compris. `release_empty_solo_agency` (`20260913160200`) ne libère qu'une agence VIERGE, liste des tables lue dans `pg_constraint` à l'appel ; sinon elle la garde et journalise `solo_agency_retained`. Une agence qui porte des données ne se quitte que sur confirmation (409 `prior_agency_holds_data`, case à cocher sur la page d'invitation). `DELETE` sur `agencies` retiré aux rôles clients ; `team_remove_member` vide aussi le lien WhatsApp du membre retiré (`20260913160300`) — un agent licencié gardait son copilote dans l'agence.
- **S10 — le lien magique KYC borné.** `magic-link-get` ne sert plus qu'une liste blanche de champs ; le dépôt borne par lien (20 pièces, 100 Mo) en edge ET en base (trigger à verrou sur le lien, `20260913160400`), lit le corps en comptant les octets (une passerelle peut retirer `Content-Length`), ne rend plus de texte d'erreur brut ; la confirmation ne revient plus en arrière. `authenticated` perd l'écriture sur `kyc_magic_link_uploads` : un UPDATE suffisait à rattacher une pièce WhatsApp (nom, numéro de pièce, date de naissance) à un lien public. `kyc-report-data` refuse un jeton de lien magique et tout jeton à plus de 600 s ; les endpoints de rendez-vous ne rendent plus que `expired`/`invalid`.
- **S11 — un seul compte par page.** La pile d'onglets (noms de clients), le brouillon d'import, la vue impersonée, les notes externes et la photo survivaient à la déconnexion et s'affichaient au compte suivant du navigateur. Registre `stockageParCompte` (clés `base:<uid>`), purges « déconnexion / changement / démarrage », gardé par `stockage-inventaire.spec.ts`. Un autre compte (événement d'auth ou écriture du stockage par un autre onglet) recharge la page sans adopter la session ; garde d'identité sur `fetch` ; la déconnexion attend `signOut` (5 s) AVANT de lâcher la session — la navigation de ProtectedRoute interrompait auth-js avant l'effacement du jeton. `crm_tabs_save` refuse une pile d'un autre compte ou d'une autre agence (`20260913160500`, client tolérant tant que PostgREST répond PGRST202) ; `check-migration-idempotence` lit la signature d'une fonction.
- **S12 — les jetons Google/Microsoft ne vivent plus dans le navigateur.** auth-js rangeait `provider_refresh_token` dans le localStorage : l'adaptateur de stockage le retire de toute écriture et le confie, au retour d'une liaison d'agenda, à un dépôt en mémoire lu une fois ; `useAuth` ne le garde plus dans son état. `anon` et `authenticated` pouvaient RÉÉCRIRE `refresh_token` : grant de table retiré, colonnes d'état seules lisibles (`20260913160600`). `save_tokens` échange le jeton de rafraîchissement avec les identifiants de MEGGA et exige le compte LIÉ à l'utilisateur — une session volée ne branche plus l'agenda sur un tiers — et journalise `calendar_connected`. E2E `auth-handoff.spec.ts` (passage vitrine → app, GoTrue réel).
- **✅ Migrations appliquées en production AVANT le merge.** Les quatorze migrations de l'audit (`20260913120000` → `20260913160600`) ont été appliquées à la main le 13.09.2026, entre 15:55 et 16:06 UTC (MCP `apply_migration`, dans l'ordre des fichiers ; chacune a passé son bloc de contrôle), puis vérifiées dans le catalogue — `crm_tabs_save` n'a plus qu'une signature, `get_cron_health` rend 54 jobs en 8 ms, 54 jobs pg_cron actifs, `anon` n'écrit plus que dans `seller_leads`. Une sonde annulée sous `authenticated` a rejoué l'appel du client **déployé** (`crm_tabs_save` à trois arguments, insertion d'audit par l'appelant) : il passe. La date ne compte donc plus pour la base — mergées le jour même, `deploy.yml` les rejoue (idempotentes) ; plus tard, il les saute sans effet. ⚠ Ce que le merge apporte encore : les edge functions et le front ; d'ici là, `accept-team-invite`, `send-email` & co tournent dans leur version de `main`.
- **Non prouvé ici, à prouver après déploiement** : un e-mail réel refusé en 403 depuis le copilote vers une adresse inconnue ; une alerte RealAdvisor réellement reçue ; les journaux Postgres sans « permission denied » de CI le lendemain ; `get_cron_health` sans timeout sur 24 h. Pour S7–S12 : une vraie liaison Google Calendar (aucune n'a jamais tourné — la provenance n'a été éprouvée que contre un faux fournisseur) ; un dépôt KYC réel à travers la passerelle ; le rechargement d'un onglet quand un autre compte se connecte, dans un vrai navigateur ; les specs backend et l'E2E `auth-handoff`, qui ne tournent qu'en CI.

#### Rapport KYC — concept épuré, une seule feuille pour l'écran et WhatsApp (13 septembre 2026)
> Sert les objectifs 2 (risque LAB/KYC) et 4 (transparence). Le rapport A4 en 3 pages passe à la maquette « Rapport KYC A4 - Concept épuré » (handoff `handoff-kyc-rapport-pdf`) : filigrane MEGGA à 5,5 % sur chaque feuille, bandeau « logo · Rapport KYC · Confidentiel », une encre et quatre gris, aucune couleur sémantique — l'état d'un contrôle se lit dans le texte. La première version (`PdfShell` Sugar, pastilles vertes/rouges, `PdfIcon`) est **supprimée**, pas archivée.

- **Un composant, deux routes.** `KycReportDocument` (polices, `@page`, trois feuilles) est monté tel quel par l'aperçu de l'agent (`/dashboard/kyc/:id/export`) et par le rendu headless tokenisé (`/kyc-report/:token`) que Cloudflare Browser Rendering capture pour WhatsApp. Ce que l'agent voit est ce qui part. Le mot-marque est un SVG **inline** (`MeggaWordmark`) : le headless capture dès `#pdf-ready`, une image encore en vol aurait donné un en-tête vide et un filigrane absent sans qu'aucune erreur ne le dise.
- **⛔ L'empreinte « SHA-256 » n'en était pas une.** La première version imprimait ce libellé au-dessus d'un repli FNV de 48 hex, sans fonction de hachage — sur un document LBA. `integrity.ts` calcule un vrai SHA-256 (`crypto.subtle`) sur une forme **canonique** du dossier (identité, verdict, statuts de screening, état de chaque contrôle, SHA-256 de chaque pièce), insensible à l'ordre des lignes rendues par la base et aux libellés traduits : deux rendus du même dossier donnent la même empreinte, et toute pièce ou tout contrôle qui bouge la change. Le serveur ne la sert jamais — c'est le navigateur qui rend qui la calcule, pour l'agent comme pour le headless (gardé par `tests/backend/kyc-report-data.spec.ts`).
- **Rien d'inventé pour remplir la maquette.** « Résidence fiscale » reste « Non renseignée » (aucune adresse de contact en base), l'« Établissement » de la source des fonds devient la **pièce liée** (`source_of_funds_doc_id`), le « Type d'achat » reste l'étape du pipeline. Ce que la maquette montrait et que la base SAIT est maintenant rendu : une ligne de screening **par liste mais un statut par source** (PEP suit `pep_status`, les quatre listes de sanctions `sanctions_status`, « adverse media » porte le pire des deux — un match SECO ne peut plus s'afficher « Clear »), l'**examen complémentaire** est la décision humaine la plus récente non supersédée de `kyc_screening_decisions` (« Listes de sanctions — **faux positif écarté**, examiné le … par … »), et le rapport nomme **qui a validé** (`validated_by` résolu) au lieu de signer du nom du demandeur.
- **La page 3 ne coupe plus en silence** : au-delà de `MAX_DOC_ROWS` (14) pièces, une ligne « … et N autres pièces au dossier » remplace ce que `overflow: hidden` aurait tranché.
- **Ce qui est prouvé, et ce qui ne l'est pas.** Vu sur le banc `/dev/crm?entree=/dashboard/kyc/k1/export` (entrée `?entree=` ajoutée au banc : le rapport s'ouvre par `window.open`, donc hors du banc) dans les trois états des fixtures — validé avec analyse contextuelle, match sanctions avec faux positif tranché, PEP à trancher en vigilance renforcée. La route `/kyc-report/:token` rend sa sentinelle d'erreur sur un jeton invalide. **Prouvé le 13.09.2026 à 12:33** : `KYC-2026-B602` livré en document WhatsApp (`delivered`, trace `kyc_report_sent`), rendu par Cloudflare sur `app.getmegga.com/kyc-report/:token` — le PREMIER passage de bout en bout de cette chaîne depuis juin. Il a fallu quatre correctifs le même jour, tous mesurés en production : le copilote inventait l'envoi (#1302), passait le nom du contact au lieu de son identifiant (#1303), inventait l'absence de dossier puis bouclait sur `open_kyc_case` (#1306), et `CLOUDFLARE_ACCOUNT_ID` portait un espace final qui rendait l'URL de rendu inroutable — `404 code 7003` — depuis sa pose (#1310, nettoyage en code). ⚠ La section « Analyse contextuelle MEGGA AI » ne se rend que si `ai_analysis` existe : `kyc-screening` n'en produit plus depuis #829, seuls des dossiers antérieurs en portent une.
- Gardes remesurées : `couleur-barreaux` (zone `kyc-report` 22 → 4 littéraux), inventaire B4 (`src/pages/agent` 315 → 312, `src/pages/public` 68 → 66) ; `kyc-report-build.spec.ts` (+10) tient le view-model et l'empreinte. i18n : 4 langues, parité verte.

#### Migration de domaine `megga.ch` → `getmegga.com` — phase C, le code (9 septembre 2026)
> Le produit passe sur `getmegga.com` ; `megga.ch` sera libérée pour devenir la **holding**. ⚠ **Seule la phase C est faite** : les phases A (préparation Cloudflare/Supabase/Google/Resend), B (bascule du domaine custom), D (renvois 301) et E (nettoyage) vivent HORS du dépôt. Plan complet et oracles : [migration-getmegga.md](migration-getmegga.md).

- **Périmètre réécrit** : **809 occurrences** dans **221 fichiers**, dont 195 portent désormais `getmegga.com`. Topologie miroir — `getmegga.com` (vitrine + login), `app.getmegga.com` (CRM), `api.getmegga.com` (Supabase), `img.getmegga.com` (R2), `send.getmegga.com` (MAIL FROM Resend).
- **⛔ Le domaine custom Supabase gouverne l'OAuth GLOBALEMENT.** Mesuré au `curl` sur `/auth/v1/authorize` : appelé sur l'URL **`.supabase.co`**, GoTrue répond quand même `redirect_uri=…api.megga.ch…`. `.supabase.co` est donc un filet pour l'**API**, jamais pour l'**OAuth** — l'URI `api.getmegga.com/auth/v1/callback` doit être enregistrée chez Google **avant** la bascule, sinon la connexion Google tombe pour tout le monde en `redirect_uri_mismatch`. Ce fait, et lui seul, impose l'ordre des phases.
- **⛔ Les points ÉCHAPPÉS sont invisibles à une réécriture de masse.** Une substitution littérale de `megga.ch` ne voit pas `megga\.ch` écrit dans une regex — et un `grep` non plus. **Quatre** sites ont été manqués ainsi, dont deux vivants et critiques : `ALLOWED_ORIGINS` d'`extract-lead` (le CRM aurait reçu `Access-Control-Allow-Origin: null`) et `ORIGINES_CRM` du worker vitrine (`/api/geo`, qui échoue **fermé** — repli français, aucune erreur). Un troisième serait devenu **creux** : l'assertion « aucun lien de retour vers l'app » d'`edge-no-html-response` cherchait `megga.ch`, donc serait passée au vert en ne cherchant plus rien.
- **⛔ Une suite verte ne dit RIEN d'un changement de domaine.** Les 2908 tests unitaires sont passés au vert le jour de la bascule — parce que leurs assertions avaient bougé avec le code qu'elles gardent. D'où `tests/unit/domaine-getmegga.spec.ts`, qui cherche **les deux formes** (littérale et échappée), refuse un scan vide, et porte un cliquet : il fait rougir une exemption de transition dont le fichier ne cite plus l'ancien domaine.
- **Ce qui n'a PAS été réécrit, par conception** : `supabase/migrations/` (appliquées — les réécrire ferait diverger le dépôt de la base), `docs/superpowers/plans/` et `specs/`, `CHANGELOG`, `handoff/`, `audits/`. Ces fichiers racontent une époque où `megga.ch` **était** l'adresse. `admin.megga.ch` est conservé partout : hôte **retiré** le 28.07.2026, le réécrire inventerait un domaine qui n'a jamais existé.
- **Les données en base basculent en DERNIER**, via `scripts/migrate-domaine-getmegga.mjs` (constat par défaut, `--apply` pour écrire) et non une migration : mesuré, **814 lignes** de `market_listings.photos_cf` portent des URLs `img.megga.ch` et alimentent le matching. Les réécrire avant que `img.getmegga.com` ne serve R2, c'est 814 annonces sans photos — et une `<img>` cassée ne lève rien. ⛔ **Ce point a annoncé 9 434 pendant quelques heures** : il conflatait « lignes ayant un `photos_cf` » (9 434) et « lignes pointant vers `img.megga.ch` » (814). 🟠 Le reste, **8 620 lignes, porte un `photos_cf` VIDE** — anomalie préexistante, sans rapport avec la migration.
- **⚠ La réécriture mécanique falsifie les mesures datées.** Elle a produit « `getmegga.com` est chez privateemail » (faux : Spacemail) et « `getmegga.com` répond 401 » (faux : il ne sert rien). Trois affirmations actionnables ont été rectifiées à la main dans `CLAUDE.md`, qui porte désormais un bandeau expliquant comment lire ses noms d'hôte.


#### Messagerie CRM — lot 2, l'écran (5 septembre 2026 — PR #1276)
> Sert les objectifs 1 (temps administratif) et 5 (remplacer un outil fragmenté). ⚠ **Trois propositions vraies ensemble** : le socle est en production, l'écran est sur une PR, et **aucune boîte n'a jamais été connectée**. Les confondre est le seul vrai piège de ce module.

- **Ce que le lot ajoute**, mesuré contre `main` : **137 fichiers, 79 créations, ~26 000 lignes**. Trois routes — `/dashboard/messagerie` (bureau **et** mobile via `ResponsiveRoute`, le mobile en lecture seule), `/oauth/mail/callback`, `/dev/messagerie`. **22 composants**, **12 hooks**, 4 modules purs `src/lib/mail/`, **252 clés** i18n dans le namespace `messages` — déclaré de longue date et **consommé par personne** jusqu'ici, il compte désormais **22 lecteurs**.
- **⛔ La trame du corps de mail se relisait elle-même.** `documentElement.scrollHeight` d'une iframe ne descend jamais sous la hauteur de l'iframe : la mesurer, c'est relire la hauteur qu'on vient d'écrire, et la marge de sécurité `+ 8` devenait le moteur d'une boucle. Mesuré sur le banc, fil « Attestation de financement » : **96 px de contenu rendus dans une trame montant 128 → 176 px**, par pas de 8 px toutes les 500 ms, jusqu'à l'expiration des 5 s de mesure — 80 px de vide sous chaque message, et davantage à chaque re-rendu (224 puis 256 px après deux bascules de thème). Corrigé en mesurant le **`body`**, stable à 104 px dès le premier relevé. ⚠ La garde est une clause de **source**, faute de mieux : jsdom ne dispose rien, `scrollHeight` y vaut 0 partout, donc aucune assertion de comportement ne distinguerait les deux nœuds. Elle vit dans la spec de la srcdoc, parce que c'est le **couple** qui doit tenir — la mesure du corps n'est juste que tant que `buildBodySrcdoc` épingle `html,body{margin:0;padding:0}`.
- **Le banc `/dev/messagerie` rend l'écran sans base de données**, à trois états : `full` (42 fils, 12 par page), `empty` (boîte connectée, zéro message) et `none` (aucune boîte — « Nouveau message » désactivé, état vide « Aucune boîte connectée »). C'est la seule surface du module photographiée en régression visuelle. ⛔ **Un banc vert n'est pas une fonctionnalité** : ce sont des fixtures, pas du courrier.
- **⛔ Ce qui n'est PAS prouvé, et qu'aucune CI verte ne dira.** **Aucun appel réel à Google ni à Microsoft n'a jamais eu lieu, dans aucun des deux lots.** En production le 05.09.2026 : `mail_accounts` **0 ligne**, `mail_threads` **0**, `mail_messages` **0**. Trois gestes hors dépôt bloquent : l'URI `…/oauth/mail/callback` absente du client OAuth Google (⇒ `redirect_uri_mismatch`, le consentement ne s'affiche même pas), l'API Gmail non activée avec `gmail.modify` non déclaré (scope **RESTRICTED**), et `MICROSOFT_CLIENT_ID`/`_SECRET` absents (⇒ `503 provider_not_configured`, l'échec voulu : bruyant). L'épreuve §7.4 reste **décochée**, son mode d'emploi littéral écrit sous la case.
- **Deux chiffres se sont périmés en silence sous la tolérance de `lint:claude-md`** : **52** jobs pg_cron et non 51 (le 52ᵉ est `mail-sync-2min`, arrivé au merge du lot 1), **131** hooks et non 119. L'écart d'un job vaut 2 % pour une tolérance de 20 % — la porte ne pouvait pas les voir. Corrigés à la main, registre `claude-md-claims.json` compris. Au passage, `docs/pages.md` avait **cinq** chiffres d'inventaire faux et listait `/dev/mandate-sign`, route dont il ne reste aucun fichier.


#### Messagerie CRM — lot 1, socle backend (4 septembre 2026 — PR #1274, mergé le 05.09.2026)
> Sert les objectifs 1 (temps administratif) et 5 (remplacer un outil fragmenté) : l'agent quitte le CRM pour répondre à un client, et le fil e-mail ne rejoint jamais la fiche contact. ⚠ **Cette entrée décrivait une BRANCHE le jour où elle a été écrite** — mesuré en production le 04.09.2026 : 0 table, 0 fonction, 0 cron. ✅ **Mergée depuis** : le 05.09.2026 la production porte **9 tables `mail_%`, 11 fonctions `mail_%` et le cron `mail-sync-2min` actif**. L'interface est le lot 2 (PR #1276), l'IMAP le lot 3.

- **Les jetons ne touchent jamais une colonne.** Ils vivent dans Supabase Vault, atteints par quatre ponts `SECURITY DEFINER` à `search_path` vide (`mail_secret_store` / `_read` / `_update` / `_delete`), révoqués de `public`/`anon`/`authenticated` et accordés au seul `service_role` ; `mail_accounts.vault_secret_id` n'est qu'un pointeur. ⚠ Le patron copié (`esign_secret_*`, `20260607183000`) n'a que **trois** fonctions — `esign_secret_update` n'existe ni au dépôt ni en prod — donc `mail_secret_update` est **écrite**, pas recopiée, et `vault.update_secret` a été vérifiée en production avant usage (5 paramètres dont 4 à défaut, `coalesce(new_name, s.name)` : l'appel à 2 arguments préserve nom et description).
- **OAuth code + PKCE en pop-up, hors GoTrue** — Supabase Auth ne sait pas détenir un jeton fournisseur pour un usage serveur. ⛔ **L'URI de redirection ne dérive PAS de `MEGGA_APP_URL`** : `redirectUriFor` la bâtit sur l'origine de l'APPELANT, une fois celle-ci trouvée dans la liste blanche `MAIL_OAUTH_ORIGINS`. La rendre pilotable par un réglage casserait la connexion de **toutes** les boîtes (`redirect_uri_mismatch`), l'URI devant correspondre caractère pour caractère à celle enregistrée chez Google et chez Microsoft — exemption argumentée dans `tests/unit/app-url-unique.spec.ts`. Une origine hors liste refuse la pop-up sur-le-champ et à l'écran, au lieu d'envoyer un lien mort.
- **Deux migrations, 9 tables, RLS sur les 9.** `20260904074500_mail_module.sql` : `mail_accounts`, `mail_oauth_states`, `mail_labels`, `mail_threads`, `mail_messages`, `mail_attachments`, `mail_drafts`, `mail_contact_aliases`, `mail_cron_locks` ; 11 fonctions `mail_*` ; `purge_activity_events_retention` **étendue** à la catégorie `messaging` — sans cette ligne les événements messaging `info` seraient conservés sans borne, ce qui n'est pas une décision mais un oubli ; `mail_threads` en `replica identity full`, parce qu'avec l'identité par défaut l'ancienne ligne d'un DELETE ne porte que la clé primaire, donc pas d'`agency_id`, donc le filtre Realtime du lot 2 jetterait l'événement ; cron `mail-sync-2min` `*/2 * * * *`, 25 comptes par tick.
- **`status` autorisait `'error'` et AUCUN chemin de code ne l'écrivait** (`20260904074600_mail_sync_failures.sql`). Seule la reconnexion sortait une boîte du balayage ; les cinq autres façons de mourir — 403 Gmail, 403/429 Graph, pointeur Vault orphelin, erreur d'ingestion, ligne `imap` non gérée — écrivaient `last_error`, que **personne ne lit**, et repartaient indéfiniment. Une boîte morte était rigoureusement indiscernable d'une boîte sans courrier neuf, tout en brûlant un des 25 créneaux du tick. `sync_failures` incrémente, remet à 0 sur succès, élargit le backoff en proportion et bascule `status='error'` au 5ᵉ échec d'affilée. ⚠ Jamais de verdict sur le seul code HTTP : le 403 de Gmail couvre aussi `rateLimitExceeded`, transitoire — un état terminal au premier 403 éteindrait une boîte saine pendant un pic de quota.
- **Un bail en base contre le double balayage** (`mail_cron_locks`) : budget 60 s, intervalle 120 s, TTL 180 s. ⛔ **Zéro ligne mise à jour ne prouve PAS qu'un autre balayage tient le bail** — la ligne peut ne pas exister du tout (base fraîche, purge manuelle), la migration étant son unique créatrice. Chaque tick répondait alors `{ok:true, skipped:'locked'}` en **200, pour toujours** : synchro morte, pg_cron au vert. Semis `on conflict do nothing`, et une erreur PostgREST n'est plus rendue comme « quelqu'un d'autre travaille » mais comme une erreur.
- **5 edge functions** (`mail-oauth`, `mail-sync`, `mail-actions`, `mail-send`, `mail-attachment`) et **9 modules purs** dans `_shared/mail/`, dont **6 portent 103 tests** unitaires ; `sync.ts`, `guard.ts` et `types.ts` n'ont pas de spec propre et ne sont exercés que par les specs backend (`mail-rls.spec.ts` 15 tests, `mail-edges.spec.ts` 9 tests, **0 sauté**). ⚠ Un spec de `_shared/` absent de l'allowlist de `vitest.config.ts` ne tourne **nulle part** — la CI type-check les edges via `deno check` mais exclut les `*.test.ts`, et n'exécute pas `deno test` : les six y sont inscrits.
- **`supabase/config.toml` pose deux `clientId` de test pour le runtime local, et c'est ce qui fait cesser un test d'être creux.** Sans valeur non vide, `mail-oauth start` court-circuitait en 503 `provider_not_configured` à chaque exécution : la construction de l'URL d'autorisation, le défi PKCE, `randomToken` et l'insertion dans `mail_oauth_states` n'étaient exercés par **rien**, et un scope erroné ou un défi cassé serait parti au vert. Aucun appel n'est fait vers Google ou Microsoft avec ; en production les valeurs viennent des secrets du projet.
- **⛔ Ce qui n'est PAS éprouvé, et qu'un 6/6 vert ne dit pas.** **Aucun appel réel à Google ni à Microsoft n'a jamais eu lieu** : tous les tests d'adaptateur injectent un faux `fetch`. Hors dépôt, il manque l'URI `https://app.megga.ch/oauth/mail/callback` dans le client OAuth Google, l'activation de l'API Gmail, la déclaration du scope **`gmail.modify` — RESTRICTED**, donc plus strict que les deux scopes Calendar déjà en attente de vérification (lever le plafond de 100 utilisateurs y ajouterait une évaluation CASA), l'inscription Entra ID et les secrets `MICROSOFT_CLIENT_ID`/`_SECRET` (absents, relevé du 16.08.2026, non re-mesuré). Dans le dépôt, il manque la route `/oauth/mail/callback` — lot 2 : **aucune boîte ne peut être connectée depuis l'app aujourd'hui**.
- **Migrations datées du 04.09** : `deploy.yml:220` saute définitivement toute migration dont le préfixe de date est antérieur au jour UTC du déploiement, et ne la rattrape jamais.
- **À faire le jour du merge, et qu'aucune porte ne réclamera** — ✅ **fait le 05.09.2026**, à la clôture du lot 2 : `CLAUDE.md` §7 passe à **52 jobs pg_cron** (l'écart de 2 % restant dans la tolérance de 20 %, `lint:claude-md` est bien resté **vert sur une prose fausse** pendant 24 h — la prédiction s'est vérifiée mot pour mot) ; §8 « Intégrations » gagne Gmail API et Microsoft Graph, avec la réserve « inscrites ≠ éprouvées » ; `docs/system-map.md` §6ter dit l'état du 05.09 ; `docs/schema.md` avait **déjà** été repris au lot 1, contrairement à ce que la carte prétendait rester dû.


#### MEGGA X devient la direction artistique du CRM (9 août 2026)
> Sert l'objectif 5 (remplacer un outil fragmenté) par la cohérence : un agent qui s'inscrit traversait une rupture visuelle entre l'onboarding (MEGGA X) et le CRM (Sugar). MEGGA X est désormais le défaut ; Sugar reste entièrement résolvable et choisissable dans Réglages › Apparence › Direction.

- **L'accent ne portait presque rien.** `sp.accent` n'est lu que **16 fois** dans tout `crm/`, sur 5 fichiers du pipeline : repeindre l'accent ne se voit quasiment pas. Ce qui porte l'identité du CRM, ce sont les teintes d'étape `CRM_STAGE_HUE`, les surfaces et la typo. Le comparateur `/design-system/da-compare` ne montrait donc rien tant qu'il ne rendait pas un atome consommant l'accent (le sélecteur de vue).
- **La densité n'était un mur que pour la reprise LITTÉRALE.** Même carte deal : **102 px** en Sugar, **239 px** en grammaire vitrine telle quelle (+134 %, soit 3 cartes par colonne au lieu de 8), **112 px** une fois resserrée — +10 %, 8 cartes. La méthode qui lève le mur sans rien inventer : descendre d'un cran sur les échelles de la vitrine sans en sortir (`br-small` 16 px au lieu de `br-large` 24, et 12 px de texte, taille que `h6` et `.badge-light.small` emploient déjà).
- **Deux affirmations à corriger sur la source.** La vitrine **a** une variante claire (74 règles `light-mode`, dont `.card-light-mode` blanche à ombre sans bordure) et **a** une palette d'état (`--system-colors--{red,green,yellow,blue}`). Rien à emprunter à Sugar ni à inventer — à une réserve mesurée près : `red-400` en TEXTE donne 3,1:1 sur blanc (échec AA), il ne tient qu'en aplat avec l'encre `neutral-100`, où il vaut 6,6:1 dans les deux modes.
- **⛔ Une alerte ne peut pas porter l'accent de marque.** Premier essai : « En retard » peint en `#424bfb`, soit le bleu du bouton primaire — l'étiquette se lisait comme une mise en avant, pas comme un avertissement.
- **Le préalable : ~4200 littéraux en variables CSS**, sur **161 fichiers** et 5 dossiers. Les styles en ligne acceptent `var()`, donc aucune signature n'a bougé et la migration s'est faite par codemod. Échelle **normalisée** au passage : le seul dossier `pipeline` portait 18 tailles de police entre 8 et 30 px avec des demi-pas (10/10.5, 12/12.5) — ce n'était pas une échelle, c'étaient 18 littéraux. Ramenée à 13 barreaux : 31 occurrences sur 85 se déplacent, 0,6 px en moyenne, et les étiquettes à 8 et 9 px disparaissent. Coût final sur la carte deal : **75 → 76 px**.
- **⚠ Trois pièges de codemod, tous silencieux.** Exclure un dossier par **basename** (`pipeline`) a aussi sauté `crm-mobile/pipeline/` — **47 littéraux restés en place, sans erreur ni avertissement**, repérés seulement en recomptant par dossier. Le renommage de barreaux doit se faire en **une passe** (`--crm-text-lg`→`--crm-text-md` ET `--crm-text-md`→`--crm-text-sm` se traversent en deux passes). Et les blocs `<style>{`…`}</style>` échappent aux regex d'inline styles (la lookahead bute sur le `px`) : 21 fichiers concernés, 6 valeurs réelles.
- **⚠ Trois défauts que seule la bascule du défaut a révélés.** `graphite-scale.spec.ts` éprouvait l'échelle Graphite à travers `crmPalette()`, qui délègue désormais : six tests vérifiaient les surfaces de la vitrine en croyant vérifier Graphite — **tout test d'une propriété de Sugar doit épingler `da: 'sugar'`**. Poser le bloc de direction au milieu de `:root` avait fait tomber `--logo-filter` et 20 couleurs d'événements DANS ce bloc, indéfinies en Sugar — ni le build ni les tests ne peuvent voir ça. Et `lint:deadcode`, **qui ne tourne qu'en CI**, a rejeté `setCrmDa` inutilisé : une préférence sans UI n'en est pas une, d'où la ligne de réglage.
- **Mécanique**, en trois points : les couleurs passent par `crmPalette()` qui délègue à `mxCrmPalette()` — délégation faite LÀ et non aux 33 points de construction, puisque tous transmettent ensuite la palette en prop, donc aucun composant ne change ; la police et la grammaire passent par `[data-crm-da="meggax"]`, **sans `:root`** pour que le sélecteur vaille aussi sur un conteneur (les propriétés personnalisées héritent, donc une région ne revient à Sugar qu'en redéclarant) ; les tokens ([megga-x-crm/tokens.ts](../src/components/megga-x-crm/tokens.ts)) sont des barreaux **réels** de la feuille vitrine, verrouillés par un test qui relit `megga-x.generated.css` et les seuils AA.

#### CRM en allemand et italien de Suisse : de 59 % d'anglais à 1 % (2 août 2026)
> Sert l'objectif 4 (transparence client) et ouvre le produit hors de l'arc lémanique. Le chantier bilingue de juin avait pose « FR+EN first-class, DE/IT = repli EN » comme contrat v1 ; ce repli n'avait jamais été mesuré, et rien ne le surveillait.

- **La parité i18n ne pouvait pas voir le problème.** Son avertissement « possiblement non traduite » compare DE et IT au **français** : ça prouve seulement que la chaîne n'est pas du français, ce qu'un copier-coller de l'anglais satisfait parfaitement. Or les locales DE/IT ont été amorcées depuis l'EN. Mesure juste (DE contre EN) : **59 % du corpus était mot pour mot l'anglais**, quand la parité annonçait ~1900 chaînes suspectes sur 7800 réelles. Nouveau gate `i18n:coverage:ci` ([scripts/i18n-coverage.mjs](../scripts/i18n-coverage.mjs)), **cliquet et non seuil** : vert à l'état courant, rouge seulement si un commit ajoute de l'anglais non traduit. Il sort deux chiffres, parce qu'un seul mentirait — une chaîne identique dans les *trois* langues est presque toujours un token universel légitime (`Email`, `Pro`, `Status`), environ 490 entrées.
- **⚠ Vérifier qu'un sous-arbre est CONSOMMÉ avant de le traduire.** La priorité annoncée n°1 — `common.portal`, 135 chaînes, « ce que voient les clients des agences » — était du **code mort** : le portail vendeur a été retiré le 26 juillet, les clés sont restées. Au total **583 clés FR = 2332 chaînes sur 4 langues** purgées, dont `messages.chat` (127), `contacts.widgets` (72), `dashboard.actionBoard` (46). Deux pièges pour qui refera ce balayage : `admin.json` est **plat**, avec des clés pointées littérales et des clés construites dynamiquement (`` t(`${ns}.title`) ``) — l'exclure, sinon faux positifs en masse ; et un test sur le motif `` `${sous-arbre}_` `` (pensé pour les pluriels i18next) matche les colonnes DB en snake_case, si bien que `portal_` matche `agency_portal_id` et fait passer le sous-arbre mort à travers.
- **6274 traductions, relues de façon adverse.** 38 lots, chacun traduit puis contre-lu contre le français source. La relecture a rendu **53 corrections qu'aucun contrôle mécanique n'attrape** : une déclinaison (`Name` est un masculin faible, donc « nach Namen »), des ruptures de registre au `du` dans un corpus au `Sie`, un familiarisme (« der Signaturlink geht raus »), des collisions de glossaire (`Objekt` désigne déjà le bien immobilier — donc `Gegenstand` pour l'entité auditée du journal), et `riservatezza` employé pour un **consentement** alors qu'il nomme le secret.
- **Trois défauts préexistants, invisibles à la mesure** parce que ces chaînes comptaient déjà comme traduites : six **eszett** en dur (`Schließen`, `Großbuchstabe`, `Außerhalb`) alors que la lettre n'existe pas en Suisse — le garde-fou vit désormais dans `lint:prose`, scopé aux locales allemandes ; six chaînes italiennes restées en **français**, dont deux boutons disant « Export PDF » au lieu de « Esporta PDF » et un « Dossier KYC » au milieu de `Chiamata`/`Visita`/`Offerta` ; et `kyc.wizard` (161 chaînes) qui ne vivait que sur une branche locale **jamais poussée**.
- **Terminologie légale suisse figée** : FR `LBA` donne `GwG` en allemand (Geldwäschereigesetz, pas le `Geldwäschegesetz` allemand) et `LRD` en italien — jamais `AMLA`, qui est l'anglais ; `OBA-FINMA` donne `GwV-FINMA` / `ORD-FINMA` ; « vigilance » donne `Sorgfaltspflichten` / `diligenza` et **jamais `vigilanza`**, qui nomme la surveillance prudentielle FINMA et rendrait l'écran faux.
- **Écarts assumés** : il reste 98 chaînes DE et 37 IT identiques à l'anglais, toutes vérifiées une par une — de vrais mots de la langue cible (`Name`, `Status`, `Team`, `ideal`, `Enterprise` ; `Password`, `Privacy`, `Follow-up`), des marques, ou des conventions locales (`{{price}} CHF` et `+{{percent}} %` en allemand). `{{agr}}`, l'accord de genre **français** de `composeAiHint`, reste volontairement absent des trois autres langues, comme l'anglais le fait déjà.

#### Console super-admin : refusion dans le CRM, audit et réorganisation (28 juillet 2026)
> Sert l'objectif 1 (temps administratif) et la fiabilité de la supervision. La console avait été isolée sur `admin.megga.ch` trois jours plus tôt ; l'isolation est **annulée** (une seule origine, une seule authentification) et l'audit qui a suivi a sorti cinq pannes de production que le code seul ne montrait pas.

- **La console était injoignable en production.** La règle `public/_redirects` renvoyant `/dashboard/admin/*` vers `admin.megga.ch` a survécu au retrait du domaine : tout rechargement, favori ou lien profond partait en 302 vers le vide, seule la navigation interne fonctionnant. Le serveur de dev ignorant `_redirects`, les suites e2e restaient vertes. **Deuxième cause, indépendante** : les liens des pages étaient restés à la racine (héritage des routes `/agencies`, `/users`) — une ligne d'agence menait au 404, le retour de la fiche agence éjectait vers la vitrine, et la palette ⌘K n'atteignait plus rien. Gardes ajoutées : `tests/unit/redirects-guard.spec.ts` et `tests/unit/admin-console-paths.spec.ts`, plus un parcours e2e cliqué (visiter des URL ne prouve rien des liens rendus).
- **Vestiges de l'app autonome purgés** : l'impersonation visait un hôte en dur, donc ouvrait la **production** depuis un poste de développement (URL désormais relative) ; l'audit d'ouverture dédoublonnait par chargement de page, si bien que seule la première entrée d'une session était journalisée (garde déplacée sur le cycle de vie du composant) ; la console empruntait `data-theme` sans le rendre au CRM ; et huit commentaires d'edge functions annonçaient une garde AAL2 alors que le 2FA est retiré (#873).
- **Douze surfaces sur dix-sept n'avaient aucun état d'erreur** : un échec réseau s'affichait « aucune donnée ». ⚠ La branche d'erreur est gardée par `isError && collection.length === 0` — en React Query v5, un refetch de fond qui échoue lève `isError` **en gardant** `data`, et un motif plat remplacerait une liste valide par une carte d'erreur. Trois écrans mentaient par ailleurs : le bento « Alertes » servait les 10 derniers événements toutes sévérités confondues, le journal d'activité affichait « temps réel » sur un sondage de 30 s, et le bouton de rapport hebdomadaire annonçait « envoyé » quoi qu'il arrive (`functions.invoke` ne lève pas sur erreur applicative).
- **La carte Flatfox du monitoring dépassait le délai d'exécution** : comptage à **17 s** mesurées (bitmap heap scan, 19 934 blocs sur 173k lignes) contre un `statement_timeout` de 8 s — elle échouait donc, en affichant un zéro rassurant. Index partiel `idx_ml_flatfox_sync` ⇒ **43 ms**. Le comptage reste EXACT : l'estimation du planificateur donne 46 023 pour 35 341 lignes réelles.
- **Dérive de schéma découverte par la régénération des types** : `reminders.kind` et `transactions.archived_at` n'étaient jamais arrivés en base — le date-guard de `deploy.yml` saute définitivement toute migration mergée après sa propre date. Quatre gestes du **pipeline agent** en étaient cassés (prochaines actions, premier suivi, planification de visite, archivage). Un fichier de types figé rend ce genre de dérive invisible : le code compile contre un schéma qui n'existe plus.
- **Réorganisation** : kit étendu (`AdminPager` + `useClientPagination`, `AdminSearchInput`, `AdminSegmentBtn`, `AdminConfirm`) ⇒ zéro copie de pagination ou de recherche ; `AdminKpiCard` supprimée ; suppressions destructives confirmées ; six correctifs d'accessibilité ; `marketplace` → `moderation` (ancien chemin redirigé) ; `AdminChangelogPage` → `AdminCommunicationPage` ; tuiles « Claude » et « Portail vendeur » retirées ; monitoring traduit ; 59 clés i18n mortes purgées. Base : `admin_notes` et `get_admin_support_stats` supprimées, rétention 180 j sur `platform_metrics`.
- **Écarts assumés** : `HealthTile` et `OpsStat` NE sont PAS fusionnés dans `AdminStat` (locaux à leur fichier, arrangement et jauge propres) ; quatre groupes de pilules gardent leur rendu (fond au repos `cardSub` sur corps blanc de modale) ; `useAdminNps` reste non borné (ses statistiques sont agrégées côté client, une limite fausserait le score).

#### Calendrier : l'écran « Première connexion » ne bloque plus l'accès (25 juillet 2026)
> Sert l'objectif 1 (temps administratif). Audit d'accès : `/dashboard/calendar` (desktop) affichait un mur plein écran tant qu'aucun agenda Google/Outlook n'était connecté — or **0 ligne dans `google_calendar_tokens` et `outlook_calendar_tokens`** en production, tous comptes confondus. La seule sortie était le lien discret « Plus tard » suivi d'une confirmation en modale, qui posait un flag `localStorage` : changer de navigateur, de machine ou vider les données du site remurait l'agent dehors. Le mobile (`MobileAgendaPage`) n'a jamais eu ce gate.

- **`CalendarOnboarding` supprimée** (−162 lignes, + asset `public/calendar/calendar-glass.svg`, + 6 clés i18n ×4 langues). `CalendarPage` monte désormais `CalendarApp` **sans condition**.
- **`CalConnectBanner`** (dans `CalendarApp`) : bandeau écartable sous la toolbar — titre, mention de confidentialité, Connecter Google / Connecter Outlook, croix. Jamais bloquant ; un calendrier sans agenda externe reste utilisable. La clé `megga.calendar.onboarded` est conservée telle quelle (les agents qui avaient déjà passé l'onboarding ne revoient rien). La config complète (statut, déconnexion, sens de synchro) reste dans Réglages › Intégrations.
- **Retour OAuth vers l'écran d'origine** : `connectGoogleCalendar({ from: 'calendar' })` / `connectOutlookCalendar(…)` propagent `&from=calendar` dans `redirectTo` ; `AuthCallbackPage` le résout via une **table d'aiguillage fermée** (`CALENDAR_RETURN_PATHS`) — une destination libre venue de l'URL de callback serait une redirection ouverte. Sans le paramètre, le comportement historique (Réglages › Intégrations) est inchangé, y compris quand la fonction est passée en référence à un `onClick`.
- **Connexion d'agenda : `linkIdentity` au lieu de `signInWithOAuth`.** Connecter Google/Outlook déclenchait une **ré-authentification complète de la session**, pas une autorisation calendrier scopée : sur un compte e-mail/mot de passe dont l'adresse Google diffère de l'e-mail CRM, Supabase ne trouve pas d'identité à lier automatiquement et bascule l'agent sur un **autre compte** (rôle par défaut `particulier` → renvoyé vers `/portal`). `linkIdentity` lie l'identité au compte connecté et échoue proprement si elle appartient à quelqu'un d'autre, au lieu de basculer. Exception conservée : quand l'identité est **déjà celle du compte courant**, la liaison échouerait (identité déjà prise) alors que `signInWithOAuth` ne risque aucune bascule — c'est la voie utilisée pour re-consentir aux scopes Calendar. Les deux `connect*Calendar` deviennent `async` et renvoient `{ error }` sans jamais jeter ; l'échec est affiché (bandeau du calendrier, toast dans Réglages) au lieu d'être avalé. ⚠ **`linkIdentity` exige « Manual linking » activé** sur le projet Supabase — même prérequis que `useSsoIdentities`, déjà en place pour le link/unlink de Réglages › Sécurité.
- **Switch de nav : `case 'reseau'` retiré des 13 écrans Sugar** (calendrier + 12 autres). L'id n'est émis par aucune nav — `CrmTopNav` n'envoie que ses 7 onglets, `CrmIconRail` que kyc/dashboard/settings, et `ctaTo` des notifications est vide par construction — donc ces branches étaient injoignables et pointaient qui plus est sur `/dashboard/network`, route neutralisée. Les redirections de route `/dashboard/network` et `/dashboard/reseau` → `/dashboard` restent en place pour les anciens liens.
...
- **Teinte sombre « noir » `#000000` par défaut** sur tout le CRM agent (`CRM_TOKENS.noir`, `SUGAR_DARK_TONE`, `sugarThemeTokens` — sweep des 22 call-sites `'meggaAi'`). `CRM_STAGES.color` passe à la rampe grise du handoff (repli hors board ; le kanban est piloté par `CRM_STAGE_HUE`).
- **La prochaine action d'un deal = son prochain reminder actif** (fin du placeholder « Prochaine étape à définir »). Migration `reminders.kind` (call/visit/kyc/match/offer/note, nullable, repli statique type→kind `REMINDER_KIND_BY_TYPE`) ; 5ᵉ query de `usePipelineScreen` (cache-helpers, auto-invalidation croisée Calendrier/ActionBoard) ; `CrmDeal.nextAction` nullable + `reminderId`.
- **Board kanban Sugar Pure** : cadre bento mono-page (la page ne défile plus — molette gérée, auto-scroll rAF au bord pendant le drag), 8 colonnes 252px teintées (`crmStageTint`), cartes 2 gabarits, FLIP framer-motion, header réduit (titre · recherche · pilule Filtres unique · 3 vues · Nouveau deal). **Retirés** : tuiles KPI, bandeau MEGGA AI, pilule rappel KYC (le rappel vit sur la fiche V4), bouton « Importer un lead » du header (reste rail + ⌘K).
...
- **Modale « Nouveau deal » plein cadre** (remplace le drawer 540px, −1 373 lignes) : Contact existant/nouveau · Bien optionnel (valeur reprise du prix) · Étape `CRM_STAGE_HUE` · Valeur. `price_offered` **enfin persisté** + reminder « Premier suivi » J+2 à la création. **Création inline** dans chaque colonne (« + » d'en-tête, Entrée/Échap, « Plus d'options » pré-remplit la modale). Disparus (actés maquette) : 4 archétypes, préremplissage IA, champ prochaine action, guards KYC/mandat, dédup email.
- **Vue Timeline réintroduite** : barres dernière-activité→échéance (passé gris / futur teinté), fenêtre 14 j, groupes repliables (En retard/Aujourd'hui/Cette semaine/Ce mois/Plus tard), **poignée de replanification persistée** (`useRescheduleReminder`, granularité jour, ≥ aujourd'hui, heure conservée) — le proto ne gardait qu'un état local. Vue Liste redessinée (pilules `crmStagePillBg` texte blanc).
