...
#### Appel d'accueil : l'agenda de l'hôte devient une boîte Workspace de MEGGA (5 août 2026)
> Sert les objectifs 1 (temps administratif) et 4 (transparence client). Le compte Google Workspace des rendez-vous existait ; rien ne le reliait au backend. Constat de production : `onboarding_hosts` **0 ligne**, `google_calendar_tokens` **0 ligne**, seule identité Google du projet un gmail personnel de développement — autrement dit la réservation répondait `no_host_available` à **toutes** les agences, et aucun lien Meet n'avait jamais pu être produit.

- **Le montage précédent demandait l'impossible.** `onboarding_hosts.profile_id` était `NOT NULL` et l'agenda ne se lisait que par `google_calendar_tokens`, alimenté par le bouton « connecter mon agenda » de l'écran **agent**. Pour que MEGGA puisse recevoir une agence, il aurait donc fallu que la boîte des rendez-vous soit un **utilisateur d'agence** (`profiles.agency_id` est `NOT NULL`) qui se connecte au CRM et clique : la plateforme devenait locataire d'elle-même, et la disponibilité de toutes les agences tenait au jeton personnel d'une seule personne — révoqué ou parti, tout s'arrête sans prévenir.
- **Compte de service + délégation à l'échelle du domaine** (`_shared/google-workspace.ts`, secret `GOOGLE_WORKSPACE_SA_KEY`). Le backend signe une assertion RS256 et agit **au nom** de `onboarding_hosts.calendar_email`, la boîte Workspace du domaine. Aucun jeton personnel, aucun consentement à renouveler, une autorisation accordée une fois dans la console Workspace et révocable au même endroit. `profile_id` devient facultatif et reste la voie d'un hôte qui préfère son agenda déjà connecté ; **la Workspace passe devant**.
- **⚠ Trois pièges, chacun silencieux, chacun encodé dans le code.** (1) `sub` porte la boîte à incarner : sans lui Google délivre un jeton parfaitement **valide** pour le compte de service lui-même, dont l'agenda est vide — le rendez-vous serait « créé », l'appel rendrait 200, et l'équipe ne verrait jamais rien arriver. (2) Un refus de `freeBusy` arrive en **200, avec `busy` vide et l'erreur à côté** : le lire sans regarder `errors` conclurait « libre toute la journée » et ouvrirait toutes les heures ouvrables à la réservation. Un agenda déclaré mais injoignable rend donc `degraded`, ce qui **écarte** l'hôte — mieux vaut une journée qui paraît pleine qu'un double booking. (3) Le réservant est porté comme **invité** de l'événement : une visioconférence créée par une boîte Workspace n'admet directement que les invités, un absent de la liste tombe sur « demander à rejoindre » et attend. `sendUpdates=none` empêche Google de doubler l'e-mail de Resend.
- **Jamais de repli silencieux.** Quand `calendar_email` est renseigné et que l'authentification échoue, le code **ne retombe pas** sur l'agenda personnel du profil : le rendez-vous atterrirait ailleurs que là où l'équipe le cherche. Garde-fou `tests/unit/google-workspace-delegation.spec.ts`, dont le client Supabase **lève s'il est touché**.
- **`onboarding_calls.calendar_provider` accepte `workspace`.** Un événement de boîte Workspace *est* un événement Google, mais ne s'atteint pas avec la même clé : déplacer ou retirer exige celle qui l'a créé. La source est donc enregistrée sur la ligne au lieu d'être redevinée à chaque geste — sans quoi un hôte qui change de voie laisse son ancienne entrée orpheline à l'heure abandonnée.
- **Console** : colonne « Agenda » (MEGGA / Membre / **Aucun**, en teinte d'avertissement), choix de la voie au formulaire d'hôte, et bouton **« Tester la connexion »** qui fait un aller-retour réel. Il n'est pas décoratif : trois des quatre pièces du montage vivent chez Google — délégation, portée exacte, activation de l'API Calendar — et aucune lecture locale ne peut les voir. Sans lui, le premier symptôme d'une pièce manquante serait « aucun créneau » chez une agence, à l'autre bout de la plateforme. Chaque motif de refus désigne **une** case à corriger, au lieu de recracher `unauthorized_client`.
- **Écart assumé** : `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` manquaient à l'inventaire des secrets de CLAUDE.md alors qu'ils sont lus depuis longtemps ; ils y entrent, avec la mention qu'ils servent l'OAuth **par utilisateur** et n'ont aucun rôle dans la délégation.
