...
- **Mini-carte Mapbox réelle (10 juin 2026)** : le placeholder rayé prévu par le handoff est remplacé par une vraie carte (`SgaMiniMap`, image statique Mapbox — non-interactive, pas d'instance GL). Colonne 2 + modal « Annonce complète ». Suit le thème (`light-v11` pin noir / `dark-v11` pin blanc), token `VITE_MAPBOX_TOKEN` (secret build, public). Coords `lat`/`lng` de la row si présentes (les 35 028 `market_listings` les ont toutes), sinon **géocodage de l'adresse à la volée** (les biens internes `properties` ont `lat`/`lng` vides), placeholder honnête en dernier recours (pas de token / adresse non résoluble). Vérifié : URL Static correcte clair+sombre, fallback `onError` gracieux, build/lint verts.
