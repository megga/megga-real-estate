...
#### Messagerie CRM — lot 1, socle backend (4 septembre 2026 — PR #1274, PAS en production)
> Sert les objectifs 1 (temps administratif) et 5 (remplacer un outil fragmenté) : l'agent quitte le CRM pour répondre à un client, et le fil e-mail ne rejoint jamais la fiche contact. ⚠ **Cette entrée décrit une BRANCHE, pas un serveur.** `claude/crm-messaging-lot1-backend-79297b`, PR #1274 ouverte, 6/6 checks verts ; mesuré en production le 04.09.2026 : **0 table `mail_%`, 0 fonction `mail_%`, 0 job cron `mail%`**. Aucune interface (lot 2), aucun IMAP (lot 3).
...
- **À faire le jour du merge, et qu'aucune porte ne réclamera** : `CLAUDE.md` §7 passe à **52 jobs pg_cron** (l'écart de 2 % reste dans la tolérance de 20 %, donc `lint:claude-md` restera vert sur une prose fausse) ; §8 « Intégrations » gagne Gmail API et Microsoft Graph ; `docs/system-map.md` ouvre une section « Messagerie » et corrige sa puce « Messaging » ; `docs/schema.md` retire `messages`/`message_threads` et ajoute les tables `mail_*`.
