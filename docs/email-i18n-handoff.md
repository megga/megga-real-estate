...
**getmegga.com**, que seul un chemin du dépôt y dépose : la connexion **par mot de passe sur la
...
`true`), pas OAuth (les jetons naissent sur `app.getmegga.com`) — et **pas le cas décrit ici**,
l'agent connecté sur l'app qui revient sur la vitrine. Les quatre ponts possibles ont été
cherchés (cookie de domaine parent, jeton d'URL, refresh token partagé, `.getmegga.com`) :
...
l'app (la seule mention d'`app.getmegga.com` dans son HTML est de la prose, dans `privacy.html`),
...
`app.getmegga.com` directement depuis un signet n'apporte aucune langue. Il basculera dans le
