...
| 9 | `agency_mrr()` (essai/impayé/gratuit = 0) + les 12 vues `v_admin_*` — p95 < 300 ms sur seed 100 agences / 1M événements | §4 §10.4 | **L** | 5-8 |
| 10 | `admin_overview()` — un objet, un fetch | §5.1 | S | 9 |
...
| # | Étape | § | Effort | Dépend de |
|---|---|---|---|---|
| 16 | Socle gestes : `rpc_receipts` (idempotence), `outbox_jobs` + worker (retries expo, dead-letter → Monitoring), enveloppe d'erreur §10.1, verrous advisory | §10.1-10.2 | **L** | G1 |
...
| 19 | KYC : `lookup` + `regenerate` — **appliquer `HANDOFF_KYC_DIAGNOSTIC.md` tel quel** (motif obligatoire serveur, plafond 3, rate limit 10/h/acteur) | §5.5 | M | 16 |
...
| 20 | Copilote : `v_ai_month` sur **`ai_usage_logs` (existante)** + dérives (mois courant seul) + **brancher `admin_set_agency_quotas` / `get_admin_quota_breaches` (existantes)** + `ai_drift_dismissals` | §5.11 | M | 5 |
