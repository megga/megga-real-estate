...
| 19b | **Décisions KYB** : brancher les 5 RPC existantes (valider · rejeter · correction · relancer · pièce d'identité) + écriture `admin_log` famille `kyb` — **zéro nouvelle RPC**, invariants du relais Thomas respectés. ⚠ L'étape 23 a **mesuré** que ces cinq n'écrivent aujourd'hui que dans `activity_events`, donc hors chaîne d'empreintes et invisibles à l'écran Sécurité : c'est exactement ce que cette ligne promet de réparer. Elles sont nommées dans le cliquet de `admin-gestes-sweep.spec.ts` — **en les branchant, retirer leurs cinq lignes de la liste `DETTE`, sinon le test rougit**. | §5.13 | S | 14b, 16 |
...
| 21 ✅ | **FAIT** — table ÉTENDUE (status draft/scheduled/published, `scheduled_for`, `published_at`), cron `changelog-publish` (*/10), 5 RPC journalisées `comm`. ⚠ Deux défauts de l'existant corrigés : `published` avait **`DEFAULT TRUE`** (une entrée insérée sans y penser partait à tous les agents) et les policies testaient **`profiles.role` en dur** au lieu d'`is_super_admin()`, ratant la moitié allowlist du mur réel. `status`/`published` ne peuvent plus diverger — **contrainte**, pas convention. ⚠ Amendement : 5ᵉ RPC `_delete`, hors liste §5.10, parce que `useChangelog.ts` exposait déjà un DELETE direct que la fermeture de la table cassait ; le hook est rebranché sur les gestes. | §5.10 | S | 16 |
| 22 ✅ | **FAIT** — `admin_log_export` (extrait signé du REGISTRE console, auto-journalisé famille `export`, empreinte SHA-256, verdict de chaîne sur la fenêtre). Distinct d'`audit-pdf-export`, qui exporte `activity_events` : celle-là raconte ce que font les AGENCES, celle-ci ce que fait MEGGA. DSAR déjà en place (`admin-dsar-export`), non refait. ⚠ **La décision « aucun CSV » n'était PAS appliquée** : la console en portait cinq via `src/lib/exportCsv.ts`. Retirés, helper supprimé, garde-fou statique posé et MUTÉ. ⚠ L'extraction précède la journalisation, sinon l'empreinte n'est pas reproductible — un test le prouve sans lire le code. | §5.4 §5.9 | S | 2 |
| 23 ◑ | **FAIT pour les gestes qui existent** (19-22 ; 17/18 restent bloqués). ⚠ **Amendement de méthode** : §10.7 dit « tests **par RPC** », j'ai livré un **balayage**. Raison mesurée : le défaut parti en prod le 01.08 (clé d'idempotence brûlée par un refus métier) avait traversé des specs qui testaient pourtant l'idempotence — elles éprouvaient toutes le chemin HEUREUX. Une liste écrite à la main couvre les neuf gestes d'aujourd'hui et laisse le dixième sans filet ; le balayage DÉCOUVRE les gestes (« toute fonction qui écrit au registre ») et impose le contrat à celui qu'on ajoutera. Six invariants : clé jamais brûlée par un refus · tout reçu réservé est scellé · tout geste gardé · aucune RPC admin qui écrit sans journaliser · enveloppe §10.1 (une seule exception, mesurée) · les exceptions restent rares. Détecteur **muté** avant livraison (rouge sur le motif fautif, vert sur les deux formes saines). ⚠ **Le balayage a ÉCHOUÉ à sa première exécution, et sa trouvaille est réelle** : **huit** RPC super-admin écrivent sans laisser de ligne au registre MEGGA. Elles journalisent dans `activity_events` — l'autre journal, celui des AGENCES, qui n'a **pas de chaîne d'empreintes** et que l'écran Sécurité ne lit pas. Ce sont pourtant des actions de MEGGA *sur* une agence. Les huit sont déjà au plan (19b pour les cinq KYB, 18 pour `set_role`, famille 17 pour plan/quotas) et bloquées par des décisions PO : l'étape 23 ne les corrige pas, mais ne se tait pas non plus. La dette est **nommée dans un cliquet** qui rougit dans les deux sens — un neuvième geste clandestin le fait échouer, un remboursement laissé dans la liste aussi. La liste ne peut que rétrécir. ⚠ Ce qui n'est **délibérément pas** ici : le « refus JWT agent » comportemental, déjà couvert par `admin-rpc-guard-sweep.spec.ts` qui appelle chaque RPC sous un rôle sans JWT — le dupliquer en statique aurait été moins bon. | §10.7 | M | 17-22 |
