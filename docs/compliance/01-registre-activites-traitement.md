...
| **Durée de conservation** | ⚠ **À DÉTERMINER — aucune durée n'est définie ni appliquée.** L'image déposée par la voie de secours réside sous `documents/{agence}/kyb-identity/` sans ligne en base, sans propriétaire, sans rétention et sans chemin de purge ; elle échappe à `delete-account` et aux deux déclencheurs de rétention 10 ans, qui ne s'arment que sur `kyc_case_id IS NOT NULL`. Dette écrite dans `20260802200000_kyb_identity_super_admin_select.sql`. |
...
| **Droits des personnes concernées** | Accès et rectification via le dirigeant de l'agence. ⚠ **Droit d'effacement non opérant sur l'image de la pièce** : aucun chemin de purge n'existe (voir « Durée de conservation ») |
...
| 2 | **L'image de pièce d'identité KYB n'a ni rétention ni purge** | Le droit d'effacement (art. 32 nLPD) est inopérant sur cette donnée ; la limitation de conservation n'est pas respectée | Direction (durée) puis technique (mise en œuvre) |
