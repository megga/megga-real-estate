...
| **Sous-traitants** | Supabase — base, Vault et Realtime en eu-west-1 (Irlande) ; **Edge Functions sans région épinglée** (réseau d'exécution de Supabase — voir point ouvert n°5) · **Google** (Gmail API ; scopes `gmail.modify openid email`) — États-Unis · **Microsoft** (Graph ; scopes `offline_access User.Read Mail.ReadWrite Mail.Send`) — États-Unis |
