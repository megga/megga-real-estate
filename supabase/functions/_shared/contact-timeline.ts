...
interface Ligne { action: string | null; object_label: string | null; created_at: string | null; sent_at: string | null }
...
export async function lireFaitsContact(supabase: SupabaseClient, agencyId: string | null, contactId: string, limite: number): Promise<FaitContact[]> {
  ...
  if (!agencyId) return []
  const base = () => supabase.from('activity_events')
    .select('action, object_label, created_at, sent_at:metadata->>sent_at')
    .eq('entity_type', 'contact').eq('entity_id', contactId).eq('agency_id', agencyId)
  const [autres, courriers] = await Promise.all([
    base().not('action', 'in', `(${ACTIONS_COURRIER.join(',')})`).order('created_at', { ascending: false }).limit(limite),
    base().in('action', [...ACTIONS_COURRIER]).order('metadata->>sent_at', { ascending: false, nullsFirst: false }).limit(limite),
  ])
  ...
  const lignes = [...((autres.data ?? []) as Ligne[]), ...((courriers.data ?? []) as Ligne[])]
  return lignes
