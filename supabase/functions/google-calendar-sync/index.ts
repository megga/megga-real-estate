...
import { compteLie, statutDuRefus, verifierLiaisonGoogle } from '../_shared/calendar-token-provenance.ts'
  ...
  try {
    ...
    switch (body.action) {
      ...
      case 'save_tokens': {
        ...
        // neuf doit appartenir au compte Google LIÉ à cet utilisateur. Sans ce
        ...
        const verdict = await verifierLiaisonGoogle(body.refresh_token, {
          clientId: GOOGLE_CLIENT_ID,
          clientSecret: GOOGLE_CLIENT_SECRET,
          compte: compteLie(lu?.user?.identities ?? null, 'google'),
        })
        if (!verdict.ok) {
        ...
        if (auditError) console.error('[google-calendar-sync] calendar_connected non journalisé :', auditError.message)
