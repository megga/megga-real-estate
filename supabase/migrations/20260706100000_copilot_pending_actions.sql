...
--
-- Une ligne max par agent (UNIQUE user_id) : une seule action en attente à la fois,
-- comme sur WhatsApp. Écrite/lue UNIQUEMENT par le service-role (ai-copilot) ; RLS
-- activée SANS policy => aucun accès anon/authenticated direct. Le scoping par agent
-- est appliqué DANS l'edge (.eq('user_id', <auth.uid dérivé du JWT>)), jamais depuis
-- un paramètre client forgeable.
