-- Ferme le trou de la GARDE d'envoi pour l'avis LPD : `whatsapp_send_allowed` acceptait
-- `lpd_notice` vers un numéro d'agent vérifié.
--
-- Son étape 5 rendait `ok` à `lpd_notice` avant de regarder qui est le destinataire. Les
-- deux migrations précédentes (20260910192017, 20260910200728) ont fermé la SOURCE — la
-- fonction whatsapp_pending_notices ne rend plus de numéro d'agent —, mais la garde, qui est
-- la seconde ligne, laissait passer. Désormais elle refuse `notice_not_for_agent`.
--
-- ⚠ LE TROU N'ÉTAIT PAS SEULEMENT CELUI DES PROFILS. Mesuré en production le 10.09.2026 :
-- le seul agent vérifié a AUSSI une fiche contact. Appelée par whatsapp-process (sans
-- contact ni profil), la garde dérive alors le sujet 'contact' à l'étape 2, et c'est par ce
-- chemin qu'est passé l'avis du 10.09. Un correctif limité à `v_kind = 'profile'` ne l'aurait
-- pas arrêté : le refus porte donc sur le NUMÉRO — même prédicat que la clause du lien de
-- whatsapp_pending_notices —, ce qui couvre les deux dérivations.
--
-- Motif exposable : le précis pour le service, `not_contactable` pour un appelant
-- authentifié — la règle que suit déjà `no_opt_in` à l'étape 7. Sans elle, un agent pourrait
-- demander un `lpd_notice` pour une de ses fiches et apprendre qu'un numéro est celui d'un
-- agent MEGGA vérifié.
--
-- Corps repris de 20260817134000, dont le texte est IDENTIQUE à la définition vivante
-- (md5(prosrc) = f1cf9bd77165b05b29906a20d930ca55, 15 079 octets, mesuré le 10.09.2026) ;
-- seule l'étape 5 change. `create or replace` : signature, défauts, type de retour,
-- propriétaire et droits (EXECUTE à authenticated et service_role) sont conservés.
begin;
...
begin
  ...
  if p_purpose = 'lpd_notice' then
    -- ⛔ L'avis est écrit pour un PROSPECT : un numéro d'agent vérifié n'en reçoit jamais,
    -- c'est un utilisateur du service, sous base contractuelle. Testé sur le NUMÉRO et non
    -- sur `v_kind = 'profile'` : un agent qui a AUSSI une fiche est dérivé 'contact' à
    -- l'étape 2 — c'était le cas du seul agent vérifié, mesuré le 10.09.2026.
    if exists (select 1 from public.whatsapp_agent_links l
               where l.verified and public.normalize_phone(l.wa_number) = v_norm) then
      return query select false,'notice_not_for_agent',
        case when auth.uid() is null then 'notice_not_for_agent' else 'not_contactable' end,
