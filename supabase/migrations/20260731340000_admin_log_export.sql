...
begin
  ...
  -- laisser deux. Sans cela, le registre porterait deux exports là où l'agent n'en a
  -- demandé qu'un, et l'audit compterait faux.
  ...
  select count(*)::integer, min(l.seq),
  ...
  v_chaine := public.admin_log_verify_chain(v_seq_min);
...
comment on function public.admin_log_export(timestamptz, timestamptz, text, text) is
  ...
  'exportée. Le rendu PDF appartient à une edge function — le SQL ne fabrique pas de PDF, '
