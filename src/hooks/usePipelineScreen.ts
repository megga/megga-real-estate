...
export interface UsePipelineScreenReturn {
  deals: CrmDeal[]
  isLoading: boolean
 ...
 */
export function usePipelineScreen(): UsePipelineScreenReturn {
  ...
  const { data: rawContacts = [], isLoading: contactsLoading, isError: contactsError, refetch: refetchContacts } = useQuery({
    queryKey: ['pipeline-contacts', agencyId, contactIds],
  ...
  const { data: kycCases = [], isLoading: kycLoading, isError: kycError, refetch: refetchKyc } = useQuery({
    queryKey: ['pipeline-kyc', agencyId, contactIds],
  ...
  const { data: rawProperties = [], isLoading: propsLoading, isError: propsError, refetch: refetchProps } = useQuery({
    queryKey: ['pipeline-properties', agencyId, propertyIds],
  ...
  // un effet post-commit). Les composants enfants (StageColumn, DealCard,
