...
import { contactToCrm } from '@/lib/crmAdapters'
import {
  registerLiveContact,
  resetLiveOverrides,
  type CrmContact,
...
export interface UseContactsScreenReturn {
  contacts: CrmContact[]
  isLoading: boolean
  isError: boolean
  refetch: () => void
...
export function useContactsScreen(): UseContactsScreenReturn {
  ...
  const {
    ...
    queryKey: ['contacts-screen', agencyId],
  ...
  const { data: kycCases = [] } = useQuery({
    queryKey: ['contacts-kyc', agencyId, contactIds],
