// MEGGA Marketplace — Property X top navigation.
// Refonte fidèle aux tokens Property X (neutrals + Objectivity).
// Logo + liens centraux + CTA. Glass overlay sur fond photo.

import { Link } from 'react-router-dom'
import { PX } from '../tokens'
import PxButton from '../PxButton'

interface PxNavProps {
  glass?: boolean // si true, fond transparent + texte blanc (pour overlay sur hero)
}

const NAV_LINKS = [
  { label: 'Acheter', to: '/acheter' },
  { label: 'Louer', to: '/louer' },
  { label: 'Vendre', to: '/vendre' },
  { label: 'Estimations', to: '/estimations' },
]

export default function PxNav({ glass = false }: PxNavProps) {
  const textColor = glass ? PX.neutral100 : PX.neutral700

  return (
    <header style={{
      position: 'absolute',
      top: 0, left: 0, right: 0,
      zIndex: 10,
      padding: '24px 40px',
    }}>
      <div style={{
        maxWidth: PX.containerDesktop,
        margin: '0 auto',
        display: 'flex',
        alignItems: 'center',
        justifyContent: 'space-between',
        gap: 24,
      }}>
        {/* Logo MEGGA */}
        <Link to="/" style={{ display: 'flex', alignItems: 'center', textDecoration: 'none' }}>
          <svg viewBox="0 0 694.81 419.02" width="44" height="26" aria-label="MEGGA">
            <path fill={textColor} d="M212.94,0c46.64,5.38,88.55,22.94,122.21,59.67-22.79,28.12-37.71,60.3-47.08,96.28-7.89-14.68-16.56-27.02-28.35-37.25-40.39-35.04-99.55-30.53-134.81,9.66-40.25,45.89-40.1,117.16.48,162.82,35.48,39.93,94.73,44.05,134.83,8.67,14.5-12.89,25.12-28.95,32.24-48.42l-95.26-.1-.03-83.65,192.78-.02c8.8,28.23,5.09,73.7-2.86,101.4-22.71,79.15-85.98,140.1-169.06,149-2.17.23-4.11.34-5.1.93h-31c-42.03-4.33-81.34-20.79-113.04-49.92C-27.8,280.23-21.84,119.65,81.31,39.5,110.93,16.49,145.39,3.92,181.93,0h31.01Z"/>
            <path fill={textColor} d="M511.94,419.01h-29c-47.56,0-91.35-24.53-123.87-60,24.65-30.5,36.53-57.89,47.2-96.18,7.43,14.3,16.5,27.51,28.71,37.93,36.96,31.55,90.34,30.86,126.22-1.89,13.97-12.75,24.27-28.48,31.18-47.43l-94.84-.08-.05-83.65,192.4-.03c2.59,9.14,3.94,17.82,4.5,27.2,4.34,72.2-25.1,142.48-83.13,186.34-29.43,22.24-63.45,34.03-99.32,37.8h0Z"/>
            <path fill={textColor} d="M511.94,0c43.2,4.34,82.78,21.02,114.61,50.52,6.43,5.96,12.05,11.43,17.39,19.2l-56.72,84.95c-7.57-14.34-16.16-25.96-27.71-36.03-33.9-29.56-83.44-31.58-119.35-4.39-12.97,9.71-22.64,21.92-30.74,35.77l-101.14-.14c10.87-40.77,32.85-75.32,63.12-102.25C402.99,19.45,441.75,4.18,482.95.02h29-.01Z"/>
          </svg>
        </Link>

        {/* Liens centraux */}
        <nav style={{
          display: 'flex',
          alignItems: 'center',
          gap: 4,
        }}>
          {NAV_LINKS.map(link => (
            <Link key={link.to} to={link.to} style={{
              padding: '10px 18px',
              fontSize: 16,
              fontWeight: 400,
              lineHeight: 1.25,
              letterSpacing: '-0.48px',
              fontFamily: PX.font.sans,
              color: textColor,
              textDecoration: 'none',
              transition: `opacity ${PX.duration.fast} ${PX.ease}`,
            }}
              onMouseEnter={e => { e.currentTarget.style.opacity = '0.7' }}
              onMouseLeave={e => { e.currentTarget.style.opacity = '1' }}>
              {link.label}
            </Link>
          ))}
        </nav>

        {/* Actions droite : Connexion + CTA */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
          <Link to="/login" style={{
            fontSize: 16,
            fontWeight: 400,
            lineHeight: 1.25,
            letterSpacing: '-0.48px',
            fontFamily: PX.font.sans,
            color: textColor,
            textDecoration: 'none',
          }}>Connexion</Link>
          <PxButton to="/acheter" variant={glass ? 'invert' : 'primary'} size="sm">
            Commencer
          </PxButton>
        </div>
      </div>
    </header>
  )
}
