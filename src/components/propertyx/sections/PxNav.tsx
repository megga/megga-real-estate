...
// Logo + liens centraux + CTA droite. Glass effect quand sur fond photo.
...
interface PxNavProps {
  glass?: boolean // si true, fond transparent + texte blanc (pour overlay sur hero)
}
...
export default function PxNav({ glass = false }: PxNavProps) {
  const textColor = glass ? PX.inkInverse : PX.ink
  const bg = glass ? 'transparent' : PX.surfaceBg
  const borderColor = glass ? 'rgba(255,255,255,0.18)' : PX.border
  ...
  return (
    <header style={{
      ...
      zIndex: 10,
      padding: `20px ${PX.space.pageX}px`,
      background: bg,
      borderBottom: glass ? 'none' : `1px solid ${borderColor}`,
    }}>
      <div style={{
        maxWidth: 1440,
        margin: '0 auto',
        ...
        <nav style={{
          ...
          {NAV_LINKS.map(link => (
            <Link key={link.to} to={link.to} style={{
              ...
              onMouseEnter={e => { e.currentTarget.style.background = glass ? 'rgba(255,255,255,0.10)' : 'rgba(10,10,10,0.04)' }}
