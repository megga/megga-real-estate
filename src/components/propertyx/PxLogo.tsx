...
export type PxLogoVariant = 'dark' | 'light'
export type PxLogoForm = 'icon' | 'text'
export type PxLogoSize = 'sm' | 'md' | 'lg'

interface PxLogoProps {
  variant?: PxLogoVariant
  form?: PxLogoForm
  size?: PxLogoSize
  to?: string         // si défini, wrap dans un Link
  className?: string
}
...
export default function PxLogo({
  variant = 'dark',
  form = 'text',
  size = 'sm',
  to,
  ...
  if (to) {
    return (
      <Link to={to} className={className} style={{ display: 'inline-flex', textDecoration: 'none' }}>
        {content}
      </Link>
    )
  }
