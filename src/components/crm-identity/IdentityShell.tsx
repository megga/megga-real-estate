...
import { IDENTITY_STEPS } from './tokens'
...
/** Borne `step` à [0, stepCount - 1] — jamais un index hors de IDENTITY_STEPS. */
// eslint-disable-next-line react-refresh/only-export-components -- fonction pure testée directement (tests/unit/identity-shell-navigation.spec.ts), même motif que useIdentityGate.ts.
 ...
 * commentaire de IDENTITY_STEPS, tokens.ts).
 */
 ...
 * telles quelles, seul le style passe de WizardTokens — retiré — à `.mx-langpicker`,
 ...
 * n'est pas une étape du parcours (IDENTITY_STEPS), seulement un état de la
 ...
 * `IDENTITY_STEPS` en est la source : un jour où l'ordre bouge, c'est ici que ça se
...
export default function IdentityShell({ preview }: { preview?: IdentityShellPreview } = {}) {
  ...
  const quitterRetour = (target: number) => {
    ...
    setStep(clampIdentityStep(target, IDENTITY_STEPS.length))
  }
  ...
  const next = async () => {
    ...
    setStep((s) => clampIdentityStep(s + 1, IDENTITY_STEPS.length))
  }
  const prev = async () => {
    if (saving) return
    await persistCurrentStep()
    ...
    <MeggaX>
      ...
      <div className="page-wrapper full-height-page mx-appshell">
        ...
        <section className="section pd-top-0 pd-bottom-0 mx-grow mx-shellbody">
          <div className="container-default w-container">
            {/* Numérotation directe depuis IDENTITY_STEPS : le rail n'a plus de
