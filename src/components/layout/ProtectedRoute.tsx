 ...
 * getmegga.com/login) → consentements nLPD (`ConsentGate`), puis rend le contenu
...
export default function ProtectedRoute({ children }: ProtectedRouteProps) {
  ...
  // 2. Non authentifié → connexion sur la VITRINE (getmegga.com/login, câblé
  ...
  if (!user) {
    if (typeof window !== 'undefined') window.location.replace('https://getmegga.com/login')
    return <BootSplash />
  }
