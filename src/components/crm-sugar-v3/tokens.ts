 ...
 *
 * `SugarV3` était un `as const` statique. Ses lecteurs — Visites, Audit, Import
 ...
 * le dépôt emploie partout ailleurs (`mxCrmPalette`, `crmSugarPalette`,
 * `ndPalette`).
 *
 ...
 */
export function sugarV3Palette(dark: boolean) {
  ...
  shadow: dark ? 'none' : `0 12px 40px ${sgVoileEncre(false, 0.06)}, 0 2px 8px ${sgVoileEncre(false, 0.03)}`,
  shadowLg: dark ? 'none' : `0 24px 60px ${sgVoileEncre(false, 0.08)}, 0 4px 16px ${sgVoileEncre(false, 0.04)}`,
  shadowHover: dark ? 'none' : `0 32px 70px ${sgVoileEncre(false, 0.10)}, 0 6px 20px ${sgVoileEncre(false, 0.05)}`,
...
export type SugarV3Palette = ReturnType<typeof sugarV3Palette>

/**
 * La police, INDÉPENDANTE du thème — et c'est pour ça qu'elle sort de la palette.
 *
 * Trois surfaces ne lisaient de `SugarV3` que ce champ. Les forcer à construire
 * une palette pour l'atteindre leur ferait choisir un thème dont elles n'ont pas
 * besoin, et un `sugarV3Palette(false)` posé là ressemblerait à une surface
 ...
 * typographie.
 */
export const SUGAR_V3_FONT = 'var(--crm-font, "Inter Tight"), system-ui, sans-serif'
 ...
 * `sugarV3Palette(useSugarDark())`. S'en servir là serait recréer exactement le
 * défaut que cette fonction corrige, et la garde `sugar-v3-contraste.spec.ts`
 * le refuse nommément.
 */
const SV3_CLAIR = sugarV3Palette(false)

/**
 * Les deux tons THÉMÉS dont les maps de libellés ont besoin, lus au RENDU.
 *
 * `readSugarDark()` est la lecture partagée (`lib/sugarDark.ts`) — celle qui
 ...
 */
const SV3_TON = {
  get muted() { return sugarV3Palette(readSugarDark()).muted },
  get ink() { return sugarV3Palette(readSugarDark()).ink },
}
