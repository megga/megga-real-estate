...
}: { name: CrmIconName; size?: number; stroke?: string; sw?: number }) {
  return (
...
export function CrmCircleBtn({
  ...
  return (
    ...
    onMouseEnter={e => { e.currentTarget.style.background = WizardTokens.card; e.currentTarget.style.boxShadow = WizardTokens.shadow }}
    onMouseLeave={e => { e.currentTarget.style.background = WizardTokens.cardSubtle; e.currentTarget.style.boxShadow = WizardTokens.shadowSm }}
    >
      {icon}
      {badge && <span style={{
        position: 'absolute', top: 8, right: 8,
        width: 8, height: 8, borderRadius: 'var(--crm-radius-pill)', background: WizardTokens.err,
        boxShadow: `0 0 0 2px ${WizardTokens.cardSubtle}`,
      }} />}
    </button>
  )
}
  ...
  return (
    <button onClick={onClick} disabled={disabled}
      ...
      style={{
        ...
        background: disabled ? WizardTokens.ghostSolid : (hover ? WizardTokens.blackHover : WizardTokens.black),
        color: WizardTokens.onBlack,
        fontFamily: 'inherit', fontWeight: 600,
        fontSize: size === 'lg' ? 'var(--crm-text-lg)' : 'var(--crm-text-md)',
        letterSpacing: 0.1,
        cursor: disabled ? 'not-allowed' : 'pointer',
        display: 'inline-flex', alignItems: 'center', gap: 'var(--crm-space-lg)',
        boxShadow: disabled ? 'none' : (hover ? WizardTokens.pillShadowHover : WizardTokens.pillShadow),
  ...
  return (
    <button onClick={onClick}
      ...
      style={{
        ...
        border: 0, background: hover ? WizardTokens.card : 'transparent',
        color: WizardTokens.inkSoft, fontFamily: 'inherit',
        fontSize: 'var(--crm-text-xl)', fontWeight: 600, cursor: 'pointer',
        display: 'inline-flex', alignItems: 'center', gap: 'var(--crm-space-md)',
        boxShadow: hover ? WizardTokens.shadow : 'none',
        transition: 'all .18s ease',
      }}>
      {icon}
      {children}
...
export function CrmInput({
  ...
  return (
    <label style={{ display: 'block' }}>
      ...
      <div style={{
        fontSize: 'var(--crm-text-2xl)', fontWeight: 400, color: WizardTokens.inkSoft,
        marginBottom: 8,
      }}>{label}</div>
      <input type={type} value={value} autoFocus={autoFocus}
        ...
        style={{
          ...
          background: focus ? WizardTokens.card : WizardTokens.cardSubtle,
          color: WizardTokens.ink, fontSize: 'var(--crm-text-xl)', fontWeight: 500,
          boxShadow: focus
            ? `0 0 0 2px ${WizardTokens.black}, 0 4px 12px rgba(0,0,0,0.05)`
            : `inset 0 0 0 1px ${WizardTokens.line}`,
          transition: 'all .18s ease',
        }} />
    </label>
  )
...
export function CrmKycChip({ status }: { status?: 'verified' | 'pending' | 'none' | 'stale' | string }) {
  const { t } = useTranslation('listings')
  // Couleurs thème par statut (non traduites) ; le libellé vient de l'i18n.
  const tone: Record<string, { color: string; bg: string }> = {
    verified: { color: WizardTokens.ok,    bg: 'rgba(16,185,129,0.10)' },
    pending:  { color: WizardTokens.warn,  bg: 'rgba(245,158,11,0.10)' },
    none:     { color: WizardTokens.muted, bg: 'rgba(122,128,136,0.10)' },
    stale:    { color: WizardTokens.warn,  bg: 'rgba(245,158,11,0.10)' },
  }
