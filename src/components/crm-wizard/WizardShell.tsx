...
import {
  WizardTokens, setWizardDark, EMPTY_WIZARD, WIZARD_STEPS, CRM_ANIM_KEYFRAMES, type WizardData,
} from './tokens'
import {
  CrmIcon, CrmCircleBtn, CrmBlackPill, CrmGhostPill,
} from './primitives'
 ...
 * bloqué sur une garde. Dérivé de `WIZARD_STEPS`, un ajout ou un retrait d'étape se
 * propage tout seul.
 */
const ETAPE = Object.fromEntries(
  WIZARD_STEPS.map((e, i) => [e.id, i]),
) as Record<(typeof WIZARD_STEPS)[number]['id'], number>
 ...
 * sept), et rien ne les remplaçait : `WIZARD_STEPS` ne sert qu'aux bornes de
 ...
 * le sur-titre disait.
 *
 * ⚠ Le dénominateur est `WIZARD_STEPS.length` — SIX depuis le retrait de l'étape
...
function ProgressionWizard({ etape, total }: { etape: number; total: number }) {
  ...
  const libelle = t('wizard.progress', {
    n: etape + 1,
    total,
    label: WIZARD_STEPS[etape]?.label ?? '',
  })
  return (
    <div
      role="progressbar"
      ...
      {Array.from({ length: total }).map((_, i) => (
        <span
          ...
          style={{
            ...
            // règle Sugar Pure qui le rendait noir, comme `CrmPalette` a
            // survécu à Sugar. `line` est le filet structurel — un segment non
            // atteint est un TRAIT, pas une surface.
            background: i <= etape ? WizardTokens.black : WizardTokens.line,
          }}
        />
      ))}
    </div>
...
export default function WizardShell({ onClose, embedded = false, dark: darkOverride }: WizardShellProps) {
  ...
  // (source de vérité lue par le Proxy WizardTokens, robuste au rendu concurrent),
    ...
    <div style={{
      ...
      <header style={{
        ...
        background: (!published && step === ETAPE.address)
          ? 'transparent'
          : `linear-gradient(180deg, ${WizardTokens.bg} 58%, ${WizardTokens.bg}00 100%)`,
      }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--crm-space-4xl)' }}>
          <div style={{ flex: 1 }} />
          <CrmCircleBtn
            icon={<CrmIcon name="close" size={18} stroke={WizardTokens.ink} />}
            onClick={onClose}
            title={t('common:actions.close')}
          />
        </div>
        {!published && (
          <ProgressionWizard etape={step} total={WIZARD_STEPS.length} />
        )}
      </header>
      ...
      {!published && <footer style={{
        ...
        background: dark
          ? `linear-gradient(180deg, transparent 0%, ${WizardTokens.bg} 72%)`
          : WizardTokens.footerFade,
        pointerEvents: 'none',
      }}>
        <div style={{ pointerEvents: 'auto', display: 'flex', alignItems: 'center', gap: 'var(--crm-space-2xl)' }}>
          {step > 0 && (
            <CrmGhostPill onClick={prev}
              icon={<CrmIcon name="arrowL" size={16} stroke={WizardTokens.inkSoft} />}>
              ...
              <div style={{
                ...
                {etatBrouillon === 'enregistre' ? (
                  <span style={{
                    ...
                    <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke={WizardTokens.isDark ? '#0B0B0C' : '#fff'} strokeWidth="3.4" strokeLinecap="round" strokeLinejoin="round"><path d="M20 6 9 17l-5-5" /></svg>
                  </span>
                ) : (
        ...
        <div style={{ pointerEvents: 'auto', display: 'flex', alignItems: 'center', gap: 'var(--crm-space-3xl)' }}>
          ...
          {step < WIZARD_STEPS.length - 1 && (
            <CrmBlackPill onClick={next} disabled={!canNext}>
