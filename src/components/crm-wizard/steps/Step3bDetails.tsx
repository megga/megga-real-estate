...
import { WizardTokens, crmOn, type WizardData, type WizardDetails } from '../tokens'
...
function Sp4bInput({
  ...
  return (
    <label style={{ display: 'flex', flexDirection: 'column', gap: 'var(--crm-space-sm)', minWidth: 0 }}>
      <span style={{ fontSize: 'var(--crm-text-md)', fontWeight: 600, color: WizardTokens.muted }}>{label}</span>
      <span style={{
        ...
        boxShadow: foc ? `inset 0 0 0 2px ${WizardTokens.black}` : 'none', transition: 'box-shadow .12s',
      }}>
        <input className="sp4b-num" type="number" value={value ?? ''} placeholder={placeholder}
          ...
          style={{
            ...
            fontSize: 'var(--crm-text-2xl)', fontWeight: 600, color: WizardTokens.ink, fontVariantNumeric: 'tabular-nums',
          }} />
        {unit && <span style={{ fontSize: 'var(--crm-text-lg)', fontWeight: 600, color: WizardTokens.muted, flexShrink: 0 }}>{unit}</span>}
      </span>
    </label>
  )
}
...
function Sp4bDateField({
  ...
  const navBtn = (d: number, path: string) => (
    <button onClick={() => nav(d)} style={{
      width: 32, height: 32, borderRadius: 'var(--crm-radius-pill)', border: 0, cursor: 'pointer', background: WizardTokens.card,
      boxShadow: WizardTokens.shadowSm, color: WizardTokens.ink, display: 'grid', placeItems: 'center', fontFamily: 'inherit',
    }}>
  ...
  return (
    <div ref={ref} style={{ position: 'relative', display: 'flex', flexDirection: 'column', gap: 'var(--crm-space-sm)', minWidth: 0 }}>
      ...
      <button onClick={() => setOpen(o => !o)} style={{
        display: 'flex', alignItems: 'center', gap: 'var(--crm-space-lg)', height: 46, padding: '0 var(--crm-space-3xl)', borderRadius: 'var(--crm-radius-lg)',
        background: WizardTokens.cardSubtle, border: 0, cursor: 'pointer', fontFamily: 'inherit', textAlign: 'left',
        boxShadow: open ? `inset 0 0 0 2px ${WizardTokens.black}` : 'none', transition: 'box-shadow .12s',
      }}>
        <span style={{ flex: 1, fontSize: 'var(--crm-text-2xl)', fontWeight: 600, color: value ? WizardTokens.ink : WizardTokens.muted, fontVariantNumeric: 'tabular-nums' }}>{display}</span>
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={WizardTokens.muted} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="5" width="18" height="16" rx="2" /><path d="M8 3v4M16 3v4M3 10h18" /></svg>
      </button>
      {open && (
        <div style={{ boxSizing: 'border-box', width: '100%', padding: 'var(--crm-space-3xl)', borderRadius: 'var(--crm-radius-xl)', background: WizardTokens.cardSubtle, animation: 'sgFadeUp .25s cubic-bezier(.2,.8,.2,1) both' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--crm-space-md)', marginBottom: 12 }}>
            {navBtn(-1, 'm15 18-6-6 6-6')}
            <span style={{ flex: 1, textAlign: 'center', fontSize: 'var(--crm-text-xl)', fontWeight: 600, color: WizardTokens.ink }}>{SP4B_MONTHS[view.m]} {view.y}</span>
            {navBtn(1, 'm9 18 6-6-6-6')}
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 'var(--crm-space-2xs)', marginBottom: 4 }}>
            {SP4B_WEEKDAYS.map((d, i) => <span key={i} style={{ textAlign: 'center', fontSize: 'var(--crm-text-xs)', fontWeight: 600, color: WizardTokens.muted }}>{d}</span>)}
          </div>
            ...
            {cells.map((d, i) => {
              ...
              return (
                <button key={i} disabled={past} onClick={() => { onChange(sp4bFmtDate(dt)); setOpen(false) }} style={{
                  ...
                  background: isSel ? WizardTokens.black : 'transparent', color: isSel ? ON : past ? WizardTokens.ghost : WizardTokens.ink,
                  boxShadow: !isSel && isToday ? `inset 0 0 0 1.5px ${WizardTokens.black}` : 'none',
                }}>{d}</button>
              )
            })}
          </div>
          <div style={{ display: 'flex', gap: 'var(--crm-space-md)', marginTop: 14 }}>
            <button onClick={() => { onChange(DISPO_TBD); setOpen(false) }} style={{
              ...
              fontSize: 'var(--crm-text-lg)', fontWeight: 600, background: isTbd ? WizardTokens.black : WizardTokens.card,
              boxShadow: isTbd ? 'none' : WizardTokens.shadowSm, color: isTbd ? ON : WizardTokens.inkSoft,
            ...
            {value && <button onClick={() => { onChange(null); setOpen(false) }} style={{
              ...
              fontSize: 'var(--crm-text-lg)', fontWeight: 600, background: 'transparent', color: WizardTokens.muted,
...
function Sp4bChip({ active, onClick, children }: { active?: boolean; onClick?: () => void; children: ReactNode }) {
  return (
    <button onClick={onClick} style={{
      ...
      fontSize: 'var(--crm-text-lg)', fontWeight: 600, background: active ? WizardTokens.black : WizardTokens.cardSubtle,
      color: active ? crmOn() : WizardTokens.inkSoft, display: 'inline-flex', alignItems: 'center', transition: 'background .12s',
    }}>
      {children}
    </button>
  )
...
function Sp4bCount({ label, value, onChange }: { label: string; value: number | null | undefined; onChange: (v: number | null) => void }) {
  ...
  const btn = (d: number, dis: boolean) => (
    <button disabled={dis} onClick={() => onChange(value == null ? (d > 0 ? 1 : null) : Math.max(0, value + d))} style={{
      width: 30, height: 30, borderRadius: 'var(--crm-radius-pill)', border: 0, cursor: dis ? 'default' : 'pointer', background: WizardTokens.card,
      boxShadow: WizardTokens.shadowSm, color: dis ? WizardTokens.ghost : WizardTokens.ink, fontSize: 'var(--crm-text-2xl)', fontWeight: 600,
      display: 'grid', placeItems: 'center', fontFamily: 'inherit',
    }}>{d > 0 ? '+' : '−'}</button>
  )
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--crm-space-md)', height: 48, padding: '0 var(--crm-space-md) 0 var(--crm-space-3xl)', borderRadius: 'var(--crm-radius-lg)', background: WizardTokens.cardSubtle }}>
      <span style={{ flex: 1, fontSize: 'var(--crm-text-lg)', fontWeight: 600, color: WizardTokens.inkSoft }}>{label}</span>
      {btn(-1, disMinus)}
      <span style={{ width: 24, textAlign: 'center', fontSize: 'var(--crm-text-xl)', fontWeight: 600, color: value == null ? WizardTokens.muted : WizardTokens.ink, fontVariantNumeric: 'tabular-nums' }}>{value == null ? '—' : value}</span>
      {btn(1, false)}
    </div>
  )
}
...
function Sp4bLbl({ children, first }: { children: ReactNode; first?: boolean }) {
  return <div style={{ fontSize: 'var(--crm-text-sm)', fontWeight: 600, color: WizardTokens.muted, margin: first ? '0 0 10px' : '22px 0 10px' }}>{children}</div>
}
...
function Sp4bRailBtn({ title, done, active, onClick }: { title: string; done: boolean; active: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} style={{
      ...
      background: active ? WizardTokens.black : 'transparent', color: active ? crmOn() : WizardTokens.inkSoft, transition: 'background .12s',
    }}>
...
export function Step3bDetails({ data, set }: StepProps) {
    ...
    <div>
      <div style={grid2}>
        <label style={{ display: 'flex', flexDirection: 'column', gap: 'var(--crm-space-sm)' }}>
          <span style={{ fontSize: 'var(--crm-text-md)', fontWeight: 600, color: WizardTokens.muted }}>{t('wizard.step3b.reference')}</span>
          <span style={{ display: 'flex', alignItems: 'center', gap: 'var(--crm-space-lg)', height: 46, padding: '0 var(--crm-space-3xl)', borderRadius: 'var(--crm-radius-lg)', background: WizardTokens.cardSubtle }}>
            <span style={{ flex: 1, fontSize: 'var(--crm-text-2xl)', fontWeight: 600, color: WizardTokens.ink, fontVariantNumeric: 'tabular-nums' }}>{det.ref}</span>
            <span style={{ fontSize: 'var(--crm-text-xs)', fontWeight: 600, color: WizardTokens.muted }}>{t('wizard.step3b.auto')}</span>
          </span>
        </label>
  ...
  return (
    <div style={{ width: '100%', boxSizing: 'border-box', margin: '0 auto', textAlign: 'left', display: 'grid', gridTemplateColumns: '230px 1fr', gap: 'var(--crm-space-4xl)', alignItems: 'start' }}>
      ...
      <nav style={{ display: 'flex', flexDirection: 'column', gap: 'var(--crm-space-xs)', padding: 'var(--crm-space-lg)', borderRadius: 'var(--crm-radius-5xl)', background: WizardTokens.card, boxShadow: WizardTokens.shadowSm }}>
        {sections.map(s => <Sp4bRailBtn key={s.id} title={s.title} done={s.done} active={cur.id === s.id} onClick={() => pick(s.id)} />)}
      </nav>
      <div style={{ borderRadius: 'var(--crm-radius-5xl)', background: WizardTokens.card, boxShadow: WizardTokens.shadow, height: 520, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ flexShrink: 0, padding: '22px 26px 0' }}>
          <span style={{ fontSize: 'var(--crm-text-2xl)', fontWeight: 600, color: WizardTokens.ink, letterSpacing: -0.2 }}>{cur.title}</span>
        </div>
