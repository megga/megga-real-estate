...
import { WizardTokens, crmOn, type WizardData, type WizardPhoto } from '../tokens'
  ...
  return (
    <div style={{ maxWidth: 900, margin: '0 auto', animation: 'sgFadeUp .5s cubic-bezier(.2,.8,.2,1) both' }}>
      ...
      <div style={{ marginBottom: 26, maxWidth: 680 }}>
        <h1 style={{ margin: '0 0 4px', fontSize: 'var(--crm-text-9xl)', fontWeight: 500, color: WizardTokens.ink, letterSpacing: -0.8, lineHeight: 1.1 }}>{t('wizard.step4.title')}</h1>
      </div>

      {photos.length > 0 && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 'var(--crm-space-xl)', marginBottom: 16, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 'var(--crm-text-lg)', fontWeight: 600, color: WizardTokens.ink }}>{t('wizard.step4.count', { count: photos.length })}</span>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--crm-space-sm)', padding: 'var(--crm-space-sm) var(--crm-space-xl)', borderRadius: 'var(--crm-radius-pill)', background: WizardTokens.card, boxShadow: WizardTokens.shadowSm, fontSize: 'var(--crm-text-md)', fontWeight: 600, color: WizardTokens.inkSoft }}>
      ...
      {cover ? (
        <div
          ...
          style={{ position: 'relative', aspectRatio: '16/9', borderRadius: 'var(--crm-radius-5xl)', overflow: 'hidden', background: WizardTokens.cardSubtle, boxShadow: over ? `inset 0 0 0 3px ${WizardTokens.black}, ${WizardTokens.shadow}` : WizardTokens.shadow, marginBottom: 16 }}>
          <img src={photoSrc(cover)} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
          <div style={{ position: 'absolute', top: 14, left: 14 }}>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--crm-space-sm)', padding: 'var(--crm-space-sm) var(--crm-space-xl)', borderRadius: 'var(--crm-radius-pill)', background: WizardTokens.black, color: crmOn(), fontSize: 'var(--crm-text-sm)', fontWeight: 600, letterSpacing: 0.2 }}>
              {IStar(crmOn(), 12)}{t('wizard.step4.cover')}
            </span>
          </div>
          <div style={{ position: 'absolute', top: 14, right: 14, display: 'flex', gap: 'var(--crm-space-md)' }}>
            <RoundBtn title={t('wizard.step4.crop')} onClick={() => setCropId(cover.id)}>{ICrop(WizardTokens.ink)}</RoundBtn>
        ...
        <div
          ...
          style={{ cursor: 'pointer', aspectRatio: '16/9', borderRadius: 'var(--crm-radius-5xl)', marginBottom: 16, background: over ? WizardTokens.cardSubtle : WizardTokens.card, boxShadow: over ? `inset 0 0 0 3px ${WizardTokens.black}` : `inset 0 0 0 1.5px ${WizardTokens.line}`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 'var(--crm-space-3xl)', transition: 'box-shadow .15s, background .15s' }}>
          <span style={{ width: 60, height: 60, borderRadius: 'var(--crm-radius-3xl)', background: WizardTokens.black, display: 'grid', placeItems: 'center', boxShadow: '0 10px 24px rgba(3,3,3,0.22)' }}>{IUp(crmOn(), 26)}</span>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: 'var(--crm-text-2xl)', fontWeight: 600, color: WizardTokens.ink }}>{t('wizard.step4.dropTitle')}</div>
            <div style={{ fontSize: 'var(--crm-text-lg)', fontWeight: 500, color: WizardTokens.muted, marginTop: 4 }}>{t('wizard.step4.dropHint')}</div>
          </div>
        </div>
      )}
          ...
          {photos.map((p, i) => {
            ...
            return (
              <div key={p.id} draggable
                ...
                style={{
                  position: 'relative', flex: '0 0 auto', width: 132, aspectRatio: '4/3', borderRadius: 'var(--crm-radius-xl)', overflow: 'hidden', background: WizardTokens.cardSubtle, cursor: 'grab',
                  boxShadow: isCover ? `inset 0 0 0 3px ${WizardTokens.black}` : WizardTokens.shadowSm,
                  outline: overIdx === i && drag !== i ? `2px dashed ${WizardTokens.black}` : 'none', outlineOffset: 2,
                  opacity: drag === i ? 0.5 : 1,
                }}>
                <img src={photoSrc(p)} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                {isCover && <div style={{ position: 'absolute', top: 6, left: 6, padding: 'var(--crm-space-2xs) var(--crm-space-md)', borderRadius: 'var(--crm-radius-pill)', background: WizardTokens.black, color: crmOn(), fontSize: 'var(--crm-text-xs)', fontWeight: 600, letterSpacing: 0.3 }}>{t('wizard.step4.coverShort')}</div>}
                <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'flex-end', justifyContent: 'flex-end', gap: 'var(--crm-space-xs)', padding: 'var(--crm-space-sm)', background: 'linear-gradient(180deg, transparent 55%, rgba(0,0,0,0.30))' }}>
                  {!isCover && <RoundBtn title={t('wizard.step4.setCover')} onClick={() => setCover(p.id)}>{IStar(WizardTokens.ink, 14)}</RoundBtn>}
                  <RoundBtn title={t('wizard.step4.crop')} onClick={() => setCropId(p.id)}>{ICrop(WizardTokens.ink, 14)}</RoundBtn>
                  <RoundBtn danger title={t('wizard.step4.delete')} onClick={() => removePhoto(p.id)}>{ITrash('#fff', 14)}</RoundBtn>
                </div>
              </div>
            )
          })}
          <button onClick={pick} style={{ flex: '0 0 auto', width: 132, aspectRatio: '4/3', borderRadius: 'var(--crm-radius-xl)', border: 0, cursor: 'pointer', fontFamily: 'inherit', background: WizardTokens.card, boxShadow: `inset 0 0 0 1.5px ${WizardTokens.line}`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 'var(--crm-space-md)', color: WizardTokens.ink }}>
            <span style={{ width: 34, height: 34, borderRadius: 'var(--crm-radius-md)', background: WizardTokens.cardSubtle, display: 'grid', placeItems: 'center' }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={WizardTokens.ink} strokeWidth="2.2" strokeLinecap="round"><path d="M12 5v14M5 12h14" /></svg>
            </span>
      ...
      {cropId && (() => {
        ...
        return ph ? <CrmCropModal photo={ph} onClose={() => setCropId(null)} onApply={(file) => { applyCrop(cropId, file); setCropId(null) }} /> : null
      })()}
    </div>
  )
}
...
function CrmCropModal({ photo, onClose, onApply }: { photo: WizardPhoto; onClose: () => void; onApply: (file: File) => void }) {
  const { t } = useTranslation('listings')
  ...
  return createPortal(
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(3,3,3,0.55)', backdropFilter: 'blur(3px)', display: 'grid', placeItems: 'center', padding: 'var(--crm-space-7xl)' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: WizardTokens.card, borderRadius: 'var(--crm-radius-6xl)', padding: 26, boxShadow: '0 40px 100px rgba(15,23,42,0.28)', maxWidth: 760, width: '100%', animation: 'sgFadeUp .3s cubic-bezier(.2,.8,.2,1) both' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 'var(--crm-space-xl)', marginBottom: 18, flexWrap: 'wrap' }}>
          <h3 style={{ margin: 0, fontSize: 'var(--crm-text-4xl)', fontWeight: 500, color: WizardTokens.ink, letterSpacing: -0.3 }}>{t('wizard.step4.cropTitle')}</h3>
          <div style={{ display: 'flex', gap: 'var(--crm-space-sm)' }}>
            {SG_CROP_ASPECTS.map(a => {
              const sel = aspect === a.r
              return <button key={a.k} onClick={() => setA(a.r)} style={{ height: 32, padding: '0 var(--crm-space-xl)', borderRadius: 'var(--crm-radius-pill)', border: 0, cursor: 'pointer', fontFamily: 'inherit', fontSize: 'var(--crm-text-md)', fontWeight: 600, background: sel ? WizardTokens.black : WizardTokens.cardSubtle, color: sel ? crmOn() : WizardTokens.inkSoft }}>{a.k === 'free' ? t('wizard.step4.aspectFree') : a.label}</button>
            })}
          </div>
        </div>

        <div style={{ display: 'grid', placeItems: 'center', background: WizardTokens.cardSubtle, borderRadius: 'var(--crm-radius-2xl)', padding: 'var(--crm-space-3xl)', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 'var(--crm-space-lg)' }}>
          <span style={{ fontSize: 'var(--crm-text-md)', fontWeight: 500, color: WizardTokens.muted }}>{t('wizard.step4.cropHint')}</span>
          <div style={{ display: 'flex', gap: 'var(--crm-space-lg)' }}>
            <button onClick={onClose} style={{ height: 44, padding: '0 var(--crm-space-5xl)', borderRadius: 'var(--crm-radius-pill)', border: 0, cursor: 'pointer', fontFamily: 'inherit', fontSize: 'var(--crm-text-lg)', fontWeight: 600, background: 'transparent', color: WizardTokens.inkSoft }}>{t('common:actions.cancel')}</button>
            <button onClick={apply} style={{ height: 44, padding: '0 var(--crm-space-7xl)', borderRadius: 'var(--crm-radius-pill)', border: 0, cursor: 'pointer', fontFamily: 'inherit', fontSize: 'var(--crm-text-lg)', fontWeight: 600, background: WizardTokens.black, color: crmOn(), boxShadow: '0 8px 22px rgba(3,3,3,0.18)' }}>{t('wizard.step4.crop')}</button>
          </div>
        </div>
      </div>
    </div>,
