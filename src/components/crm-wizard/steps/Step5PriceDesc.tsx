...
import { WizardTokens, crmOn, fmtCHF, type WizardData } from '../tokens'
    ...
    <div style={{ maxWidth: 820, margin: '0 auto', animation: 'sgFadeUp .5s cubic-bezier(.2,.8,.2,1) both' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--crm-space-md)', marginBottom: 44 }}>
        {Array.from({ length: 7 }).map((_, i) => (
          <div key={i} style={{ flex: 1, height: 5, borderRadius: 'var(--crm-radius-pill)', background: i <= 5 ? WizardTokens.black : WizardTokens.line, transition: 'background .25s' }} />
        ))}
      </div>
      ...
      {phase === 'price' && (
        <div style={{ animation: 'sgFadeUp .35s cubic-bezier(.2,.8,.2,1) both' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 40 }}>
            <div style={{ display: 'inline-flex', padding: 'var(--crm-space-xs)', borderRadius: 'var(--crm-radius-pill)', background: WizardTokens.cardSubtle }}>
              {[
                ...
                return (
                  <button key={t.v} onClick={() => setTx(t.v)} style={{
                    ...
                    background: sel ? WizardTokens.black : 'transparent',
                    color: sel ? crmOn() : WizardTokens.inkSoft,
                    fontFamily: 'inherit', fontSize: 'var(--crm-text-lg)', fontWeight: 600, cursor: 'pointer',
                    boxShadow: sel ? WizardTokens.pillShadow : 'none',
                  }}>{t.l}</button>
                )
              })}
            </div>
          ...
          {/* Question (centrée) */}
          <div style={{
            textAlign: 'center', fontSize: 'var(--crm-text-md)', fontWeight: 600, color: WizardTokens.muted,
            marginBottom: 10,
          }}>
          ...
          <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'center', gap: 'var(--crm-space-2xl)' }}>
            <input
              ...
              style={{
                ...
                fontSize: 104, fontWeight: 500, color: WizardTokens.ink, letterSpacing: -4, lineHeight: 1,
                textAlign: 'center', fontVariantNumeric: 'tabular-nums',
              }} />
          </div>
          <div style={{ textAlign: 'center', marginTop: 12, fontSize: 'var(--crm-text-4xl)', fontWeight: 500, color: WizardTokens.muted }}>
          ...
          {transaction === 'location' && (
            <div style={{ display: 'flex', justifyContent: 'center', marginTop: 24 }}>
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--crm-space-xl)', padding: 'var(--crm-space-lg) var(--crm-space-xl) var(--crm-space-lg) var(--crm-space-5xl)', borderRadius: 'var(--crm-radius-pill)', background: WizardTokens.cardSubtle }}>
                <span style={{ fontSize: 'var(--crm-text-md)', fontWeight: 600, color: WizardTokens.muted}}>{tr('wizard.step5.chargesLabel')}</span>
                <input type="text" inputMode="numeric"
                  value={fmtCHF(data.charges)} onChange={e => onChargesChange(e.target.value)} placeholder="350"
                  style={{
                    width: 84, height: 36, border: 0, outline: 'none', background: WizardTokens.card, borderRadius: 'var(--crm-radius-pill)',
                    textAlign: 'center', fontFamily: 'inherit', fontSize: 'var(--crm-text-2xl)', fontWeight: 600, color: WizardTokens.ink,
                    boxShadow: WizardTokens.shadowSm, fontVariantNumeric: 'tabular-nums',
                  }} />
                <span style={{ fontSize: 'var(--crm-text-lg)', fontWeight: 600, color: WizardTokens.inkSoft }}>CHF</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div style={{ animation: 'sgFadeUp .35s cubic-bezier(.2,.8,.2,1) both' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 28 }}>
            <button onClick={() => setPhase('price')} style={{
              ...
              background: WizardTokens.card, boxShadow: WizardTokens.shadowSm, cursor: 'pointer',
              fontFamily: 'inherit', fontSize: 'var(--crm-text-lg)', fontWeight: 600, color: WizardTokens.ink,
            }}>
              <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke={WizardTokens.inkSoft} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"><path d="m15 18-6-6 6-6" /></svg>
              {display || '0'} CHF{transaction === 'location' ? tr('wizard.perMonth') : ''}
              <span style={{ fontSize: 'var(--crm-text-md)', fontWeight: 600, color: WizardTokens.muted }}>· {tr('wizard.step5.modify')}</span>
            </button>
          </div>

          <div style={{ textAlign: 'center', marginBottom: 26 }}>
            <h1 style={{ margin: 0, fontSize: 'var(--crm-text-8xl)', fontWeight: 500, color: WizardTokens.ink, letterSpacing: -0.8 }}>{tr('wizard.step5.descTitle')}</h1>
          </div>

          {/* Éditeur — feuille + toolbar/compteur qualité en bas */}
          <div style={{ background: WizardTokens.card, borderRadius: 'var(--crm-radius-5xl)', boxShadow: WizardTokens.shadow, overflow: 'hidden' }}>
            <textarea
              value={visibleDesc}
              ...
              style={{
                ...
                fontSize: 'var(--crm-text-xl)', lineHeight: 1.65, color: WizardTokens.ink, fontWeight: 500,
                letterSpacing: -0.1, background: 'transparent',
              }} />
            <div style={{ padding: aiMenuOpen ? 8 : '10px 14px 10px 18px', background: WizardTokens.cardSubtle, transition: 'padding .15s ease' }}>
              {!aiMenuOpen ? (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 'var(--crm-space-2xl)' }}>
                  ...
                  {aiPhase === 'error' ? (
                    <span style={{ fontSize: 'var(--crm-text-sm)', fontWeight: 600, color: WizardTokens.err }}>{tr('wizard.step5.ai.error')}</span>
                  ) : (
                    <span style={{
                      fontSize: 'var(--crm-text-sm)', fontWeight: 600, fontVariantNumeric: 'tabular-nums',
                      color: charCount >= idealChars ? WizardTokens.ok : charCount >= minChars ? WizardTokens.warn : WizardTokens.muted,
                    }}>
                      {charCount === 0
                  ...
                  <button onClick={() => !busy && setAiMenuOpen(true)} aria-label={tr('wizard.step5.ai.title')} title={tr('wizard.step5.ai.title')} style={{
                    ...
                    background: WizardTokens.card, color: WizardTokens.ink, display: 'grid', placeItems: 'center',
                    cursor: busy ? 'wait' : 'pointer', boxShadow: WizardTokens.shadowSm,
                  }}>
                    {busy ? (
                      <div style={{ width: 15, height: 15, borderRadius: 'var(--crm-radius-pill)', border: `2px solid ${WizardTokens.line}`, borderTopColor: WizardTokens.ink, animation: 'sgSpin .8s linear infinite' }} />
                    ) : (
                      <AiStar color={WizardTokens.ink} />
                    )}
                  </button>
                </div>
              ) : (
                /* Barre de prompt MEGGA AI — étoile + saisie libre + Affiner/Créer */
                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--crm-space-md)', background: WizardTokens.card, borderRadius: 'var(--crm-radius-pill)', padding: 'var(--crm-space-sm) var(--crm-space-md) var(--crm-space-sm) var(--crm-space-3xl)', boxShadow: WizardTokens.shadowSm }}>
                  <AiStar color={WizardTokens.ink} />
                  <input autoFocus value={aiPrompt}
                    ...
                    style={{ flex: 1, minWidth: 0, height: 30, border: 0, outline: 'none', background: 'transparent', fontFamily: 'inherit', fontSize: 'var(--crm-text-xl)', fontWeight: 500, color: WizardTokens.ink }} />
                  <button onClick={() => { setAiMenuOpen(false); setAiPrompt('') }} style={{
                    ...
                    color: WizardTokens.inkSoft, fontFamily: 'inherit', fontSize: 'var(--crm-text-lg)', fontWeight: 600, cursor: 'pointer', flexShrink: 0,
                  ...
                  <button onClick={runPrompt} style={{
                    ...
                    background: WizardTokens.black, color: crmOn(), fontFamily: 'inherit', fontSize: 'var(--crm-text-lg)', fontWeight: 600, cursor: 'pointer',
