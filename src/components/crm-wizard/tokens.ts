...
import i18nSg from '@/i18n' // labels d'étapes i18n (getters WIZARD_STEPS)
import { MXC_COLOR, mxCrmPalette } from '@/components/megga-x-crm/tokens'
import { crmMix } from '@/components/crm/tokens'
//
// Deux thèmes (light + dark). `WizardTokens` n'est PAS un objet figé : c'est un Proxy
// qui lit le thème actif (`__sgActive`) au moment de l'accès. Le shell réassigne
// le thème via `setWizardDark(dark)` au tout début de son render, et comme tous
// les step files lisent `WizardTokens.foo` AU RENDER, ils suivent automatiquement le
// thème — zéro threading de prop vers chacun.
//
// Pourquoi un Proxy et pas un simple `let` réassigné ? Identité stable : un import
// `{ WizardTokens }` garde la même référence quel que soit le thème actif, donc aucune
// capture (`const SP = WizardTokens`) ne se retrouve figée sur l'ancien objet.
//
// ─── Direction ───────────────────────────────────────────────────────────
...
// TOUJOURS blanc — d'où `crmOn()` devenue constante, plus bas.
//
...
const ACCENT_HOVER_LIGHT = crmMix(MXC_COLOR.accent, '#000000', 0.14)
const ACCENT_HOVER_DARK = crmMix(MXC_COLOR.accent, '#FFFFFF', 0.12)
...
export type WizardPalette = typeof WIZARD_LIGHT

const WIZARD_DARK: WizardPalette = {
  bg: MX_DARK.pageBg,
 ...
 * `WizardTokens.*` est correcte quel que soit le timing de render. Le Proxy garde une identité
 * stable (aucune capture `const SP = WizardTokens` ne se fige).
 */
export function setWizardDark(dark: boolean | null): void {
  __forceDark = dark
}

function crmIsDarkActive(): boolean {
...
function sgActive(): WizardPalette {
  return crmIsDarkActive() ? WIZARD_DARK : WIZARD_LIGHT
}

// `WizardTokens` = Proxy à identité stable qui résout le thème actif à chaque lecture.
export const WizardTokens: WizardPalette = new Proxy({} as WizardPalette, {
  get: (_t, k) => (sgActive() as Record<PropertyKey, unknown>)[k],
  has: (_t, k) => k in WIZARD_LIGHT,
  ownKeys: () => Reflect.ownKeys(WIZARD_LIGHT),
 ...
 */
export const crmOn = (): string => MXC_COLOR.n1000

/** Voile translucide POSÉ SUR l'accent — blanc, pour la raison ci-dessus. */
export const crmAcc = (a: number): string => `rgba(255,255,255,${a})`
 ...
 *
 * ⚠ Séparé de `crmAcc` le 11 août 2026. Les deux coïncidaient tant que l'accent
 ...
 * carte de l'étape Adresse, qui portent `WizardTokens.muted` en encre, seraient
 * devenues du gris sur du blanc au milieu d'une carte sombre.
 */
export const crmVeil = (a: number): string =>
...
export const CRM_ANIM_KEYFRAMES = `
  @keyframes sgFadeUp {
