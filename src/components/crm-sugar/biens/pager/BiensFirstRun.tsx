    ...
    >
      ...
      <div style={{ position: 'relative', zIndex: 2, height: '100%', display: 'flex', alignItems: 'center', padding: '0 7%' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', alignItems: 'start', width: '100%' }}>
          <h1
            style={{ margin: 0, maxWidth: 460, fontSize: 40, fontWeight: 500, lineHeight: 1.2, color: '#FFFFFF', textShadow: '0 0 18px rgba(255,255,255,0.55), 0 0 6px rgba(255,255,255,0.35)', whiteSpace: 'pre-line' }}
          >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 40, maxWidth: 440 }}>
            {steps.map((n) => (
              <div key={n}>
                <div style={{ fontSize: 44, fontWeight: 500, lineHeight: 1, marginBottom: 14 }}>{String(n).padStart(2, '0')}</div>
                <div style={{ fontSize: 'var(--crm-text-2xl)', fontWeight: 600, marginBottom: 9 }}>{t(`biens.firstRun.step${n}.title`)}</div>
            ...
            <button
              type="button"
              onClick={onStart}
              style={{ alignSelf: 'flex-start', height: 46, padding: '0 34px', borderRadius: 'var(--crm-radius-pill)', border: 'none', cursor: 'pointer', background: sp.accent, color: sp.accentInk, fontFamily: 'inherit', fontSize: 'var(--crm-text-xl)', fontWeight: 600, marginTop: 8 }}
            >
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, background: sp.pageBg, display: 'grid', placeItems: 'center', fontFamily: '"Inter Tight", system-ui, sans-serif' }}>
      <div style={{ textAlign: 'center', maxWidth: 460, padding: '0 var(--crm-space-7xl)' }}>
        <h2 style={{ margin: 0, fontSize: 'var(--crm-text-5xl)', fontWeight: 500, letterSpacing: -0.5, color: sp.ink }}>{t('biens.followUp.emptyFirstRun.title')}</h2>
