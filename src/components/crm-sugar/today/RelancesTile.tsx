...
const SEED_TOTAL = 47
const SEED_SEG = [
  { key: 'chaud', label: 'chauds', count: 8, color: '#E08A45' },
  { key: 'tiede', label: 'tièdes', count: 21, color: '#F2B855' },
  { key: 'froid', label: 'froids', count: 18, color: '#6F8CFF' },
] as const
...
function bucketize(scores: number[]) {
  ...
  return [
    { key: 'chaud', label: 'chauds', count: chaud, color: '#E08A45' },
    { key: 'tiede', label: 'tièdes', count: tiede, color: '#F2B855' },
    { key: 'froid', label: 'froids', count: froid, color: '#6F8CFF' },
  ]
}
    ...
    <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', minHeight: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 14 }}>
          ...
          <div style={{ paddingBottom: 5 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: TK.inkDim, lineHeight: 1.2, whiteSpace: 'nowrap' }}>leads à relancer</div>
            <div style={{ fontSize: 12, color: TK.sub, fontWeight: 600, whiteSpace: 'nowrap' }}>préparés par l'IA</div>
          </div>
        </div>
        {/* barre température */}
        <div style={{ marginTop: 18 }}>
          <div style={{ display: 'flex', height: 9, borderRadius: 999, overflow: 'hidden', gap: 2 }}>
            {seg.map((s) => (
              <div key={s.key} title={`${s.label} · ${s.count}`} style={{ flex: Math.max(s.count, 0.0001), background: s.color }} />
            ))}
          </div>
          <div style={{ display: 'flex', gap: 16, marginTop: 9 }}>
            {seg.map((s) => (
              <div key={s.key} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <span style={{ fontSize: 11.5, color: TK.sub, fontWeight: 600 }}>{s.label}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ marginTop: 'auto', paddingTop: 16 }}>
        <button
          ...
          <RXIcon name="bolt" size={17} sw={2.1} />Démarrer la session
        </button>
      </div>
    </div>
  )
