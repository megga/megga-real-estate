    ...
    <div className="axf-grid" style={{ height: '100%', width: '100%', display: 'flex', gap: 'var(--crm-space-3xl)' }}>
      ...
      <div className="axf-col-left" style={{ flex: '0 0 clamp(300px, 27%, 396px)', minWidth: 0, minHeight: 0, display: 'flex', flexDirection: 'column', gap: 'var(--crm-space-3xl)' }}>
        <div style={{ flex: '0.98 1 0', minHeight: 0, display: 'grid' }}>
          <div style={{ background: A.card, borderRadius: 'var(--crm-radius-6xl)', padding: '26px 28px', boxShadow: A.shadowLg, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 'var(--crm-space-3xl)', minHeight: 0, minWidth: 0, overflow: 'hidden' }}>
            <div style={{ fontSize: 'var(--crm-text-sm)', fontWeight: 800, letterSpacing: 1.3, textTransform: 'uppercase', color: A.muted }}>{tr('analytics.firstrun.objectiveEyebrow')}</div>
            ...
            {target > 0 && (
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 'var(--crm-space-sm)', padding: 'var(--crm-space-xs) var(--crm-space-xl)', borderRadius: 'var(--crm-radius-pill)', background: A.pillAhead.bg, color: A.pillAhead.fg, boxShadow: A.pillAhead.sh, fontSize: 'var(--crm-text-md)', fontWeight: 800, width: 'max-content' }}>
            ...
            <div style={cap}>
              ...
              {onGoSettings && (
                <>{' '}<button onClick={onGoSettings} style={{ border: 0, background: 'transparent', padding: 0, cursor: 'pointer', fontFamily: 'inherit', fontSize: 'var(--crm-text-md)', fontWeight: 800, color: A.ink, textDecoration: 'underline', textUnderlineOffset: 2 }}>{tr('analytics.hero.modify')}</button></>
              )}
            </div>
          </div>
        </div>
        <div style={{ flex: '1.02 1 0', minHeight: 0, display: 'grid' }}>
          <div style={{ background: A.card, borderRadius: 'var(--crm-radius-6xl)', padding: 'var(--crm-space-4xl) var(--crm-space-6xl) var(--crm-space-3xl)', boxShadow: A.shadow, display: 'flex', flexDirection: 'column', minHeight: 0, minWidth: 0, overflow: 'hidden' }}>
            ...
            <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', gap: 'var(--crm-space-md)', marginTop: 14 }}>
              <div style={{ flex: 58, background: ghost, borderRadius: 'var(--crm-radius-2xl)', minHeight: 34 }} />
              <div style={{ flex: 28, background: ghost, borderRadius: 'var(--crm-radius-2xl)', minHeight: 26, opacity: 0.72 }} />
              <div style={{ flex: 14, background: ghost, borderRadius: 'var(--crm-radius-2xl)', minHeight: 20, opacity: 0.45 }} />
            </div>
      ...
      <div className="axf-col-right" style={{ flex: '1 1 0', minWidth: 0, minHeight: 0, display: 'flex', flexDirection: 'column', gap: 'var(--crm-space-3xl)' }}>
        <div style={{ flex: '1.9 1 0', minHeight: 0, display: 'grid' }}>
          <div style={{ background: A.card, borderRadius: 'var(--crm-radius-6xl)', padding: '20px 26px 14px', boxShadow: A.shadow, display: 'flex', flexDirection: 'column', minHeight: 0, minWidth: 0, overflow: 'hidden' }}>
            <h3 style={{ margin: 0, fontSize: 'var(--crm-text-2xl)', fontWeight: 800, color: A.ink, letterSpacing: -0.4, flexShrink: 0 }}>{tr('analytics.chart.title')}</h3>
        ...
        <div className="axf-strip" style={{ flex: '1 1 0', minHeight: 0, minWidth: 0, display: 'flex', gap: 'var(--crm-space-3xl)' }}>
          ...
          <div style={{ flex: '1.05 1 0', minWidth: 0, minHeight: 0, display: 'grid' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gridTemplateRows: '1fr 1fr', gap: 'var(--crm-space-xl)', minHeight: 0, minWidth: 0 }}>
              {[0, 1, 2, 3].map(i => (
                <div key={i} style={{ background: A.card, borderRadius: 'var(--crm-radius-3xl)', boxShadow: A.shadowSm, padding: 'var(--crm-space-xl) var(--crm-space-2xl)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minWidth: 0, minHeight: 0, overflow: 'hidden' }}>
