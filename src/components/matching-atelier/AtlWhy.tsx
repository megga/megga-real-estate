...
import { ATL_ENGAGE_TONE, ATL_KYC } from './constants'
...
function AtlVerifiedSeal({ size = 17, label }: { size?: number; label: string }) {
  return (
    <svg width={size} height={size} viewBox="0 0 19 19" style={{ flexShrink: 0, display: 'block' }} role="img" aria-label={label}>
      ...
      <path d={ATL_SEAL_D} fill="#0041D9" fillRule="evenodd" clipRule="evenodd" />
    </svg>
  )
}
...
function AtlReasonsGroupes({ reasons }: { reasons: AtelierReason[] }) {
  ...
  const group = (title: string, items: AtelierReason[], tone: 'pos' | 'neg', delay: string) => (
    <div className="atl-rgroup" style={{ animationDelay: delay }}>
      <div className="gh">
        <span className="gt">{title}</span>
      </div>
      {items.map((r, i) => (
        <div className="atl-rgrow" key={i}>
...
export default function AtlWhy({ b, poolCount, onSend, onSkip, onLater, onRelance, onPivot, onStartKyc }: AtlWhyProps) {
  ...
  const engageTone = ATL_ENGAGE_TONE[b.status]

  return (
    <div className="atl-why-anim" key={b.matchId}>
      <div className="atl-why-head">
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div className="atl-why-nm" style={{ color: 'var(--ink)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {b.first} {b.last}
            </div>
      ...
      <div className="atl-why-scroll">
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          <span className={`atl-spill ${engageTone}`}>
            <AtlIcon d="sparkle" size={11} /> {b.engage}
          </span>
        </div>

        {poolCount > 1 && (
          <button className="atl-morebtn" onClick={onPivot}>
