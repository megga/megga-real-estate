  ...
  return (
    <div className="sgk-thumb" style={{ width: size, height: size, borderRadius: radius }}>
      {url
        ...
        : <SgaIcon d="camera" size={15} style={{ color: 'var(--ink-dim)' }} />}
    </div>
  )
}
        ...
        return (
          ...
          >
            ...
            <div className="sgk-menu-txt">
              ...
              <div className="meta nums">{sgaFmtCHF(p.listing.price)}</div>
            </div>
            {current && (
              <span className="sgk-menu-check" aria-label={t('cockpit.currentListing')}>
                <SgaIcon d="check" size={12} sw={2.4} />
              </span>
            )}
          </button>
        )
...
export interface SgaCockpitProps {
...
export default function SgaCockpit({
          ...
          <div className="sgk-id-line">
            ...
            {canSwitch && (
              <span className="sgk-chev" data-open={menuOpen}>
                <SgaIcon d="chevron-down" size={13} sw={2} />
              </span>
            )}
          </div>
          <div className="sgk-price nums">{sgaFmtCHF(listing.price)}</div>
        </div>
      </button>
          ...
          <div className="sgk-file" title={t('cockpit.progressAria', { done, total })}>
            {remaining > 0 ? (
              ...
              <span className="sgk-file-done">
                <SgaIcon d="check" size={13} sw={2.4} />{t('cockpit.queueDone')}
              </span>
            )}
          </div>
        </>
