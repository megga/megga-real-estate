...
export default function AtlSendSheet({ b, L, onClose, onSent }: Props) {
      ...
      <div className="atl-modal" style={{ width: 460, maxWidth: '92vw' }} ref={refPiegeFocus} role="dialog" aria-modal="true" aria-label={t('sendSheet.title', { name })}>
        ...
        <div style={{ padding: '0 24px 8px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          <button className="btn btn-primary" style={{ width: '100%' }} onClick={onWhatsApp} disabled={busy || !hasPhone} autoFocus={hasPhone}>
            <AtlIcon d="message" size={16} /> {t('sendSheet.whatsapp')}
          </button>
          <button className="btn btn-ghost" style={{ width: '100%', boxShadow: 'inset 0 0 0 1px var(--hairline)' }} onClick={onCopy} disabled={busy} autoFocus={!hasPhone}>
            <AtlIcon d={copied ? 'check' : 'copy'} size={15} /> {copied ? t('sendSheet.copied') : t('sendSheet.copyLink')}
          </button>
