...
//
// Choix non évidents :
//  · Montée dans un PORTAIL (AtlOverlayHost), comme les autres overlays. Le handoff
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        {L.agency.phone && (
          <a className="btn btn-ghost btn-sm" href={`tel:${L.agency.phone.replace(/\s/g, '')}`} style={{ textDecoration: 'none' }}>
            <AtlIcon d="phone" size={13} /> {t('common:actions.call')}
          </a>
        )}
      </div>
    </div>
...
export default function AtlAnnonceVue({ L, buyer, onClose, onPropose }: AtlAnnonceVueProps) {
  const { t } = useTranslation('matching')
    ...
    >
          ...
          {hasImg && (
            <div className="sgi-film">
              ...
              {G.length > 4 && (
                ...
                >
                  {filmOpen ? <AtlIcon d="close" size={14} /> : `+${G.length - 4}`}
                </button>
              )}
            </div>
          )}
      ...
      {sheet === 'open' || sheet === 'folding' ? (
        ...
        <div className={'sgi-mini' + (sheet === 'unfolding' ? ' is-closing' : '')}>
          ...
          <span className="mp nums">{atlFmtCHF(L.price)}</span>
