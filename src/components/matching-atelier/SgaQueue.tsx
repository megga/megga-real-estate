...
//   couleur de --sga-kyc-seal, aligné sur le token kycSeal de crm-mobile/tokens.
...
/** Sceau KYC vérifié — recopie de MmVerifiedBadge, couleur via --sga-kyc-seal. */
function SgaVerifiedSeal({ size = 14 }: { size?: number }) {
  ...
  return (
    ...
    >
      ...
      <path d={SGA_SEAL_D} fill="var(--sga-kyc-seal)" fillRule="evenodd" clipRule="evenodd" />
    </svg>
  )
}
  ...
  return (
    <div className={'sga-row' + (exiting ? ` exit-${exiting}` : '')} data-sel={selected} onClick={onClick}>
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        {b.kyc === 'verified' && <SgaVerifiedSeal size={14} />}
      </div>
    </div>
  )
}
...
export default function SgaQueue({
  filtered,
  selectedId,
  exiting,
  query,
  ...
  const searchBar = (
    <div className="sga-queue-h" data-pos={searchTop ? 'top' : 'below'}>
  ...
  return (
    <section className="sga-panel sga-enter" aria-label={t('atelier.buyerQueue')}>
      {searchTop && searchBar}
      {searchTop ? (
        <div className="sga-queue-head" data-compact="true">
      ...
      {snoozed.length > 0 && (
        <div className="sga-snoozed sga-programmer" data-open={snzOpen}>
          <button
                  ...
                  {g.rows.map(({ b, until }) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="sga-prog-actions">
                          {onReschedule && (
                            <button
