...
import { fmtM, sgaFmtCHF, sgaInitials, sgaReturnDate, sgaScoreColor } from './format'
  ...
  return (
    <div className={'sgb-lrow' + (exiting ? ` exit-${exiting}` : '')} data-sel={selected} onClick={onClick}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div className="sub nums">{sgaFmtCHF(m.L.price)} · {m.L.rooms ?? '—'} p · {m.L.area ?? '—'} m²</div>
      </div>
      <span className="sc nums" style={{ color: sgaScoreColor(m.score) }}>{m.score}</span>
    </div>
  )
}
      ...
      <div className="sgb-pivot-scroll">
        <div className="sgb-id">
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'center' }}>
            <span className={`chip sm tone-${kyc.tone}`}><SgaIcon d="shield" size={12} /> {kyc.label}</span>
...
interface SgaAcheteurModeProps {
  b: AtelierBuyer
...
export default function SgaAcheteurMode({ b, pool, gestes, onOpenDeal }: SgaAcheteurModeProps) {
  ...
  const snoozed: Array<{ m: AtelierPoolMatch; until: string }> = [
    ...
    ...dbSnoozed.map(m => ({ m, until: sgaReturnDate(m.snoozedUntil ?? undefined) })),
  ]
