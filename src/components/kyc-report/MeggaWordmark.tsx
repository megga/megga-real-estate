// MEGGA — Le mot-marque, en SVG INLINE, pour le rapport KYC imprimé.
//
// Mêmes tracés que `public/megga-wordmark.svg`. Inliné plutôt que chargé par
// `<img src>` parce que le rendu headless (Cloudflare Browser Rendering) capture la
// page dès que la sentinelle `#pdf-ready` apparaît : une image encore en vol
// donnerait un en-tête vide et un filigrane absent, sans qu'aucune erreur ne le
// dise. Un SVG inline est peint avec le DOM — il ne peut pas manquer.
//
// `fill: currentColor` : la même pièce sert l'en-tête (encre pleine, 11 px de
// haut) et le filigrane (520 px de large, opacité 5,5 %) — la couleur et
// l'opacité viennent du conteneur.

import type { CSSProperties } from 'react'

/** Rapport largeur/hauteur du tracé (viewBox 1920 × 419). */
const RATIO = 1920 / 419
...
export function MeggaWordmark({ width, height, style }: MeggaWordmarkProps) {
  const h = height ?? (width != null ? width / RATIO : 11)
