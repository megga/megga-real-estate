...
function formatFilterSummary(filters: SavedSearchFilters): string {
  ...
  if (filters.rooms) parts.push(`${filters.rooms}+ p.`)
        ...
        <div className="overflow-y-auto flex-1 scrollbar-hide">
          {searches.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 px-6">
              ...
              <p className="text-sm font-medium text-gray-700 mb-1">{t('search.noSavedSearches')}</p>
              <p className="text-xs text-gray-500 text-center">{t('search.noSavedSearchesDesc')}</p>
            </div>
          ) : (
                ...
                >
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      ...
                      <button
                        ...
                        title={search.alertEnabled ? t('search.disableAlerts') : t('search.enableAlerts')}
                      >
                      ...
                      <button
                        ...
                        className="h-7 w-7 rounded-full flex items-center justify-center text-gray-500 hover:text-red-500 hover:bg-red-50 transition-colors opacity-0 group-hover:opacity-100"
                  ...
                  <div className="flex items-center gap-3 mt-2 text-xs text-gray-500">
                    {search.resultsCount > 0 && (
                      <span>{search.resultsCount.toLocaleString('fr-CH')} {t('search.properties')}</span>
                    )}
                    <span>{formatRelativeDate(search.createdAt)}</span>
                    {search.alertEnabled && (
                      <span className="text-accent font-medium">
                        {t('search.alert')} {search.alertFrequency === 'daily' ? t('search.frequencyDaily') : t('search.frequencyWeekly')}
                      </span>
                    )}
                  </div>
                </div>
