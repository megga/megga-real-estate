...
function SignalRow({ s, first, onCta, tk }: {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '11px 0', borderTop: first ? 'none' : `1px solid ${tk.hair}`, minWidth: 0 }}>
      <span style={{ position: 'relative', width: 30, height: 30, flexShrink: 0 }}>
        <span style={{ width: 30, height: 30, borderRadius: 999, background: s.av, color: '#fff', display: 'grid', placeItems: 'center', fontSize: 11, fontWeight: 700 }}>{s.initials}</span>
      </span>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 12.5, fontWeight: 600, color: tk.inkSoft, lineHeight: 1.35, overflow: 'hidden', display: '-webkit-box', WebkitBoxOrient: 'vertical', WebkitLineClamp: 2 }}>
          <span style={{ fontWeight: 800, color: tk.ink }}>{s.who.split(' ')[0]}</span> {s.text}
        </div>
        <div style={{ marginTop: 2, fontSize: 10.5, fontWeight: 600, color: s.late ? '#C45A00' : tk.muted, fontVariantNumeric: 'tabular-nums' }}>{s.meta}</div>
      </div>
      <button
        onClick={() => onCta(s)}
        style={{ flexShrink: 0, border: 0, background: 'transparent', cursor: 'pointer', fontFamily: 'inherit', fontSize: 11.5, fontWeight: 800, color: tk.ink, padding: '10px 0 10px 8px', whiteSpace: 'nowrap' }}
      >{s.cta}</button>
    </div>
  )
}
...
export function MobileTodayHScreen() {
  ...
  const secTitle = { margin: '18px 4px 8px', fontSize: 10.5, fontWeight: 800, letterSpacing: 1.1, textTransform: 'uppercase' as const, color: tk.muted }
  const card = (extra?: React.CSSProperties): React.CSSProperties => ({ borderRadius: 18, background: tk.card, boxShadow: tk.shadow, ...extra })
  ...
  return (
    <div style={{ padding: '0 16px', fontFamily: MOBILE_FONT }}>
      ...
      <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: 'calc(env(safe-area-inset-top) + 14px) 2px 6px' }}>
        ...
        <button
          ...
          style={{ width: 38, height: 38, borderRadius: 999, border: 0, background: tk.cardSubtle, display: 'grid', placeItems: 'center', cursor: 'pointer' }}
        >
          <MEIcon name="search" size={18} color={tk.ink} strokeWidth={2.1} />
        </button>
      </header>

      <h1 style={{ margin: '6px 0 0', fontSize: 23, fontWeight: 800, letterSpacing: -0.7, color: tk.ink, lineHeight: 1.1 }}>{t('today.h.mobileTitle')}</h1>
      <div style={{ marginTop: 3, fontSize: 12, fontWeight: 700, color: tk.muted, fontVariantNumeric: 'tabular-nums' }}>
        ...
        <div key={sel.id} style={card({ marginTop: 14, overflow: 'hidden', boxShadow: tk.shadowLg, animation: 'mthIn .3s cubic-bezier(.2,.8,.2,1) both' })}>
          {sel.photo && (
            <div style={{ position: 'relative', aspectRatio: '16 / 7.5', overflow: 'hidden' }}>
              ...
              {sel.price && (
                <span style={{ position: 'absolute', left: 12, bottom: 10, padding: '4px 11px', borderRadius: 999, background: 'rgba(11,12,14,0.78)', color: '#fff', fontSize: 11, fontWeight: 800, fontVariantNumeric: 'tabular-nums' }}>
          ...
          <div style={{ padding: '15px 16px 16px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <span style={{ width: 40, height: 40, borderRadius: 999, flexShrink: 0, display: 'grid', placeItems: 'center', background: sel.av, color: '#fff', fontSize: 12.5, fontWeight: 800 }}>{sel.initials}</span>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 15.5, fontWeight: 800, letterSpacing: -0.3, color: tk.ink }}>{sel.contact}</div>
                {sel.role && <div style={{ marginTop: 1, fontSize: 11, fontWeight: 700, color: tk.muted }}>{sel.role}</div>}
              </div>
              <span style={{ flexShrink: 0, padding: '5px 11px', borderRadius: 999, background: blockColor(sel.kind, isDark), color: '#fff', fontSize: 10.5, fontWeight: 800, fontVariantNumeric: 'tabular-nums' }}>
                {sel.time} · {t('today.h.minutes', { count: sel.dur })}
              </span>
            </div>
            {sel.line && (
              <div style={{ marginTop: 11, fontSize: 13, fontWeight: 600, color: tk.inkSoft, lineHeight: 1.55, textDecoration: sel.done ? 'line-through' : 'none', opacity: sel.done ? 0.6 : 1 }}>{sel.line}</div>
            )}
            <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
              <button
                onClick={() => nav(sel.route, sel.navRef)}
                style={{ flex: 1, height: 44, borderRadius: 999, border: 0, cursor: 'pointer', fontFamily: 'inherit', fontSize: 13, fontWeight: 800, background: tk.accent, color: tk.accentInk }}
              >{sel.cta}</button>
              ...
              {canMarkDone(sel) && (
                <button
                  ...
                  style={{ width: 44, height: 44, borderRadius: 999, border: 0, cursor: 'pointer', flexShrink: 0, display: 'grid', placeItems: 'center', background: sel.done ? '#059669' : tk.cardSubtle, color: sel.done ? '#fff' : tk.ink }}
                >
      ...
      {!isLoading && !day.blocks.length ? (
        <div style={card({ padding: '20px 16px', textAlign: 'center' })}>
          <div style={{ fontSize: 14, fontWeight: 800, color: tk.ink }}>{t('today.h.day.emptyTitle')}</div>
          <div style={{ marginTop: 4, fontSize: 12, fontWeight: 600, color: tk.muted, lineHeight: 1.5 }}>{t('today.h.day.emptyDesc')}</div>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
          {timeline.map((it) => {
            if (it.kind === 'free') {
              ...
              return (
                <div key="free" style={{ display: 'flex', gap: 10, alignItems: 'stretch' }}>
                  <span style={{ width: 40, flexShrink: 0, paddingTop: 12, fontSize: 10.5, fontWeight: 800, color: tk.muted, fontVariantNumeric: 'tabular-nums', textAlign: 'right' }}>{hm(it.from)}</span>
                  <div style={{ flex: 1, minWidth: 0, borderRadius: 14, border: `1.5px dashed ${tk.hair}`, padding: '11px 13px' }}>
                    <span style={{ fontSize: 12, fontWeight: 800, color: tk.ink }}>{t('today.h.freeWindow', { duration: label })}</span>
                    {day.free && (
                      <span style={{ display: 'block', marginTop: 1, fontSize: 10.5, fontWeight: 600, color: tk.muted, fontVariantNumeric: 'tabular-nums' }}>
            ...
            return (
              <div key={b.id} style={{ display: 'flex', gap: 10, alignItems: 'stretch' }}>
                <span style={{ width: 40, flexShrink: 0, paddingTop: 12, fontSize: 10.5, fontWeight: 800, color: current ? tk.ink : tk.muted, fontVariantNumeric: 'tabular-nums', textAlign: 'right' }}>{b.time}</span>
                <button
                  onClick={() => setSelId(b.id)}
                  style={{ flex: 1, minWidth: 0, borderRadius: 14, border: 0, cursor: 'pointer', fontFamily: 'inherit', padding: '11px 13px', textAlign: 'left', background: col, color: '#fff', opacity: b.done ? 0.55 : 1, boxShadow: on ? '0 0 0 2px rgba(255,255,255,0.92) inset' : 'none' }}
                >
                  <span style={{ display: 'flex', alignItems: 'center', gap: 7, minWidth: 0 }}>
                    <span style={{ flex: 1, minWidth: 0, fontSize: 12.5, fontWeight: 800, letterSpacing: -0.1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', textDecoration: b.done ? 'line-through' : 'none' }}>{b.contact}</span>
                    {b.done && <MEIcon name="check" size={13} color="#fff" strokeWidth={2.6} />}
                  </span>
                  <span style={{ display: 'block', marginTop: 1, fontSize: 10.5, fontWeight: 600, opacity: 0.85, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
      ...
      {/* Pendant ton absence */}
      <div style={{ ...secTitle, display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', gap: 10 }}>
        <span>{t('today.h.absence.title')}</span>
        {absenceTotal > 0 && (
          <button
            onClick={() => setSheet(true)}
            style={{ border: 0, background: 'transparent', cursor: 'pointer', fontFamily: 'inherit', fontSize: 11, fontWeight: 800, color: tk.ink, padding: 0, textTransform: 'none', letterSpacing: 0 }}
          >{t('today.h.absence.seeAll', { count: absenceTotal })}</button>
        )}
      </div>
      <div style={card({ padding: '3px 15px' })}>
        {feed.length
          ? feed.map((s, i) => <SignalRow key={s.id} s={s} first={i === 0} onCta={(x) => { void onSignal(x) }} tk={tk} />)
          : <div style={{ padding: '16px 0', fontSize: 12.5, fontWeight: 600, color: tk.muted, textAlign: 'center' }}>{t('today.h.absence.emptyTitle')}</div>}
      </div>
      ...
      {sheet && (
        <div onClick={() => setSheet(false)} style={{ position: 'fixed', inset: 0, zIndex: 60, background: tk.overlay }}>
          <div
            ...
            style={{ position: 'absolute', left: 10, right: 10, bottom: 'calc(94px + env(safe-area-inset-bottom, 0px))', maxHeight: '70%', overflowY: 'auto', background: tk.sheetBg, borderRadius: 22, boxShadow: tk.shadowLg, padding: '16px 17px 14px', animation: 'mthSheet .28s cubic-bezier(.2,.8,.2,1) both' }}
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
              ...
              <button
                ...
                style={{ border: 0, background: 'transparent', cursor: 'pointer', fontFamily: 'inherit', fontSize: 11.5, fontWeight: 800, color: tk.muted, padding: '6px 0' }}
              >{t('today.h.absence.markAll')}</button>
            </div>
            <div style={{ fontSize: 11, fontWeight: 600, color: tk.muted, marginTop: 1 }}>
            ...
            {absenceGroups.map((g) => (
              <div key={g.name} style={{ marginTop: 14 }}>
                <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: 1, textTransform: 'uppercase', color: tk.muted }}>{g.name}</div>
      ...
      {toast && (
        <div style={{ position: 'fixed', left: '50%', transform: 'translateX(-50%)', bottom: 'calc(104px + env(safe-area-inset-bottom, 0px))', zIndex: 70, background: tk.accent, color: tk.accentInk, borderRadius: 999, padding: '10px 17px', fontSize: 12, fontWeight: 700, whiteSpace: 'nowrap', maxWidth: '86%', overflow: 'hidden', textOverflow: 'ellipsis', boxShadow: tk.shadowLg, animation: 'mthIn .25s cubic-bezier(.2,.8,.2,1) both' }}>{toast}</div>
      )}
    </div>
  )
}
