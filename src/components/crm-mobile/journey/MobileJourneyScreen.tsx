    ...
    <div style={{ fontFamily: MOBILE_FONT, color: tk.ink }}>
      <header style={{ display: 'flex', alignItems: 'center', padding: 'calc(env(safe-area-inset-top) + 14px) 18px 4px' }}>
        <button type="button" onClick={() => { if (live) navigate('/dashboard/more') }} aria-label={t('mobile.journey.back', { defaultValue: 'Plus' })} style={{ display: 'flex', alignItems: 'center', gap: 'var(--crm-space-sm)', height: 38, padding: '0 var(--crm-space-2xl) 0 var(--crm-space-md)', borderRadius: 'var(--crm-radius-pill)', border: `1px solid ${tk.cardBorder}`, cursor: 'pointer', background: tk.card, boxShadow: tk.shadowSm, fontFamily: 'inherit' }}>
          ...
          <span style={{ fontSize: 'var(--crm-text-lg)', fontWeight: 600, color: tk.ink }}>{t('mobile.journey.back', { defaultValue: 'Plus' })}</span>
        </button>
      </header>

      <div style={{ padding: 'var(--crm-space-xs) var(--crm-space-4xl) 0' }}>
        <h1 style={{ margin: '4px 0 0', fontSize: 'var(--crm-text-6xl)', fontWeight: 500, letterSpacing: -1, color: tk.ink, lineHeight: 1.05 }}>{t('journey.title')}</h1>
        <div style={{ marginTop: 7, fontSize: 'var(--crm-text-xl)', fontWeight: 600, color: tk.inkSoft }}>{t('journey.filters.activeCount', { count: filtered.length })}</div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 'var(--crm-space-md)', overflowX: 'auto', margin: '14px 0 0', padding: 'var(--crm-space-2xs) var(--crm-space-4xl) var(--crm-space-xs)', scrollbarWidth: 'none' }}>
        {FILTERS.map((f) => {
          ...
          return (
            <button key={f} type="button" onClick={() => setFilter(f)} style={{ flexShrink: 0, height: 34, padding: '0 var(--crm-space-3xl)', borderRadius: 'var(--crm-radius-pill)', border: 0, cursor: 'pointer', fontFamily: 'inherit', fontSize: 'var(--crm-text-md)', fontWeight: 600, letterSpacing: -0.2, background: on ? tk.accent : tk.card, color: on ? tk.accentInk : tk.inkSoft, boxShadow: on ? tk.shadow : tk.shadowSm, whiteSpace: 'nowrap' }}>
              {label}
            </button>
          )
        })}
      ...
      <div style={{ padding: 'var(--crm-space-3xl) var(--crm-space-4xl) 0' }}>
        {live && isLoading && dossiers.length === 0 ? (
          ...
          <div style={{ textAlign: 'center', padding: '44px 24px', background: tk.card, borderRadius: 'var(--crm-radius-4xl)', boxShadow: tk.shadowSm, border: `1px solid ${tk.cardBorder}` }}>
            <div style={{ fontSize: 'var(--crm-text-xl)', fontWeight: 600, color: tk.ink }}>{t('mobile.journey.error', { defaultValue: 'Chargement impossible' })}</div>
            <button type="button" onClick={() => refetch()} style={{ marginTop: 16, height: 44, padding: '0 var(--crm-space-6xl)', borderRadius: 'var(--crm-radius-pill)', border: 0, cursor: 'pointer', fontFamily: 'inherit', fontSize: 'var(--crm-text-xl)', fontWeight: 600, background: tk.accent, color: tk.accentInk }}>{t('mobile.journey.retry', { defaultValue: 'Réessayer' })}</button>
          </div>
        ) : filtered.length === 0 ? (
    ...
    <button type="button" onClick={onOpen} style={{ textAlign: 'left', width: '100%', border: `1px solid ${tk.cardBorder}`, background: tk.card, borderRadius: 'var(--crm-radius-5xl)', boxShadow: tk.shadowSm, padding: 'var(--crm-space-3xl) var(--crm-space-3xl) var(--crm-space-2xl)', cursor: 'pointer', fontFamily: 'inherit', display: 'block' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--crm-space-lg)' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 'var(--crm-text-2xl)', fontWeight: 600, color: tk.ink, letterSpacing: -0.4, lineHeight: 1.15, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{d.title}</div>
        ...
        <span style={{ flexShrink: 0, display: 'inline-flex', alignItems: 'center', gap: 'var(--crm-space-xs)', padding: 'var(--crm-space-xs) var(--crm-space-lg)', borderRadius: 'var(--crm-radius-pill)', background: u.bg, color: '#fff', fontSize: 'var(--crm-text-xs)', fontWeight: 600, letterSpacing: 0.3, whiteSpace: 'nowrap' }}>
      ...
      {task ? (
        <div style={{ marginTop: 15, display: 'flex', alignItems: 'center', gap: 'var(--crm-space-lg)', padding: 'var(--crm-space-lg) var(--crm-space-xl)', borderRadius: 'var(--crm-radius-xl)', background: tk.accent, color: tk.accentInk }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 'var(--crm-text-xs)', fontWeight: 600, color: tk.mode === 'dark' ? 'rgba(3,3,3,.55)' : 'rgba(255,255,255,.6)', whiteSpace: 'nowrap' }}>{t('mobile.journey.inProgress', { defaultValue: 'En cours' })} · {stage ? stage.label : ''}</div>
            <div style={{ fontSize: 'var(--crm-text-lg)', fontWeight: 600, letterSpacing: -0.2, marginTop: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{task.label}</div>
          </div>
      ...
      {PARCOURS_STAGES.map((s, i) => {
        ...
        return (
          <div key={s.id} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', position: 'relative' }}>
            ...
            <span style={{ marginTop: 7, fontSize: 'var(--crm-text-xs)', fontWeight: status === 'active' ? 600 : 500, color: status === 'todo' ? tk.muted : status === 'active' ? tk.ink : tk.muted, whiteSpace: 'nowrap', letterSpacing: -0.1, textAlign: 'center' }}>{s.label.split(' ').pop()}</span>
          </div>
        )
      })}
    </div>
