...
/** Tracé du sceau (8 lobes + coche en négatif, `evenodd`) — handoff KYC, viewBox 0 0 19 19. */
const SEAL_PATH =
...
/**
 * Bleu du sceau sur un fond SOMBRE. `#0041D9`, le bleu de marque que portent les
 * surfaces claires, tombe à 2,3:1 sur le canvas `#030303` du parcours d'onboarding :
 * la coche s'y devine sans se lire. Cette valeur est la teinte que le CRM emploie
 * déjà en mode sombre pour ce même sceau.
 */
export const VERIFIED_SEAL_ON_DARK = '#6F8CFF'

interface Props {
  /** Côté du carré, en pixels. */
  size?: number
  /** Couleur du sceau. Aucune valeur par défaut : le fond décide, pas le composant. */
  color: string
  /** Nom accessible. Le sceau porte une information, il n'est pas décoratif. */
  ariaLabel: string
}

export default function VerifiedSeal({ size = 20, color, ariaLabel }: Props) {
  return (
    <svg
      width={size}
      height={size}
    ...
    >
      {/* Le disque blanc est SOUS le tracé : la coche du sceau est un négatif
          (`evenodd`), sans lui elle serait un trou sur le fond de la page. */}
      <circle cx="9.5" cy="9.5" r="5.6" fill="#FFFFFF" />
      <path d={SEAL_PATH} fill={color} fillRule="evenodd" clipRule="evenodd" />
    </svg>
  )
}
