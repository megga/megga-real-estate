 ...
 * l'échelle grise de `DossierTokens` : quatorze valeurs par thème, dont le noir Sugar
 ...
 * l'écran, et le plan l'avait attribué à `DossierTokens` : la mesure a montré que la
 * fiche n'importe de `DossierTokens` qu'un formateur de DATE.
 *
 * Le passer en FONCTION de `CrmPalette` est ce qui empêche la recopie de
 * revenir : il n'y a plus de valeur à recopier, seulement un barreau à
 * désigner. Même geste que `ndPalette`, et que `crmPalette` elle-même.
 *
...
export interface DsPal {
  ...
  card: string
  sub: string
  /** Encres, du plus fort au plus faible — même ordre que `CrmPalette`. */
  ink: string
  soft: string
  muted: string
  /**
