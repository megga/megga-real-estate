 ...
 *  3. sinon le glyphe du type, BLANC, sur l'aplat de sa teinte — un léger dégradé de haut
 *     en bas, la famille du logo WhatsApp. ⛔ C'était un voile pâle sous un glyphe teinté
 *     (11 % en clair, 24 % en sombre) : à côté des photos et du logo, les types se
 *     lisaient comme des cases vides, et deux types d'un même domaine se confondaient
 *     (14.09.2026, Julien : « fais pareil pour les icônes des évènements »). Chaque type
 *     a désormais SA teinte (`data.ts`), et c'est elle qui fait la tuile.
 *
 ...
 * ⚠ La tuile ne suit pas le thème : un aplat teinté sous un glyphe blanc se lit pareil
 * sur fond clair et sur fond noir — comme le logo WhatsApp, qui porte ses couleurs de
 * MARQUE. Seuls l'anneau de la pastille (le fond derrière la tuile) et le blanc viennent
 * de l'appelant.
 */
...
interface Props {
  ...
  anneau: string
  /** Encre posée sur une teinte PLEINE (le glyphe de la tuile et de sa pastille) — le blanc. */
...
export default function TuileNotif({ n, anneau, encrePastille, children }: Props) {
  ...
  return (
    <span aria-hidden data-canal={n.canal ?? undefined} style={{
      ...
      // Le dégradé tient la teinte au MILIEU, là où se pose le glyphe : son contraste sous
      // le blanc est celui que `data.ts` a mesuré, ni éclairci ni assombri.
      background: photo || whatsapp ? 'transparent'
        : `linear-gradient(180deg, color-mix(in srgb, ${meta.dot} 86%, ${MXC_COLOR.n1000}), color-mix(in srgb, ${meta.dot} 86%, ${MXC_COLOR.n100}))`,
    }}>
      {photo ? (
        <>
          <img
        ...
        <MEIcon name={meta.icon} size={20} color={encrePastille} strokeWidth={1.9} />
      )}
      {children}
    </span>
  )
