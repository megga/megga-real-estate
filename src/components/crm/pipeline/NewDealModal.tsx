 ...
 * étape (pilules CRM_STAGE_HUE) · valeur. Aucune extraction IA (« Importer un
...
import {
  CRM_STAGE_ORDER, CRM_STAGE_HUE, crmFmtCHF,
  type StageId, type CrmPalette,
} from '../tokens'
 ...
 * sources — `useContactsScreen` et `useListingsScreen`, gatées sur la session — donc
...
export function NewDealModal({ open, onClose, sp, dark, prefill, banc }: Props) {
  ...
  const overlayStyle: React.CSSProperties = {
    ...
    animation: 'crm-overlay-fade .25s ease both',
  }
    ...
    return (
      <div style={{ ...overlayStyle, justifyContent: 'center', alignItems: 'center' }}>
        <div style={{
          ...
          animation: 'crm-fade-up .45s cubic-bezier(.2,.8,.2,1) both',
        }}>
