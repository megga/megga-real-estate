 ...
 * Port 1:1 du handoff crm-screen-pipeline-sugar.jsx (§FilterPill,
 * §StageFilter, §RiskFilter, §PeriodFilter, §SegmentedView).
...
import { CRM_STAGE_ORDER, CRM_STAGE_HUE, type CrmPalette, type StageId } from '../tokens'
...
export function SegmentedView({
  value, onChange, sp,
}: { value: PipelineView; onChange?: (v: PipelineView) => void; sp: CrmPalette }) {
...
function sgFilterIsDark(sp: CrmPalette, dark?: boolean): boolean {
...
export function FilterPill({ sp, label, value, active, children, dark }: FilterPillProps) {
  const { t } = useTranslation('pipeline')
...
// ─── Stage filter (pastilles CRM_STAGE_HUE) ─────────────────────────────
export function StageFilter({
  sp, value, onChange, dark,
}: { sp: CrmPalette; value: StageId[]; onChange: (v: StageId[]) => void; dark: boolean }) {
  const { t } = useTranslation('pipeline')
  const isDark = sgFilterIsDark(sp, dark)
      ...
      {CRM_STAGE_ORDER.map(s => {
        ...
        return (
          <button key={s}
            ...
            <span style={{ width: 8, height: 8, borderRadius: 'var(--crm-radius-pill)', background: CRM_STAGE_HUE[s], flexShrink: 0 }} />
...
export function RiskFilter({
  sp, value, onChange, dark,
}: { sp: CrmPalette; value: RiskFilterValue; onChange: (v: RiskFilterValue) => void; dark: boolean }) {
  const { t } = useTranslation('pipeline')
  const isDark = sgFilterIsDark(sp, dark)
  const rowSel = sp.solidBgSub
  const boxBorder = isDark ? 'rgba(255,255,255,.28)' : '#CDD3DB'
...
export function PeriodFilter({
  sp, value, onChange, dark,
}: { sp: CrmPalette; value: number; onChange: (v: number) => void; dark: boolean }) {
  const { t } = useTranslation('pipeline')
  const isDark = sgFilterIsDark(sp, dark)
  const rowSel = sp.solidBgSub
  const boxBorder = isDark ? 'rgba(255,255,255,.28)' : '#CDD3DB'
