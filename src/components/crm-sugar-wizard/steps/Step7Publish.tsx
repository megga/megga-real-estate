...
function SgP7Seg<T extends string>({ options, value, onChange }: {
  ...
  return (
    <div style={{ display: 'inline-flex', background: SugarV2.cardSubtle, borderRadius: 999, padding: 4, gap: 2 }}>
      {options.map((o) => {
        const sel = value === o.v
        return (
          <button key={o.v} onClick={() => onChange(o.v)} style={{
            height: 38, padding: '0 18px', borderRadius: 999, border: 0, cursor: 'pointer', fontFamily: 'inherit',
    ...
    <div style={{ maxWidth: 860, margin: '0 auto', animation: 'sgFadeUp .5s cubic-bezier(.2,.8,.2,1) both' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 32 }}>
        {Array.from({ length: 7 }).map((_, i) => (
          <div key={i} style={{ flex: 1, height: 5, borderRadius: 999, background: SugarV2.black }} />
        ))}
      </div>

      <div style={{ textAlign: 'center', marginBottom: 26 }}>
        <h1 style={{ margin: 0, fontSize: 34, fontWeight: 700, color: SugarV2.ink, letterSpacing: -0.8 }}>
      ...
      {publicPublish && missing.length > 0 && (
        <div style={{ background: SugarV2.card, borderRadius: 22, boxShadow: SugarV2.shadowLg, padding: '22px 26px', marginBottom: 22 }}>
          <div style={{ fontSize: 15, fontWeight: 800, color: SugarV2.ink, letterSpacing: -0.3 }}>
            {t('wizard.step7.checklist.heading', { count: missing.length })}
          </div>
          <div style={{ fontSize: 12.5, fontWeight: 500, color: SugarV2.muted, marginTop: 4, lineHeight: 1.45 }}>
              ...
              <div key={i} style={{
                ...
                <span style={{
                  ...
                  {c.ok
                    ...
                    : <span style={{ width: 7, height: 7, borderRadius: 999, background: SugarV2.isDark ? '#E0738C' : '#8E1F3D' }} />}
                </span>
                <span style={{
                  flex: 1, fontSize: 13.5, fontWeight: c.ok ? 500 : 700, color: c.ok ? SugarV2.muted : SugarV2.ink,
                ...
                {!c.ok && onGoStep && (
                  <button onClick={() => onGoStep(c.step)} style={{
                    ...
                    fontFamily: 'inherit', fontSize: 12, fontWeight: 700, background: SugarV2.cardSubtle, color: SugarV2.ink,
      ...
      {publicPublish && missing.length === 0 && !mandateSigned && (
        <div style={{ fontSize: 12.5, fontWeight: 600, color: SugarV2.muted, textAlign: 'center', marginBottom: 18, lineHeight: 1.5 }}>
          {t('wizard.step7.mandateNote')}
        </div>
      )}

      {/* ── ANNONCE COMPLÈTE (aperçu) ── */}
      <div style={{ background: SugarV2.card, borderRadius: 22, overflow: 'hidden', boxShadow: SugarV2.shadowLg }}>
        ...
        <div style={{
          ...
          {photos.length > 1 && (
            <span style={{
              position: 'absolute', bottom: 14, right: 14, padding: '5px 11px', borderRadius: 999,
              background: 'rgba(0,0,0,0.65)', color: '#fff', fontSize: 10.5, fontWeight: 600,
            }}>1 / {photos.length}</span>
          )}
        </div>
        {/* Bande miniatures */}
        {photos.length > 1 && (
          <div style={{ display: 'flex', gap: 8, padding: '10px 14px', background: SugarV2.cardSubtle }}>
            {photos.slice(0, 4).map((p, i) => {
              const src = p.previewUrl || p.url || null
              return (
                <div key={i} style={{
                  width: 76, height: 54, borderRadius: 10, overflow: 'hidden',
            ...
            {photos.length > 4 && (
              <div style={{
                width: 76, height: 54, borderRadius: 10, background: SugarV2.card,
                display: 'grid', placeItems: 'center', color: SugarV2.muted, fontSize: 12, fontWeight: 700,
              }}>+{photos.length - 4}</div>
            )}
          </div>
        )}
        ...
        <div style={{ padding: 28 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 18, marginBottom: 20 }}>
            <div style={{ minWidth: 0 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: SugarV2.muted, letterSpacing: 1, textTransform: 'uppercase', marginBottom: 6 }}>
                {tx === 'vente' ? t('wizard.txBadge.sale') : t('wizard.txBadge.rent')} · {data.canton || t('wizard.country')}
              </div>
              <div style={{ fontSize: 24, fontWeight: 700, color: SugarV2.ink, letterSpacing: -0.5, lineHeight: 1.2 }}>
                {typeLabel(data.type, t)}{data.rooms ? ` ${data.rooms} ${t('wizard.step7.roomsWord')}` : ''}{data.addr ? ` — ${data.addr}` : ''}
              </div>
              <div style={{ fontSize: 13, fontWeight: 500, color: SugarV2.muted, marginTop: 4 }}>
                {[data.addr, data.postCode, data.canton].filter(Boolean).join(' · ') || t('wizard.step7.addressPlaceholder')}
              </div>
            </div>
            <div style={{ textAlign: 'right', flexShrink: 0 }}>
              <div style={{ fontSize: 30, fontWeight: 800, color: SugarV2.ink, letterSpacing: -1, fontVariantNumeric: 'tabular-nums' }}>{fmtCHF(price) || '—'}</div>
              <div style={{ fontSize: 11, fontWeight: 700, color: SugarV2.muted, letterSpacing: 0.5, marginTop: 2 }}>
          ...
          <div style={{
            ...
            background: SugarV2.line, borderRadius: 14, overflow: 'hidden', marginBottom: 22,
          }}>
            {specs.map((s, i) => (
              <div key={i} style={{ background: SugarV2.card, padding: '14px 16px' }}>
                <div style={{ fontSize: 10, fontWeight: 700, color: SugarV2.muted, letterSpacing: '.06em', textTransform: 'uppercase' }}>{s.l}</div>
                <div style={{
                  fontSize: 16, fontWeight: 700, color: SugarV2.ink, marginTop: 4, fontVariantNumeric: 'tabular-nums',
                  display: 'flex', alignItems: 'center', gap: 6,
                }}>
                  {s.dpe && <span style={{ width: 8, height: 8, borderRadius: 999, background: '#5AA469' }} />}
                  {s.v}
                </div>
              </div>
            ))}
          ...
          {data.description && (
            <>
              <div style={{ fontSize: 11, fontWeight: 700, color: SugarV2.muted, letterSpacing: '.08em', textTransform: 'uppercase', marginBottom: 8 }}>
                {t('wizard.step7.descriptionLabel')}
              </div>
              <p style={{ margin: '0 0 22px', fontSize: 13.5, color: SugarV2.inkSoft, fontWeight: 500, lineHeight: 1.65 }}>{data.description}</p>
            </>
          )}
          ...
          {featLabels.length > 0 && (
            <>
              <div style={{ fontSize: 11, fontWeight: 700, color: SugarV2.muted, letterSpacing: '.08em', textTransform: 'uppercase', marginBottom: 10 }}>
                {t('wizard.step7.featuresLabel')}
              </div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 24 }}>
                {featLabels.map((f, i) => (
                  <span key={i} style={{ padding: '5px 11px', borderRadius: 999, background: SugarV2.cardSubtle, color: SugarV2.ink, fontSize: 11.5, fontWeight: 600 }}>{f}</span>
                ))}
              </div>
            </>
          )}
          ...
          <div style={{ padding: '14px 16px', borderRadius: 14, background: SugarV2.cardSubtle, display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 40, height: 40, borderRadius: 999, background: SugarV2.black, color: sgOn(), display: 'grid', placeItems: 'center', fontSize: 13, fontWeight: 700 }}>{agentInitials}</div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: SugarV2.ink }}>{agentName}</div>
              <div style={{ fontSize: 11.5, color: SugarV2.muted, fontWeight: 500 }}>{t('wizard.step7.agentLine', { canton: data.canton || t('wizard.country') })}</div>
            </div>
            <button disabled style={{
              height: 34, padding: '0 16px', borderRadius: 999, border: 0,
              background: SugarV2.black, color: sgOn(), fontFamily: 'inherit', fontSize: 12, fontWeight: 700, cursor: 'default',
      ...
      <div style={{ background: SugarV2.card, borderRadius: 22, boxShadow: SugarV2.shadow, padding: 26, marginTop: 22 }}>
        <div style={{ fontSize: 16, fontWeight: 700, color: SugarV2.ink, letterSpacing: -0.3, marginBottom: 20 }}>{t('wizard.step7.optionsTitle')}</div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 22, marginBottom: 20 }}>
          {/* Visibilité */}
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: SugarV2.muted, letterSpacing: 0.6, textTransform: 'uppercase', marginBottom: 10 }}>{t('wizard.step7.visibility.eyebrow')}</div>
            <SgP7Seg
            ...
            />
            <div style={{ fontSize: 12, fontWeight: 500, color: SugarV2.muted, marginTop: 10, lineHeight: 1.45 }}>
          ...
          {/* Quand */}
          <div>
            <div style={{ fontSize: 11, fontWeight: 700, color: SugarV2.muted, letterSpacing: 0.6, textTransform: 'uppercase', marginBottom: 10 }}>{t('wizard.step7.whenPublish')}</div>
            <SgP7Seg
              value={mode}
            ...
            {mode === 'schedule' && (
              <input type="date" min={today} value={schedDate}
                ...
                style={{
                  width: '100%', boxSizing: 'border-box', height: 44, marginTop: 12, padding: '0 14px', borderRadius: 12,
                  border: 0, outline: 'none', fontFamily: 'inherit', fontSize: 14, fontWeight: 600,
            ...
            {mode === 'draft' && (
              <div style={{ fontSize: 12, fontWeight: 500, color: SugarV2.muted, marginTop: 10, lineHeight: 1.45 }}>{t('wizard.step7.draftNote')}</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginTop: 22 }}>
        <div style={{ flex: 1 }} />
        {blocked && (
          <span style={{ fontSize: 12.5, fontWeight: 600, color: SugarV2.muted }}>
            {t('wizard.step7.blockedHint', { count: missing.length })}
          </span>
        )}
        <button onClick={publish} disabled={!canPublish} style={{
          height: 52, padding: '0 28px', borderRadius: 999, border: 0,
          background: canPublish ? SugarV2.black : SugarV2.ghost, color: sgOn(), fontFamily: 'inherit', fontSize: 15, fontWeight: 700,
          cursor: canPublish ? 'pointer' : 'not-allowed', display: 'inline-flex', alignItems: 'center', gap: 10,
