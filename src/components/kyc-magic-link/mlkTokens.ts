...
/*
 * ⛔ LA FAMILLE QUI ENCODE (erreur, alerte, succès, étoiles) N'EST PLUS ICI.
 * Elle est née dans ce fichier sous le nom `MLK_STATUT`, SÉPARÉE de `MLK` parce
 * que `MLK` ne porte que des NEUTRES descendus de l'échelle, et qu'un état ne
 * décore pas : le mêler à la palette neutre prétendrait que la direction le
 * gouverne. Empruntée par trois écrans du CRM, elle a déménagé le 13 septembre
 * 2026 dans `megga-x-crm/statut.ts` (`STATUT_CLAIR`), avec ses mesures.
 */
