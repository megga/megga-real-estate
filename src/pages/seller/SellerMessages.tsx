...
export default function SellerMessages() {
  ...
  const { threads, sendMessage, isSending } = useMessaging(null)
  const [messageText, setMessageText] = useState('')
  const messagesEndRef = useRef<HTMLDivElement>(null)

  // Auto-select seller's first thread
  const sellerThread = threads.find((t) => t.contact_type === 'seller') ?? threads[0] ?? null
  const threadId = sellerThread?.id ?? null

  // Use messaging for the seller's thread
  const { messages: threadMessages } = useMessaging(threadId)
