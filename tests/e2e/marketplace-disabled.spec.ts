...
// l'app React (app.getmegga.com). Les routes d'affichage des annonces redirigent
// vers la vitrine getmegga.com via MarketplaceDisabledRedirect (window.location).
...
// Comportement attendu = redirection externe vers getmegga.com. On NE peut pas
// exiger que getmegga.com réponde (domaine externe, password-gated, hors réseau de
// test) : on vérifie donc que l'app a QUITTÉ sa propre route marketplace
// (window.location pointe vers getmegga.com ou la nav externe a démarré), et qu'AUCUN
...
const DISABLED_ROUTES = [
  ...
  // redirigent désormais aussi vers la vitrine getmegga.com.
  '/about',
  '/contact',
  '/sell',
  '/estimates',
  ...
  '/agents',
  '/agencies',
  // Modèle de connexion déplacé sur la vitrine (getmegga.com/login) — les écrans de
...
test.describe('Marketplace publique désactivée → vitrine getmegga.com', () => {
  for (const path of DISABLED_ROUTES) {
    test(`${path} ne rend plus la marketplace (redirige hors app)`, async ({ page, baseURL }) => {
      // La redirection externe (window.location.replace vers getmegga.com) interrompt
