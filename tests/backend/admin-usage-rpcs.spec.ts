...
describe.skipIf(!HAS_KEYS)('RPC usage admin — gardées super_admin/service, agrégats exacts', () => {
  ...
  beforeAll(async () => {
    ...
    // simple console.warn, les assertions qui en dépendent étaient sautées et les
    // tests passaient à vide (transaction_type 'sell' violait la CHECK depuis toujours).
    ...
    const { data: prop, error: propErr } = await service
      .from('properties')
      .insert({ agency_id: setup.agencyAId, title: `Usage seed ${setup.stamp}`, status: 'active', transaction_type: 'buy' })
      .select('id')
      .single()
    ...
    if (aiErr) throw new Error(`seed ai_usage_logs: ${aiErr.message}`)
    ...
    if (waErr) throw new Error(`seed whatsapp_messages: ${waErr.message}`)
  })
  ...
  it('get_admin_usage_overview porte les agrégats + caps de l\'agence', async () => {
    ...
    expect(Number(mine!.active_properties)).toBeGreaterThanOrEqual(1)
