/**
 * Garde-fou : les encres de `DossierTokens` sont lisibles sur ses propres surfaces.
 *
 * ── POURQUOI CE FICHIER EXISTE ───────────────────────────────────────────────
 * `crm-dossiers/tokens.ts` est entré dans le cliquet de GRAMMAIRE au lot 2 du
 ...
 * 1. Tout jeton employé en `color:` atteint l'AA sur les surfaces de `DossierTokens`.
 ...
 *    seuil de texte.
 * 3. `DossierTokens` est MONO-THÈME, et la clause le dit : c'est un objet statique,
...
import { dossierPalette } from '@/components/crm-dossiers/tokens'
...
/**
 * ⛔ LES DEUX THÈMES, depuis que `DossierTokens` est une FONCTION (16 août 2026).
 *
 * La clause « DossierTokens est mono-thème » de ce fichier avait été écrite pour
 ...
 */
const THEMES = [
  { nom: 'clair', p: dossierPalette(false) },
  { nom: 'sombre', p: dossierPalette(true) },
] as const

/** Les surfaces que la palette porte elle-même — une encre s'y pose. */
function surfacesDe(p: ReturnType<typeof dossierPalette>): Record<string, string> {
...
const SOURCES = balayer(repoPath('src'), [])
  ...
  .filter((s) => /\bdossierPalette\b/.test(s.code))
...
describe('Contraste DossierTokens — les encres d’un objet partagé par cinq surfaces', () => {
   ...
   *
   * Ce fichier interdisait à `DossierTokens` d'avoir une branche de thème — non pas
   ...
   * RÉELLEMENT distincts : si `dossierPalette` cessait de brancher, tout ce
