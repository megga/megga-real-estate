...
describe('géo — l\'endpoint passe devant le gate Basic Auth', () => {
  const env = { ASSETS: { fetch: async () => new Response('asset', { status: 200 }) } }

  /** Requête Cloudflare simulée : `cf` n'existe pas sur un `Request` standard. */
  function requete(url: string, init: RequestInit & { cf?: unknown } = {}) {
    const r = new Request(url, init)
    if (init.cf) Object.defineProperty(r, 'cf', { value: init.cf, configurable: true })
    return r
  }

  const { fetch: workerFetch } = (worker as { default: { fetch: (r: Request, e: unknown) => Promise<Response> } }).default
  const handler = (r: Request, e: unknown) => workerFetch(r, e)

  it('répond 200 SANS mot de passe, là où toute autre page répond 401', async () => {
    // C'est LE test de non-régression du branchement : `/api/geo` n'est ni dans
    // PUBLIC_PAGES ni sous un préfixe ouvert. S'il glissait sous le gate, le CRM
    // recevrait un 401 et la détection serait morte sans un bruit.
    const gatee = await handler(requete('https://megga.ch/pricing'), env)
    expect(gatee.status, 'le gate doit toujours protéger le marketing').toBe(401)

    const geo = await handler(requete('https://megga.ch/api/geo', { cf: { country: 'CH', regionCode: 'ZH' } }), env)
    expect(geo.status).toBe(200)
    expect(await geo.json()).toMatchObject({ country: 'CH', region: 'ZH', language: 'de' })
  })

  it('accepte les deux formes de chemin', async () => {
    for (const url of ['https://megga.ch/api/geo', 'https://megga.ch/api/geo/', 'https://megga.ch/API/GEO']) {
      const r = await handler(requete(url), env)
      expect(r.status, url).toBe(200)
    }
  })

  it('interdit toute mise en cache de la réponse', async () => {
    // La réponse dépend de l'IP et de l'Accept-Language de CHAQUE visiteur :
    // mise en cache, elle servirait l'allemand d'un Zurichois à tout le monde.
    const r = await handler(requete('https://megga.ch/api/geo'), env)
    expect(r.headers.get('Cache-Control')).toBe('no-store')
    expect(r.headers.get('CDN-Cache-Control')).toBe('no-store')
    expect(r.headers.get('Vary')).toContain('Accept-Language')
  })

  it('répond à la préflight du CRM, qui ne porte jamais d\'Authorization', async () => {
    const r = await handler(
      requete('https://megga.ch/api/geo', { method: 'OPTIONS', headers: { Origin: 'https://app.megga.ch' } }),
      env
    )
    expect(r.status).toBe(204)
    expect(r.headers.get('Access-Control-Allow-Origin')).toBe('https://app.megga.ch')
  })

  it('n\'ouvre la réponse qu\'aux origines admises', async () => {
    const permis = await handler(
      requete('https://megga.ch/api/geo', { headers: { Origin: 'https://app.megga.ch' } }), env
    )
    expect(permis.headers.get('Access-Control-Allow-Origin')).toBe('https://app.megga.ch')

    const refuse = await handler(
      requete('https://megga.ch/api/geo', { headers: { Origin: 'https://evil.com' } }), env
    )
    expect(refuse.headers.get('Access-Control-Allow-Origin')).toBeNull()
  })

  it('refuse les méthodes d\'écriture', async () => {
    const r = await handler(requete('https://megga.ch/api/geo', { method: 'POST' }), env)
    expect(r.status).toBe(405)
  })

  it('s\'abstient quand Cloudflare ne fournit rien — cas de l\'aperçu local', async () => {
    const r = await handler(requete('https://megga.ch/api/geo'), env)
    expect(await r.json()).toMatchObject({ country: null, region: null, confidence: 'low' })
  })
})
